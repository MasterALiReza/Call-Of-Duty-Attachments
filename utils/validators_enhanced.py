...
from config.constants import (
    MAX_ATTACHMENT_CODE_LENGTH,
    MIN_ATTACHMENT_CODE_LENGTH,
    MAX_ATTACHMENT_NAME_LENGTH,
    MIN_ATTACHMENT_NAME_LENGTH,
    MAX_COMMENT_LENGTH,
...
class AttachmentValidator:
    ...
    def validate_code(code: str) -> ValidationResult:
        ...
        if not re.match(r"^[A-Za-z0-9_-]+$", code):
            return ValidationResult(False, "validation.attachment.code.invalid_chars")
    ...
    def validate_image(
        ...
        if file_size is not None:
            if file_size > MAX_IMAGE_SIZE_BYTES:
                ...
                return ValidationResult(
                    False, "validation.image.too_large", {"max_mb": int(max_mb)}
                )
...
class CategoryValidator:
    ...
    def validate_category(category: str) -> ValidationResult:
        ...
        if category not in WEAPON_CATEGORIES:
            return ValidationResult(
                False, "validation.category.invalid", {"category": category}
            )
    ...
    def validate_mode(mode: str) -> ValidationResult:
        ...
        if mode not in GAME_MODES:
            return ValidationResult(False, "validation.mode.invalid", {"mode": mode})
    ...
    def validate_weapon(weapon: str, category: str, db) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                False, "validation.weapon.db_error", {"error": str(e)}
            )
...
class TextValidator:
    ...
    @staticmethod
    def validate_length(
        text: str, max_length: int, min_length: int = 1, field_name: str = "text"
    ) -> ValidationResult:
        """
        اعتبارسنجی طول متن
        ...
        if len(text) < min_length:
            return ValidationResult(
                False, f"validation.{field_name}.too_short", {"min": min_length}
            )

        if len(text) > max_length:
            return ValidationResult(
                False, f"validation.{field_name}.too_long", {"max": max_length}
            )
    ...
    def validate_comment(text: str) -> ValidationResult:
        ...
        return TextValidator.validate_length(
            text, MAX_COMMENT_LENGTH, min_length=1, field_name="comment"
        )
    ...
    def validate_ticket_subject(text: str) -> ValidationResult:
        ...
        return TextValidator.validate_length(
            text, MAX_TICKET_SUBJECT_LENGTH, min_length=3, field_name="ticket_subject"
        )
    ...
    def validate_faq_question(text: str) -> ValidationResult:
        """اعتبارسنجی سوال FAQ"""
        return TextValidator.validate_length(
            text, MAX_FAQ_QUESTION_LENGTH, min_length=5, field_name="faq_question"
        )
    ...
    def validate_faq_answer(text: str) -> ValidationResult:
        """اعتبارسنجی پاسخ FAQ"""
        return TextValidator.validate_length(
            text, MAX_FAQ_ANSWER_LENGTH, min_length=10, field_name="faq_answer"
        )
