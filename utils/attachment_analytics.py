...
class AttachmentAnalytics:
    ...
    def __init__(self, db_adapter: "DatabaseAdapter"):
        ...
        self.db = db_adapter
        if not hasattr(self.db, "get_connection") or not hasattr(
            self.db, "transaction"
        ):
            raise ValueError(
                "DatabaseAdapter must support pooled get_connection()/transaction()"
            )
        logger.info("AttachmentAnalytics initialized with DatabaseAdapter")

    async def track_view(
        self, attachment_id: int, user_id: int = None, session_id: str = None
    ) -> bool:
    ...
    async def track_rating(self, attachment_id: int, user_id: int, rating: int) -> bool:
        ...
        success = await self._track_action(
            attachment_id, "rate", user_id, metadata=metadata
        )
        if success:
                ...
                async with conn.cursor() as cursor:
                    ...
                    if user_id:
                        await self._update_user_engagement(
                            user_id, attachment_id, action_type, conn
                        )
            return True
    ...
    async def _update_user_engagement(
        ...
        try:
            if conn is not None:
                async with conn.cursor() as cursor:
                    await self._execute_engagement_update(
                        cursor, user_id, attachment_id, action_type, rating
                    )
            else:
    ...
    async def _execute_engagement_update(
        self,
        cursor,
        user_id: int,
        attachment_id: int,
                    ...
                    if result:
                        ...
                        if stats["total_views"] > 0:
                            stats["engagement_rate"] = (
                                float(stats["total_clicks"]) / stats["total_views"]
                            ) * 100
            ...
            async with self.db.get_connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    for att in attachments:
                        ...
                        stats["total_interactions"] += (att["total_views"] or 0) + (
                            att["total_clicks"] or 0
                        )
        ...
        try:
            async with self.db.transaction() as conn:
                async with conn.cursor() as cursor:
                    ...
                    logger.info(
                        f"✅ Updated performance scores for {len(attachment_ids)} attachments"
                    )
        except Exception as e:
            ...
            async with self.db.get_connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if result:
                        ...
                        scores["popularity"] = (
                            views * 1 + clicks * 3 + shares * 5 + users * 2
                        ) / 10
                        if views > 0:
                    ...
                    rating_count = (
                        result["rating_count"]
                        if result and result.get("rating_count")
                        else 0
                    )
                    if avg_rating:
                    ...
                    # Top flags
                    await cursor.execute(
                        "SELECT is_top, is_season_top FROM attachments WHERE id = %s",
                        (attachment_id,),
                    )
                    result = await cursor.fetchone()
                    if result:
                        ...
                        if result["is_season_top"]:
                            scores["quality"] = min(100, scores["quality"] + 15)

        except Exception as e:
            logger.error(
                f"Error calculating scores for attachment {attachment_id}: {e}"
            )
    ...
    async def get_underperforming_attachments(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            async with self.db.get_connection() as conn:
                async with conn.cursor() as cursor:
                    await cursor.execute(
                        "SELECT AVG(total_views) as avg FROM attachments WHERE total_views > 0"
                    )
                    result = await cursor.fetchone()
                    avg_views = (
                        float(result["avg"]) if result and result.get("avg") else 100.0
                    )
