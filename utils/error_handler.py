...
class ErrorHandler:
    ...
    async def handle_telegram_error(
        self, update: Update, context: CustomContext, error: Exception
    ) -> None:
    ...
    def get_error_stats(self) -> dict:
        ...
        return {
            ...
            "most_common": max(self.error_count, key=self.error_count.get)
            if self.error_count
            else None,
        }
...
def safe_execute(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    این دکوراتور مانع از crash شدن برنامه در اثر استثناهای پیش‌بینی نشده می‌شود.
...
def log_function_call(func):
    ...
    async def async_wrapper(*args, **kwargs):
        ...
        try:
            ...
            elapsed = (datetime.now() - start_time).total_seconds()
            logger.error(
                f"{func.__name__} با خطا مواجه شد بعد از {elapsed:.2f} ثانیه: {e}"
            )
            raise
