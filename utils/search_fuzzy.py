...
class FuzzySearchEngine:
    ...
    async def build_search_index(self, force: bool = False):
        ...
        with self._cache_lock:
            try:
                ...
                logger.info(
                    f"Fuzzy index built in {elapsed:.3f}s: {len(self._cache['weapons'])} weapons, {len(self._cache['attachments'])} attachments, {len(self._cache['codes'])} codes"
                )
            except Exception as e:
                logger.error(f"Error building fuzzy index: {e}")
                raise

    async def fuzzy_match_weapons(
        self, query: str, threshold: int = 70, limit: int = 5
    ) -> List[Tuple[str, int]]:
        ...
        try:
            matches = process.extract(
                query, self._cache["weapons"], scorer=fuzz.ratio, limit=limit
            )
    ...
    async def fuzzy_match_attachments(
        self, query: str, threshold: int = 70, limit: int = 10
    ) -> List[Tuple[str, int]]:
        ...
        try:
            matches = process.extract(
                query, self._cache["attachments"], scorer=fuzz.ratio, limit=limit
            )
            results = [(name, score) for name, score in matches if score >= threshold]
            logger.debug(
                f"Fuzzy attachment matches for '{query}': {len(results)} results"
            )
            return results
        except Exception as e:
    ...
    async def fuzzy_match_codes(
        self, query: str, threshold: int = 80, limit: int = 5
    ) -> List[Tuple[str, int]]:
        ...
        try:
            matches = process.extract(
                query, self._cache["codes"], scorer=fuzz.ratio, limit=limit
            )
    ...
    def fuzzy_match(
        self, query: str, threshold: int = 70
        ...
        return {
            "weapons": process.extract(
                query, self._cache["weapons"], scorer=fuzz.ratio, limit=5
            ),
            "attachments": process.extract(
                query, self._cache["attachments"], scorer=fuzz.ratio, limit=10
            ),
            "codes": process.extract(
                query, self._cache["codes"], scorer=fuzz.ratio, limit=5
            ),
        }
    ...
    async def get_suggestions(self, partial_query: str, limit: int = 5) -> List[str]:
        ...
        if len(suggestions) < limit:
            matches = process.extract(
                partial_query, self._cache["weapons"], scorer=fuzz.ratio, limit=limit
            )
