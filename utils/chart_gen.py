...
class ChartGenerator:
    ...
    def __init__(self, width: int = 800, height: int = 400, theme: str = "dark"):
        ...
        if theme == "dark":
            ...
            self.bar_color = (70, 130, 255)  # Ox Loadout Blue
    ...
    def generate_bar_chart(
        self, data: List[Tuple[str, float]], title: str = ""
    ) -> io.BytesIO:
        """
        ...
        # Draw Axis
        draw.line(
            [(margin_l, margin_t), (margin_l, self.height - margin_b)],
            fill=self.grid_color,
            width=2,
        )
        draw.line(
            [
                (margin_l, self.height - margin_b),
                (self.width - margin_r, self.height - margin_b),
            ],
            fill=self.grid_color,
            width=2,
        )
        ...
        for i, (label, value) in enumerate(data):
            ...
            draw.text(
                (x0 + bar_w // 2, self.height - margin_b + 20),
                display_label,
                fill=self.text_color,
                anchor="mm",
            )
