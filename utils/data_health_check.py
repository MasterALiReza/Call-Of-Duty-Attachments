...
class DataHealthChecker:
    ...
    async def check_missing_images(self) -> List[Dict]:
        ...
        try:
            ...
            if missing_images:
                ...
                logger.warning(
                    f"⚠️ Found {len(missing_images)} attachments without images"
                )
            else:
    ...
    async def check_duplicate_codes(self) -> List[Dict]:
        ...
        try:
            ...
            if local_duplicates:
                self.issues["CRITICAL"].append(
                    {
                        "type": "duplicate_codes_local",
                        "count": len(local_duplicates),
                        "details": local_duplicates,
                    }
                )
                logger.warning(
                    f"⚠️ Found {len(local_duplicates)} duplicate codes within weapons"
                )

            if global_duplicates:
                self.issues["WARNING"].append(
                    {
                        "type": "duplicate_codes_global",
                        "count": len(global_duplicates),
                        "details": global_duplicates,
                    }
                )
                logger.warning(
                    f"⚠️ Found {len(global_duplicates)} codes shared across different weapons"
                )
    ...
    async def check_empty_weapons(self) -> List[Dict]:
        ...
        try:
            ...
            if empty_weapons:
                ...
                if pol == "ignore":
                    logger.info(
                        f"ℹ️ Empty weapons found but ignored by policy: {len(empty_weapons)}"
                    )
                else:
                    sev = (
                        "INFO"
                    ...
                    logger.warning(
                        f"⚠️ Empty weapons reported with severity={sev}: {len(empty_weapons)}"
                    )
            else:
            ...
            async with self.db.get_connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    required = {
                        ...
                        "idx_weapons_name_trgm",
                        "idx_attachments_search_composite",
                        "ux_attachments_weapon_mode_code",
                        "idx_attachments_weapon_mode",
                    ...
                    det = {
                        ...
                        "count": len(missing) + (0 if has_trgm else 1),
                    }
                    details = []
                    if not has_trgm:
        ...
        try:
            async with self.db.get_connection() as conn:
                async with conn.cursor() as cursor:
                    for tbl in tables:
                        await cursor.execute(
                            "SELECT pg_get_serial_sequence(%s, 'id') as seq", (tbl,)
                        )
                        row = await cursor.fetchone()
                        seq = row.get("seq") if row else None
                        if not seq:
                            self.issues["WARNING"].append(
                                {"type": "sequence_missing", "table": tbl, "count": 1}
                            )
                            continue
                        try:
                        ...
                        max_id = (
                            max_row.get("coalesce")
                            if max_row and "coalesce" in max_row
                            else list(max_row.values())[0]
                            if max_row
                            else 0
                        )
                        if last_val < max_id:
    ...
    async def check_sparse_weapons(self) -> List[Dict]:
        ...
        try:
            ...
            if sparse_weapons:
                ...
                logger.info(
                    f"ℹ️ Found {len(sparse_weapons)} weapons with <3 attachments"
                )
        ...
        try:
            async with self.db.get_connection() as conn:
                async with conn.cursor() as cursor:
                    metrics = {}
                    await cursor.execute(
                        "SELECT COUNT(*) as cnt FROM weapon_categories"
                    )
                    metrics["total_categories"] = self._get(
                        await cursor.fetchone(), "cnt"
                    )
                    ...
                    metrics["total_attachments"] = self._get(
                        await cursor.fetchone(), "cnt"
                    )
                    await cursor.execute(
                    ...
                    await cursor.execute(
                        "SELECT COUNT(*) as cnt FROM attachments WHERE is_top = TRUE"
                    )
                    metrics["top_attachments"] = self._get(
                        await cursor.fetchone(), "cnt", default=0
                    )
                    await cursor.execute(
                        "SELECT COUNT(*) as cnt FROM attachments WHERE is_season_top = TRUE"
                    )
                    metrics["season_attachments"] = self._get(
                        await cursor.fetchone(), "cnt", default=0
                    )
                    await cursor.execute(
                        "SELECT COUNT(*) as cnt FROM attachments WHERE image_file_id IS NOT NULL AND image_file_id != ''"
                    )
                    with_images = self._get(await cursor.fetchone(), "cnt", default=0)
                    metrics["attachments_with_images"] = with_images
                    metrics["attachments_without_images"] = (
                        metrics["total_attachments"] - with_images
                    )
                    metrics["image_coverage"] = (
                        with_images / metrics["total_attachments"] * 100
                        if metrics["total_attachments"] > 0
                        else 0
                    )
                ...
                async with tconn.cursor() as cursor:
                    ...
                    if self.metrics:
                        ...
                        total_attachments = int(
                            self.metrics.get("total_attachments", 0)
                        )
                        image_coverage = float(self.metrics.get("image_coverage", 0))
                        with_images = int(total_attachments * image_coverage / 100)
                        without_images = int(
                            total_attachments * (100 - image_coverage) / 100
                        )
                        weapons_with_attachments = max(
                            0, total_weapons - empty_weapons_count
                        )
    ...
    def calculate_health_score(self) -> float:
        ...
        score = 100.0
        critical_count = sum(
            (issue.get("count", 0) for issue in self.issues.get("CRITICAL", []))
        )
        score -= min(critical_count * 5, 50)
        warning_count = sum(
            (issue.get("count", 0) for issue in self.issues.get("WARNING", []))
        )
    ...
    def _generate_text_report(self) -> str:
        ...
        lines.append(
            f"  • Total Attachments: {self.metrics.get('total_attachments', 0)}"
        )
        lines.append(
            f"  • Image Coverage: {self.metrics.get('image_coverage', 0):.1f}%"
        )
        ...
        for severity in ["CRITICAL", "WARNING", "INFO"]:
            if severity in self.issues:
                lines.append(
                    f"{('❌' if severity == 'CRITICAL' else '⚠️' if severity == 'WARNING' else 'ℹ️')} {severity} ISSUES:"
                )
    ...
    def _generate_markdown_report(self) -> str:
        ...
        lines.append("|--------|-------|")
        lines.append(
            f"| Total Categories | {self.metrics.get('total_categories', 0)} |"
        )
        lines.append(f"| Total Weapons | {self.metrics.get('total_weapons', 0)} |")
        lines.append(
            f"| Total Attachments | {self.metrics.get('total_attachments', 0)} |"
        ...
        if self.metrics.get("category_distribution"):
            ...
            for cat in self.metrics["category_distribution"]:
                lines.append(
                    f"| {cat['category']} | {cat['weapons']} | {cat['attachments']} |"
                )
            lines.append("")
        ...
        for severity in ["CRITICAL", "WARNING", "INFO"]:
            if severity in self.issues:
                ...
                for issue in self.issues[severity]:
                    lines.append(
                        f"- **{issue['type']}**: {issue.get('count', 0)} found"
                    )
                lines.append("")
        score = self.calculate_health_score()
        emoji = "🟢" if score >= 80 else "🟡" if score >= 60 else "🔴"
        ...
        if "missing_images" in [i["type"] for i in self.issues.get("CRITICAL", [])]:
            lines.append(
                "1. **Upload missing images** for attachments through admin panel"
            )
        if "duplicate_codes_local" in [
            i["type"] for i in self.issues.get("CRITICAL", [])
        ]:
            lines.append(
                "2. **Fix duplicate codes** (per weapon) to ensure unique identification"
            )
        if "duplicate_codes_global" in [
            i["type"] for i in self.issues.get("WARNING", [])
        ]:
            lines.append(
                "3. **Review shared codes** across weapons (if they should be unique)"
            )
            ...
            async with self.db.get_connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if invalid_images:
                        ...
                        logger.warning(
                            f"⚠️ Found {len(invalid_images)} attachments with suspicious image IDs"
                        )
            ...
            async with self.db.get_connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if stats:
                        ...
                        logger.info(
                            f"ℹ️ Found {len(stats)} frequent/potentially slow searches"
                        )
                    return stats
        except Exception as e:
    ...
    async def run_full_check(self, save_to_db=True) -> Dict:
        ...
        try:
            ...
            report_path = os.path.join(
                report_dir,
                f"health_check_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md",
            )
        ...
        return {
            ...
            "critical_count": sum(
                (i.get("count", 0) for i in self.issues.get("CRITICAL", []))
            ),
            "warning_count": sum(
                (i.get("count", 0) for i in self.issues.get("WARNING", []))
            ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--auto-fix",
        action="store_true",
        help="Automatically fix simple issues (USE WITH CAUTION)",
    )
    parser.add_argument(
        "--no-save", action="store_true", help="Don't save results to database"
    )
    parser.add_argument(
