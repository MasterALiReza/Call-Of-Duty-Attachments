...
class ContentValidator:
    ...
    def check_blacklisted_words(self, text: str) -> Optional[Dict]:
        ...
        for word, info in self.blacklist_cache.items():
            ...
            if normalized_word in normalized:
                logger.warning(
                    f"Blacklisted word detected: '{word}' (category: {info['category']}, severity: {info['severity']})"
                )
                return {
                    "word": word,
    ...
    def validate_text(
        self, text: str, max_length: int = 100, check_spam: bool = True
    ...
    def validate_attachment_submission(
        ...
        name_valid, name_reason, name_violation = self.validate_text(
            name, max_name_length, check_spam=True
        )
        desc_valid = True
        desc_reason = "OK"
        desc_violation = None
        if description and description.strip():
            desc_valid, desc_reason, desc_violation = self.validate_text(
                description, max_desc_length, check_spam=True
            )
        return {
