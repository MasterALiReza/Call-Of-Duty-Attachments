...
class SuggestedAttachmentsManager:
    ...
    async def add_to_suggested(
        ...
        try:
            if not 1 <= priority <= MAX_PRIORITY:
                logger.warning(
                    f"Invalid priority {priority}, using default {DEFAULT_PRIORITY}"
                )
            ...
            if success:
                logger.info(
                    f"Attachment {attachment_id} added to suggested list for {mode} by admin {admin_id}"
                )
            else:
                logger.error(
                    f"Failed to add attachment {attachment_id} to suggested list"
                )
            return success
    ...
    async def remove_from_suggested(self, attachment_id: int, mode: str) -> bool:
        ...
        """
        try:
            success = await self.db.attachments.remove_suggested_attachment(
                attachment_id, mode
            )
            if success:
                logger.info(
                    f"Attachment {attachment_id} removed from suggested list for {mode}"
                )
            else:
                logger.error(
                    f"Failed to remove attachment {attachment_id} from suggested list"
                )
            return success
        except Exception as e:
    ...
    async def clear_all_suggested(self, mode: str = None) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Cleared {count_before} suggested attachments for mode: {mode or 'all'}"
                )
            return success
        except Exception as e:
    ...
    async def is_suggested(self, attachment_id: int, mode: str) -> bool:
        ...
        try:
            return await self.db.attachments.is_attachment_suggested(
                attachment_id, mode
            )
        except Exception as e:
    ...
    def format_suggested_list_text(
        self, items: List[Dict], mode: str, page: int = 1
    ) -> str:
        ...
        if not items:
            return (
                f"❌ هیچ اتچمنت پیشنهادی برای {GAME_MODES.get(mode, mode)} وجود ندارد."
            )
        ...
        for idx, item in enumerate(page_items, start=start_idx + 1):
            ...
            priority_emoji = self._get_priority_emoji(
                attachment.get("priority", DEFAULT_PRIORITY)
            )
    ...
    async def send_suggested_gallery(
        self, update: Update, context: CustomContext, mode: str
    ) -> None:
        ...
        try:
            ...
            if not items:
                keyboard = [
                    [
                        InlineKeyboardButton(
                            "⬅️ بازگشت", callback_data="suggested_attachments"
                        )
                    ],
                ...
                return
            start_message = (
                f"🎯 در حال ارسال {len(items)} اتچمنت پیشنهادی {mode_name}..."
            )
            if update.callback_query:
            ...
            for item in items:
                ...
                if attachment.get("image"):
                    ...
                    media = InputMediaPhoto(
                        media=attachment.get("image"), caption=caption[:1024]
                    )
            ...
            for group in media_groups:
                try:
                    await context.bot.send_media_group(
                        chat_id=update.effective_chat.id, media=group
                    )
                except Exception as e:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "📋 نمایش لیست", callback_data=f"suggested_list_mode_{mode}"
                    )
                ],
                [
                    InlineKeyboardButton(
    ...
    def build_suggested_keyboard(
        self, items: List[Dict], page: int, mode: str
        ...
        if total_pages > 1:
            ...
            if page > 1:
                nav_buttons.append(
                    InlineKeyboardButton(
                        "◀️ قبلی", callback_data=f"sugglist_page_{page - 1}"
                    )
                )
            nav_buttons.append(
            ...
            if page < total_pages:
                nav_buttons.append(
                    InlineKeyboardButton(
                        "▶️ بعدی", callback_data=f"sugglist_page_{page + 1}"
                    )
                )
