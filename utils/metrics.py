...
class QueryMetrics:
        ...
        with self._lock:
            ...
            if duration > SLOW_QUERY_THRESHOLD_SEC:
                ...
                if len(self.slow_query_log) > self._max_slow_log_size:
                    self.slow_query_log = self.slow_query_log[
                        -self._max_slow_log_size :
                    ]
    ...
    def slow_query_rate(self) -> float:
        ...
        return (
            (self.slow_queries / self.total_queries) if self.total_queries > 0 else 0.0
        )
    ...
    def get_stats(self) -> Dict[str, any]:
        return {
            ...
            "average_duration_sec": round(
                self.total_duration / self.total_broadcasts, 2
            )
...
class MetricsCollector:
    ...
    def get_all_stats(self) -> Dict[str, any]:
        ...
        return {
            ...
            "avg_handler_response_ms": round(
                sum(self.handler_response_times)
                / len(self.handler_response_times)
                * 1000,
                2,
            )
            if self.handler_response_times
            else 0,
        }
