...
class LoggerManager:
    ...
    def _setup_root_logger(self):
        ...
        logging.info(
            f"Logging system initialized successfully | Directory: {LOG_DIR.absolute()}"
        )
    ...
    def decorator(func: Callable):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.log(
                    level, f"[SUCCESS] {func_name} completed | Time: {elapsed:.3f}s"
                )
                return result
            except Exception as e:
                elapsed = time.time() - start_time
                logger.error(
                    f"[ERROR] {func_name} failed | Time: {elapsed:.3f}s | Error: {type(e).__name__}: {str(e)}",
                    exc_info=True,
                )
                raise
...
def log_db_operation(operation_name: str = None):
    ...
    def decorator(func: Callable):
        ...
        def wrapper(*args, **kwargs):
            ...
            params_str = (
                f"args={(args[1:3] if len(args) > 1 else args)}, kwargs={kwargs}"
            )
            ...
            try:
                ...
                elapsed = time.time() - start_time
                db_logger.info(
                    f"[DB-SUCCESS] {op_name} | Time: {elapsed:.3f}s | Result: {result}"
                )
                return result
            except Exception as e:
                elapsed = time.time() - start_time
                db_logger.error(
                    f"[DB-ERROR] {op_name} | Time: {elapsed:.3f}s | Error: {type(e).__name__}: {str(e)}",
                    exc_info=True,
                )
                raise
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if update and update.effective_user:
                ...
                user_logger.info(
                    f"[USER] Action: {action} | User: {username} ({user_id})"
                )
            try:
                result = await func(*args, **kwargs)
                return result
                ...
                user_logger.error(
                    f"[USER-ERROR] Action failed: {action} | Error: {str(e)}"
                )
                raise
            finally:
        ...
        async def wrapper(*args, **kwargs):
            ...
            if update and update.effective_user:
                ...
                admin_logger.warning(
                    f"[ADMIN] Action: {action} | Admin: {admin_name} ({admin_id})"
                )
            try:
                result = await func(*args, **kwargs)
                ...
                admin_logger.error(
                    f"[ADMIN-ERROR] Action failed: {action} | Error: {str(e)}",
                    exc_info=True,
                )
                raise
            finally:
...
def log_exception(logger: logging.Logger, exc: Exception, context: str = ""):
    ...
    """
    logger.error(
        f"{('[' + context + '] ' if context else '')}Exception: {type(exc).__name__}: {str(exc)}",
        exc_info=True,
...
def log_performance(
    logger: logging.Logger, operation: str, duration: float, threshold: float = 1.0
):
    """
    ...
    if duration > threshold:
        logger.warning(
            f"[SLOW] Operation: {operation} | Time: {duration:.3f}s (threshold: {threshold}s)"
        )
    else:
