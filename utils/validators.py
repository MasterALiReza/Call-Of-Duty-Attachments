...
# Patterns مجاز
WEAPON_NAME_PATTERN = re.compile(
    r"^[a-zA-Z0-9\-_\s\u0600-\u06FF]+$"
)  # حروف، اعداد، فاصله، فارسی
ATTACHMENT_CODE_PATTERN = re.compile(r"^[A-Z0-9\-]+$")  # فقط حروف بزرگ، اعداد و خط تیره
# فرمت دقیق‌تر برای channel ID
CHANNEL_USERNAME_PATTERN = re.compile(
    r"^@[a-zA-Z][a-zA-Z0-9_]{4,31}$"
)  # @username (5-32 chars)
CHANNEL_NUMERIC_ID_PATTERN = re.compile(
    r"^-100\d{10}$"
)  # -100xxxxxxxxxx (13 digits total)
...
def validate_attachment_code(code: str) -> Tuple[bool, Optional[str]]:
    ...
    if not ATTACHMENT_CODE_PATTERN.match(code):
        return (
            False,
            "کد اتچمنت فقط می‌تواند شامل حروف بزرگ انگلیسی، اعداد و خط تیره باشد",
        )
...
def validate_category(
    category: str, valid_categories: list
