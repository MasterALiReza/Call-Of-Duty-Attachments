...
-- Grant necessary privileges via the database owner role selected at bootstrap.
...
-- Grant all privileges and transfer ownership to the current database owner.
DO $$
DECLARE
    target_role NAME;
    tbl RECORD;
BEGIN
    ...
    IF target_role IS NULL THEN
        RAISE NOTICE 'Skipping table ownership alignment because database owner could not be resolved.';
        RETURN;
    END IF;
    ...
    LOOP
        EXECUTE format('ALTER TABLE %I OWNER TO %I', tbl.tablename, target_role);
    END LOOP;
END
$$;

-- Transfer ownership of all sequences to the current database owner.
DO $$
DECLARE
    target_role NAME;
    seq RECORD;
BEGIN
    ...
    IF target_role IS NULL THEN
        RAISE NOTICE 'Skipping sequence ownership alignment because database owner could not be resolved.';
        RETURN;
    END IF;

    FOR seq IN SELECT sequencename FROM pg_sequences WHERE schemaname = 'public'
    LOOP
        EXECUTE format('ALTER SEQUENCE %I OWNER TO %I', seq.sequencename, target_role);
    END LOOP;
END
$$;
