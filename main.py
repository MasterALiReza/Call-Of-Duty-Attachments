    ...
    if env_proxies or system_proxies:
        ...
        if system_proxies:
            ...
            for k, v in system_proxies.items():
                print(f"     * {k}: {v}")
        print(
            "💡 Hint: If connection fails, try setting BOT_PROXY_URL=NONE in .env to force direct connection."
        )
    else:
...
class OX_LOADOUTAttachmentsBot:
    """کلاس اصلی ربات مدیریت اتچمنت‌های OX_LOADOUT"""
    ...
    def __init__(self):
        ...
        self.alert_system = (
            None  # Will be initialized in post_init once application is ready
        )
        self.application = None
        self.is_shutting_down = False
        logger.info("OX_LOADOUTAttachmentsBot initialized with ServiceContainer")
    ...
    async def post_init(self, application):
        ...
        logger.info("OX_LOADOUT Attachments Bot started successfully!")
        database = self.db
        ...
        if (
            hasattr(self, "user_registry")
            and self.user_registry
            and hasattr(self.user_registry, "initialize")
        ):
    ...
    def run(self):
        ...
        if proxy_url and proxy_url.upper() == "NONE":
            ...
            logger.info(
                "🌐 Using default connection settings (may auto-detect system proxies)"
            )
            request_obj = HTTPXRequest(
                read_timeout=request_timeout,
                connect_timeout=connect_timeout,
    ...
    def _run_webhook(self):
        ...
        if not webhook_secret:
            try:
                secret_file = pathlib.Path(
                    os.getenv("WEBHOOK_SECRET_FILE", ".webhook_secret_token")
                ).expanduser()
                ...
                logger.warning(
                    f"⚠️  Could not persist webhook secret token, generating a temporary one. Error: {e}"
                )
                webhook_secret = secrets.token_urlsafe(32)
        ...
        logger.info(
            f"   SSL: {'Custom cert' if cert_path else 'Reverse proxy / built-in'}"
        )
        if not cert_path and not key_path:
            logger.info(
                "   SSL Note: If you don't provide cert/key, you MUST terminate HTTPS via a reverse proxy (e.g., Nginx) in front of this port."
            )
            logger.info(
                "             Telegram does not deliver webhooks over plain HTTP."
            )

        # Check for privileged port binding issues on Linux
        if sys.platform != "win32" and webhook_port < 1024 and os.getuid() != 0:
            logger.critical(
                f"❌ CRITICAL: Attempting to bind to privileged port {webhook_port} without root privileges."
            )
            logger.critical(
                "   On Linux, only root can bind to ports below 1024. Use a port like 8443 or run behind Nginx."
            )
            raise PermissionError(
                f"Permission denied for port {webhook_port} (non-root user)"
            )
        ...
        try:
            ...
            if cert_path and key_path:
                ...
                logger.warning(
                    "⚠️  Both WEBHOOK_CERT_PATH and WEBHOOK_KEY_PATH must be set together. Ignoring SSL config."
                )
