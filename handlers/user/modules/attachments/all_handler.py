...
class AllAttachmentsHandler(BaseUserHandler):
    ...
    async def show_all_attachments(self, update: Update, context: CustomContext):
        ...
        if (
            ...
            and "__" in query.data
            and not query.data.startswith("all_page_")
        ):
            from_search = True
        ...
        if from_search:
            br_atts = await self.db.attachments.get_all_attachments(
                category, weapon_name, mode="br"
            )
            mp_atts = await self.db.attachments.get_all_attachments(
                category, weapon_name, mode="mp"
            )
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t("menu.buttons.back", lang), callback_data="search_weapon"
                    )
                ]
            )
        ...
        all_attachments = await self.db.attachments.get_all_attachments(
            category, weapon_name, mode=mode
        )
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                InlineKeyboardButton(
                                    t("menu.buttons.back", lang),
                                    callback_data=f"mode_{mode}_{weapon_name}",
                                )
                            ]
                        ]
                    ),
        ...
        if page > 1:
            nav_buttons.append(
                InlineKeyboardButton(
                    t("nav.prev", lang), callback_data=f"all_page_{page - 1}"
                )
            )
        nav_buttons.append(
        ...
        if page < total_pages:
            nav_buttons.append(
                InlineKeyboardButton(
                    t("nav.next", lang), callback_data=f"all_page_{page + 1}"
                )
            )
    ...
    async def show_all_attachments_msg(self, update: Update, context: CustomContext):
        ...
        for i, att in enumerate(all_attachments[start_idx:end_idx], start_idx + 1):
            stats = await self.db.analytics.get_attachment_stats(
                att["id"], period="all"
            )
        ...
        for i, att in enumerate(all_attachments[start_idx:end_idx], start_idx + 1):
            ...
            if likes > 0:
                button_text += f" 👍{likes}"
            keyboard.append(
                [InlineKeyboardButton(button_text, callback_data=f"att_{att['code']}")]
            )
    ...
    async def attachment_detail_with_mode(self, update: Update, context: CustomContext):
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang),
                    callback_data=f"all_{category}__{weapon_name}",
                )
            ]
        )
    ...
    async def attachment_detail(self, update: Update, context: CustomContext):
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data=f"all_page_{page}"
                )
            ]
        )
