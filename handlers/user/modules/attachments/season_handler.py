...
class SeasonTopHandler(BaseUserHandler):
    ...
    async def season_top_select_mode_msg(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("mode.br_btn", lang), callback_data="season_top_mode_br"
                ),
                InlineKeyboardButton(
                    t("mode.mp_btn", lang), callback_data="season_top_mode_mp"
                ),
            ],
            [
                InlineKeyboardButton(
        ...
        text = (
            t("menu.buttons.season_top", lang)
    ...
    async def season_top_media_with_mode(self, update: Update, context: CustomContext):
        ...
        for i, raw_item in enumerate(items, 1):
            ...
            if att_id:
                await self.db.analytics.track_attachment_view(
                    query.from_user.id, att_id
                )
            feedback_kb = None
            if att_id:
        ...
        back_markup = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        t("menu.buttons.back", lang), callback_data="season_top"
                    )
                ],
                [
                    InlineKeyboardButton(
        ...
        await query.message.reply_text(
            t("success.generic", lang), reply_markup=back_markup
        )
    ...
    def _season_top_build_page(
        ...
        if page < total_pages:
            nav.append(
                InlineKeyboardButton(
                    t("nav.next", lang), callback_data=f"slist_page_{page + 1}"
                )
            )
        if nav:
            keyboard.append(nav)
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="season_top_list"
                )
            ]
        )
        keyboard.append(
    ...
    async def season_top_list_select_mode_msg(
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("mode.br_btn", lang), callback_data="season_list_mode_br"
                ),
                InlineKeyboardButton(
                    t("mode.mp_btn", lang), callback_data="season_list_mode_mp"
                ),
            ],
            [
                InlineKeyboardButton(
        ...
        now = datetime.now().strftime("%H:%M:%S")
        text = (
            t("menu.buttons.season_list", lang)
            + f" _{t('notification.updated', lang, time=now)}_\n\n"
    ...
    async def season_top_list_page_navigation(
        self, update: Update, context: CustomContext
    ):
    ...
    async def season_top_item_detail(self, update: Update, context: CustomContext):
        ...
        back_markup = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        t("menu.buttons.back", lang), callback_data=f"slist_page_{page}"
                    )
                ]
            ]
        )
