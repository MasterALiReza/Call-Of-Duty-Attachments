...
class CMSUserHandler(BaseUserHandler):
    ...
    async def cms_home(self, update: Update, context: CustomContext) -> None:
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("cms.types.news", lang), callback_data="cms_type_news"
                ),
                InlineKeyboardButton(
                    t("cms.types.tutorial", lang), callback_data="cms_type_tutorial"
                ),
            ],
            [
                InlineKeyboardButton(
                    t("cms.types.event", lang), callback_data="cms_type_event"
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def cms_type_selected(self, update: Update, context: CustomContext) -> None:
        ...
        if (not cms_enabled) or total <= 0:
            ...
            try:
                ...
                await query.message.reply_text(
                    t("cms.disabled", lang), reply_markup=kb, parse_mode="Markdown"
                )
            return
    ...
    def _cms_build_page(
        self, ctype: str, page: int, lang: str
        ...
        rows = self.cms.get_published_content(
            content_type=ctype, limit=ITEMS_PER_PAGE + 1, offset=offset
        )
        ...
        if not page_items:
            keyboard = [
                [
                    InlineKeyboardButton(
                        t("menu.buttons.back", lang), callback_data="cms"
                    )
                ]
            ]
        ...
        for it in page_items:
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        f"{title[:52]}{date_txt}", callback_data=f"cms_view_{cid}"
                    )
                ]
            )
        ...
        if page > 1:
            nav_row.append(
                InlineKeyboardButton(
                    t("nav.prev", lang), callback_data=f"cmslist_page_{page - 1}"
                )
            )
        ...
        if has_next:
            nav_row.append(
                InlineKeyboardButton(
                    t("nav.next", lang), callback_data=f"cmslist_page_{page + 1}"
                )
            )
        if nav_row:
        ...
        keyboard.append(
            [InlineKeyboardButton(t("menu.buttons.back", lang), callback_data="cms")]
        )
        # عنوان + نمایش شماره صفحه/کل
        text = (
            t("cms.list.title", lang, count=total)
            + "\n"
    ...
    async def cms_list_page_navigation(
        ...
        if not cms_enabled:
            ...
            await safe_edit_message_text(
                query, t("cms.disabled", lang), reply_markup=kb
            )
            return
    ...
    async def cms_view(self, update: Update, context: CustomContext) -> None:
        ...
        kb = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        t("menu.buttons.back", lang), callback_data=back_cb
                    )
                ]
            ]
        )
        try:
            await safe_edit_message_text(
                query, text, reply_markup=kb, parse_mode="Markdown"
            )
        except Exception:
