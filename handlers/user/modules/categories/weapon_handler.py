...
from config.config import WEAPON_CATEGORIES_IDS
...
class WeaponHandler(BaseUserHandler):
        ...
        if not weapons:
            reply_markup = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            t("menu.buttons.back", lang), callback_data="categories"
                        )
                    ]
                ]
            )
            await safe_edit_message_text(
                query, t("attachment.none", lang), reply_markup=reply_markup
            )
            return
    ...
    async def show_weapon_menu(self, update: Update, context: CustomContext):
        ...
        if selected_mode:
            ...
            weapon_data = await self.db.attachments.get_weapon_attachments(
                category, weapon_name, mode=selected_mode
            )
            ...
            all_attachments = (
                ...
                else weapon_data.get("all_attachments", [])
            )
            top_attachments = (
                [a for a in all_attachments if a.get("top") or a.get("season_top")]
                if isinstance(weapon_data, list)
                else weapon_data.get("top_attachments", [])
            )
        ...
        br_data = await self.db.attachments.get_weapon_attachments(
            category, weapon_name, mode="br"
        )
        mp_data = await self.db.attachments.get_weapon_attachments(
        ...
        br_count = (
            ...
            else len(br_data.get("all_attachments", []))
        )
        mp_count = (
            len(mp_data)
            if isinstance(mp_data, list)
            else len(mp_data.get("all_attachments", []))
        )
