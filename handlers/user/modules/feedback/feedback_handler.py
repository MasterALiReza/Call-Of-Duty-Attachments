...
class FeedbackHandler(BaseUserHandler):
    ...
    async def handle_vote_dislike(self, update: Update, context: CustomContext):
        ...
        # ثبت رأی
        result = await self.db.analytics.vote_attachment(
            user_id, attachment_id, vote=-1
        )
    ...
    async def handle_feedback_request(self, update: Update, context: CustomContext):
        ...
        context.user_data["feedback_prev_is_text"] = bool(
            prev_text and not prev_caption
        )
        try:
                        ...
                        [
                            [
                                InlineKeyboardButton(
                                    ...
                                    callback_data=f"att_fb_cancel_{attachment_id}",
                                )
                            ]
                        ]
                    ),
    ...
    async def handle_feedback_text(self, update: Update, context: CustomContext):
        ...
        # ثبت بازخورد
        success = await self.db.analytics.submit_attachment_feedback(
        ...
        if success:
            ...
            logger.info(
                f"Feedback received: user={user_id}, att={attachment_id}, len={len(feedback_text)}"
            )
        else:
    ...
    async def handle_feedback_cancel(self, update: Update, context: CustomContext):
        ...
        if query:
            ...
            if prev_markup is None and attachment_id:
                prev_markup = InlineKeyboardMarkup(
                    self.build_feedback_buttons(attachment_id, lang=lang)
                )
            try:
                if prev_is_text:
                    await safe_edit_message_text(
    ...
    async def _update_feedback_buttons(
        self, query, attachment_id: int, vote_result: dict, lang: str
    ):
        ...
        try:
            ...
            try:
                ...
                ) and not bool(query.message)
                # پیام‌هایی که از طریق اینلاین ارسال شده‌اند: message.via_bot پر است
                is_via_inline_message = (
                    bool(getattr(query.message, "via_bot", None))
                    if query.message
                    else False
                )
            except Exception:
        ...
        if is_group:
            ...
            if bot_username:
                rows.append(
                    [
                        InlineKeyboardButton(
                            t("share.send_pm", lang),
                            url=f"https://t.me/{bot_username}?start=att-{attachment_id}-{mode or 'br'}",
                        )
                    ]
                )
            rows.append(
                [
                    InlineKeyboardButton(
                        t("share.send_image_group", lang),
                        switch_inline_query_current_chat=f"att:{attachment_id}-{mode or 'br'}",
                    )
                ]
            )
        else:
            ...
            rows.append(
                [
                    InlineKeyboardButton(
                        t("attachment.copy_code", lang),
                        callback_data=f"att_copy_{attachment_id}",
                    )
                ]
            )
            rows.append(
                [
                    InlineKeyboardButton(
                        t("attachment.feedback", lang),
                        callback_data=f"att_fb_{attachment_id}",
                    )
                ]
            )
    ...
    def build_feedback_buttons(
        ...
        return self.build_attachment_keyboard(
            attachment_id, like_count, dislike_count, lang, is_group=False
        )
