...
class SearchHandler(BaseUserHandler):
    ...
    async def search_start_msg(self, update: Update, context: CustomContext):
        ...
        await update.message.reply_text(
            text,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t('search.cancel', lang), callback_data="main_menu")]]),
            parse_mode='Markdown',
        )
        return SEARCHING
    ...
    async def search_process(self, update: Update, context: CustomContext):
        ...
        try:
            ...
            if user_id:
                await self.db.analytics.track_search(user_id, query_text, total_results, float(elapsed_ms))
        except Exception:
            pass
        ...
        if weapons_results:
            ...
            for item in weapons_results[:3]:
                ...
                weapon_atts = [
                    a for a in attachments_results if a['weapon'] == weapon_name and a['category'] == category_key
                ]
    ...
    async def send_attachment_quick(self, update: Update, context: CustomContext):
        ...
        await self._send_attachment_by_identity(update, context, category, weapon, mode, code)
