...
class GuidesHandler(BaseUserHandler):
    ...
    async def _send_guide(
        self, update: Update, key: str, mode: str = "br", context: CustomContext = None
    ):
        """ارسال محتوای یک بخش راهنما (basic/sens/hud) با پشتیبانی از mode."""
        lang = (
            (await get_user_lang(update, context, self.db) or "fa") if context else "fa"
        )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("mode.br_btn", lang), callback_data="game_settings_br"
                ),
                InlineKeyboardButton(
                    t("mode.mp_btn", lang), callback_data="game_settings_mp"
                ),
            ],
            [
                InlineKeyboardButton(
    ...
    async def game_settings_mode_selected(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                ...
                InlineKeyboardButton(
                    basic_text, callback_data=f"show_guide_basic_{mode}"
                ),
            ],
            [InlineKeyboardButton(sens_text, callback_data=f"show_guide_sens_{mode}")],
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="game_settings_menu"
                )
            ],
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    async def show_guide_inline(self, update: Update, context: CustomContext):
            ...
            [
                [
                    InlineKeyboardButton(
                        ...
                        callback_data=f"game_settings_{mode}",
                    )
                ]
            ]
        )
        ...
        await query.message.reply_text(
            t("success.generic", lang), reply_markup=back_button
        )
