...
class SuggestedHandler(BaseUserHandler):
    ...
    async def suggested_attachments_select_mode(
    ...
    async def suggested_attachments_select_mode_msg(
        self, update: Update, context: CustomContext
    ):
        ...
        text = (
            t("menu.buttons.suggested", lang)
            + f" _{t('notification.updated', lang, time=now)}_\n\n"
            + t("suggested.choose_mode", lang)
        )
        await update.message.reply_text(
            text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard)
    ...
    async def suggested_media_with_mode(self, update: Update, context: CustomContext):
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("list.show", lang), callback_data=f"suggested_list_mode_{mode}"
                )
            ]
        )
        # دکمه بازگشت
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="suggested_attachments"
                )
            ]
        )

        text = (
            t("suggested.weapons_title", lang, mode=mode_name)
            + "\n\n"
    ...
    async def suggested_weapon_attachments(
        self, update: Update, context: CustomContext
    ):
        ...
        weapon_attachments = await self.db.attachments.get_suggested_ranked(
            mode, category=category, weapon=weapon
        )
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                t("menu.buttons.back", lang),
                                callback_data=f"suggested_mode_{mode}",
                            )
                        ]
                    ]
                ),
        ...
        for item in weapon_attachments:
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        button_text, callback_data=f"sugg_send_{mode}_{att_id}"
                    )
                ]
            )
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data=f"suggested_mode_{mode}"
                )
            ]
        )
    ...
    def _suggested_build_page(
        self, items: list, page: int, mode: str, lang: str
        ...
        if total_pages > 1:
            ...
            if page > 1:
                nav.append(
                    InlineKeyboardButton(
                        t("nav.prev", lang), callback_data=f"sugglist_page_{page - 1}"
                    )
                )
            nav.append(
            ...
            if page < total_pages:
                nav.append(
                    InlineKeyboardButton(
                        t("nav.next", lang), callback_data=f"sugglist_page_{page + 1}"
                    )
                )
            keyboard.append(nav)
    ...
    async def suggested_list_page_navigation(
        self, update: Update, context: CustomContext
    ):
