...
class NotificationHandler:
    ...
    async def admin_exit_and_notifications(
    ...
    async def notification_settings(self, update: Update, context: CustomContext):
        ...
        status_txt = (
            t("notification.enabled", lang)
            if enabled
            else t("notification.disabled", lang)
        )
        text = (
            t("notification.settings.title", lang)
            + f" {t('notification.updated', lang, time=now)}\n\n"
        )
        ...
        keyboard = []
        toggle_text = (
            t("notification.toggle_all.disable", lang)
            if enabled
            else t("notification.toggle_all.enable", lang)
        )
        keyboard.append(
            [InlineKeyboardButton(toggle_text, callback_data="user_notif_toggle")]
        )
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("notification.events.button", lang),
                    callback_data="user_notif_events",
                )
            ]
        )
        keyboard.append(
    ...
    async def notification_settings_with_check(
        self, update: Update, context: CustomContext
    ):
    ...
    async def notification_events_menu(self, update: Update, context: CustomContext):
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="user_notif_back"
                )
            ]
        )
    ...
    async def view_attachment_from_notification(
        self, update: Update, context: CustomContext
    ):
        ...
        try:
            ...
            logger.info(
                f"Parsed notification callback - Category: {category}, Weapon: {weapon}, Code: {code}, Mode: {mode}"
            )
