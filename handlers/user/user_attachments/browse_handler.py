    ...
    if "br" in enabled_modes:
        mode_buttons.append(
            InlineKeyboardButton(
                t("mode.br_btn", lang), callback_data="ua_browse_mode_br"
            )
        )
    if "mp" in enabled_modes:
        mode_buttons.append(
            InlineKeyboardButton(
                t("mode.mp_btn", lang), callback_data="ua_browse_mode_mp"
            )
        )
...
async def browse_mode_selected(update: Update, context: CustomContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                t("list.show", lang), callback_data=f"ua_browse_all_{mode}"
            )
        ],
        [
            InlineKeyboardButton(
                t("category.choose", lang), callback_data=f"ua_browse_select_cat_{mode}"
            )
        ],
...
async def browse_show_category_menu(update: Update, context: CustomContext):
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            t("menu.buttons.back", lang), callback_data="ua_browse"
                        )
                    ]
                ]
            ),
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                t("menu.buttons.back", lang), callback_data=f"ua_browse_mode_{mode}"
            )
        ]
    )
...
async def show_attachments_page(update: Update, context: CustomContext):
    ...
    if total_count == 0:
        mode_name = t(f"mode.{mode}_btn", lang)
        cat_name = (
            t(f"category.{category}", "en")
            if category != "all"
            else t("ua.all_categories", lang)
        )
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            t("ua.submit", lang), callback_data="ua_submit"
                        ),
                        InlineKeyboardButton(
                            t("menu.buttons.back", lang),
                            callback_data=f"ua_browse_mode_{mode}",
                        ),
                    ]
                ]
            ),
    ...
    attachments = await db.users.get_approved_user_attachments_paginated(
        mode, category, limit=ATTACHMENTS_PER_PAGE, offset=page * ATTACHMENTS_PER_PAGE
    )
    ...
    cat_display = (
        ...
        if category == "all"
        else WEAPON_CATEGORIES_SHORT.get(category, category)
    )
    ...
    for att in attachments:
        ...
        att_name = (
            att.get("name") or att.get("attachment_name") or t("common.unknown", lang)
        )
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(t("nav.prev", lang), callback_data="ua_browse_prev")
        )
    if page < total_pages - 1:
        nav_buttons.append(
            InlineKeyboardButton(t("nav.next", lang), callback_data="ua_browse_next")
        )
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                t("menu.buttons.back", lang),
                callback_data=f"ua_browse_mode_{context.user_data['browse_mode']}",
            )
        ]
    )
    ...
    if query:
        try:
            ...
            try:
                ...
                log_exception(
                    logger, delete_exc, "ua_browse.show_attachments_page.delete_source"
                )
...
async def browse_prev_page(update: Update, context: CustomContext):
    """صفحه قبل"""
    context.user_data["browse_page"] = max(
        0, context.user_data.get("browse_page", 0) - 1
    )
    await show_attachments_page(update, context)
...
async def view_attachment_detail(update: Update, context: CustomContext):
    ...
    lang = await get_user_lang(update, context, db) or "fa"
    username = (
        attachment.get("username")
        or attachment.get("first_name")
        or t("user.anonymous", lang)
    )
    ...
    att_name = escape_markdown(
        str(attachment.get("attachment_name", attachment.get("name", "Unknown"))),
        version=2,
    )
    ...
    try:
            ...
            async with conn.cursor() as cur:
                try:
                    ...
                    log_exception(
                        logger,
                        fallback_exc,
                        "ua_browse.view_attachment_detail.precheck_legacy_reporter",
                    )
                    await cur.execute(
                        """
                        SELECT 1 FROM user_attachment_reports
        ...
        log_exception(
            logger,
            pre_err,
            "ua_browse.view_attachment_detail.precheck_already_reported",
        )

    row1 = [InlineKeyboardButton("👍", callback_data=f"ua_like_{attachment_id}")]
    if not already_reported:
        row1.append(
            InlineKeyboardButton("⚠️", callback_data=f"ua_report_{attachment_id}")
        )
    keyboard = [
        row1,
        [
            InlineKeyboardButton(
                t("menu.buttons.back", lang), callback_data="ua_browse_back"
            )
        ],
    ]
...
async def report_attachment(update: Update, context: CustomContext):
            ...
            async with conn.cursor() as cur:
                ...
                try:
                    ...
                    log_exception(
                        logger,
                        fallback_exc,
                        "ua_browse.report_attachment.duplicate_precheck_legacy",
                    )
                ...
                try:
                    ...
                    log_exception(
                        logger,
                        fallback_exc,
                        "ua_browse.report_attachment.daily_limit_legacy",
                    )
                ...
                if today_count >= 5:
                    lang = await get_user_lang(update, context, db) or "fa"
                    await query.answer(
                        t("ua.report.limit_reached", lang), show_alert=True
                    )
                    return
    except Exception as pre_err:
    ...
    try:
            ...
            async with conn.cursor() as cursor:
                ...
                try:
                    ...
                    log_exception(
                        logger,
                        fallback_exc,
                        "ua_browse.report_attachment.insert_legacy",
                    )
                    await cursor.execute(
                        """
        ...
        try:
            ...
            log_exception(
                logger, cache_exc, "ua_browse.report_attachment.invalidate_stats_cache"
            )
        used_now = (today_count or 0) + 1
        lang = await get_user_lang(update, context, db) or "fa"
        await query.answer(
            t("ua.report.saved_today", lang, used=used_now), show_alert=True
        )
    except Exception as exc:
...
browse_handlers = [
    ...
    CallbackQueryHandler(
        browse_show_all_attachments, pattern="^ua_browse_all_(br|mp)$"
    ),
    CallbackQueryHandler(
        browse_show_category_menu, pattern="^ua_browse_select_cat_(br|mp)$"
    ),
    CallbackQueryHandler(
        browse_category_selected, pattern="^ua_browse_cat_(?!.*select)"
    ),
