...
async def reorder_channels_menu_impl(
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                t("admin.channels.reorder.title", lang), callback_data="noop"
            )
        ]
    )
    ...
    for idx, channel in enumerate(channels):
        ...
        if idx > 0:
            row.append(
                InlineKeyboardButton(
                    "⬆️", callback_data=f"move_up_{channel['channel_id']}"
                )
            )
        else:
        ...
        row.append(
            InlineKeyboardButton(f"{idx + 1}. {channel['title']}", callback_data="noop")
        )

        if idx < len(channels) - 1:
            row.append(
                InlineKeyboardButton(
                    "⬇️", callback_data=f"move_down_{channel['channel_id']}"
                )
            )
        else:
            row.append(InlineKeyboardButton("  ", callback_data="noop"))
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                t("admin.channels.reorder.confirm", lang), callback_data="channel_menu"
            )
        ]
    )
...
async def handle_move_channel_impl(
    ...
    if len(parts) < 3:
        try:
            ...
            lang = "fa"
        await query.answer(
            t("admin.channels.reorder.invalid_operation", lang), show_alert=True
        )
        return reorder_state
    ...
    if action == "move_up":
        ...
        try:
            ...
            lang = "fa"
        message = (
            t("admin.channels.reorder.moved_up", lang)
            if success
            else t("admin.channels.reorder.move_up_failed", lang)
        )
        ...
        try:
            ...
            lang = "fa"
        message = (
            t("admin.channels.reorder.moved_down", lang)
            if success
            else t("admin.channels.reorder.move_down_failed", lang)
        )
    else:
        try:
            lang = (
    ...
    if success:
        ...
        logger.info(
            "[channel] Cleared membership cache for %s users after reordering",
            cleared_count,
        )
