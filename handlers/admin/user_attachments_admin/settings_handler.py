    ...
    keyboard = [
        [
            InlineKeyboardButton(
                t("admin.ua.settings.buttons.disable_system", lang)
                if system_enabled
                else t("admin.ua.settings.buttons.enable_system", lang),
        ...
        [
            InlineKeyboardButton(
                t("admin.ua.settings.buttons.categories", lang),
                callback_data="ua_settings_categories",
            ),
            InlineKeyboardButton(
                t("admin.ua.settings.buttons.blacklist", lang),
                callback_data="ua_settings_blacklist",
            ),
        ],
        [
            InlineKeyboardButton(
                t("admin.ua.settings.buttons.limits", lang),
                callback_data="ua_settings_limits",
            )
        ],
        [
            InlineKeyboardButton(
...
async def toggle_system(update: Update, context: CustomContext):
    ...
    try:
        ...
        await db.settings.set_user_attachment_setting(
            "system_enabled", new_value, user_id
        )

        status_word = (
            t("common.enabled_word", lang)
            if new_value == "1"
            else t("common.disabled_word", lang)
        )
        await query.answer(
            t("admin.ua.settings.system.toggled", lang, status=status_word),
            show_alert=True,
        )
...
async def toggle_mode(update: Update, context: CustomContext):
    ...
    try:
        ...
        enabled_modes_str = (
            await db.settings.get_ua_setting("enabled_modes") or '["mp","br"]'
        )
        enabled_modes = json.loads(enabled_modes_str)
        ...
        success = await db.settings.set_user_attachment_setting(
            "enabled_modes", new_value, user_id
        )
        ...
        if success:
            ...
            await query.answer(
                t(
                    "admin.ua.settings.mode.toggled",
                    lang,
                    mode=mode_name,
                    status=status_word,
                ),
...
async def show_blacklist(update: Update, context: CustomContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                t("admin.ua.blacklist.buttons.add", lang),
                callback_data="ua_settings_blacklist_add",
            )
        ],
        [
            InlineKeyboardButton(
                t("admin.ua.blacklist.buttons.remove", lang),
                callback_data="ua_settings_blacklist_remove",
            )
        ],
        [
            InlineKeyboardButton(
                t("menu.buttons.back", lang), callback_data="ua_admin_settings"
            )
        ],
    ]
...
async def show_limits_settings(update: Update, context: CustomContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.daily", lang, n=daily_limit),
                callback_data="ua_settings_limit_daily",
            ),
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.image", lang, mb=f"{max_image_size_mb:.1f}"),
                callback_data="ua_settings_limit_image",
            ),
        ],
        [
            InlineKeyboardButton(
                t(
                    "admin.ua.limits.buttons.rate",
                    lang,
                    requests=rate_limit_requests,
                    minutes=int(rate_limit_minutes),
                    min_label=t("common.min", lang),
                ),
                callback_data="ua_settings_limit_rate",
            )
        ],
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.name", lang, n=max_name_length),
                callback_data="ua_settings_limit_name",
            ),
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.desc", lang, n=max_description_length),
                callback_data="ua_settings_limit_desc",
            ),
        ],
        [
            InlineKeyboardButton(
...
async def show_categories_settings(update: Update, context: CustomContext):
    ...
    try:
        ...
        enabled_categories_str = (
            await db.settings.get_ua_setting("enabled_categories") or "[]"
        )
        enabled_categories = json.loads(enabled_categories_str)
    except (TypeError, ValueError, json.JSONDecodeError) as exc:
        log_exception(
            logger, exc, "ua_settings.show_categories_settings.parse_enabled_categories"
        )
    ...
    message += (
        t(
            "admin.ua.categories.summary",
            lang,
            total=len(all_categories),
            enabled=len(enabled_categories),
        )
        + "\n\n"
    )
    ...
    for cat_name in all_categories:
        ...
        display_name = (
            f"{emoji_map.get(cat_name, '')} {t('category.' + cat_name, 'en')}"
        )
...
async def toggle_category(update: Update, context: CustomContext):
    ...
    try:
        ...
        success = await db.settings.set_user_attachment_setting(
            "enabled_categories", new_value, user_id
        )
        ...
        if success:
            ...
            await query.answer(
                t(
                    "admin.ua.categories.toggled",
                    lang,
                    category=t("category." + category_name, "en"),
                    status=status_word,
                ),
                show_alert=True,
            )
        ...
        await _handle_boundary_error(
            update, context, exc, "ua_settings.toggle_category"
        )
...
async def start_add_blacklist_word(update: Update, context: CustomContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                t("menu.buttons.cancel", lang), callback_data="ua_settings_blacklist"
            )
        ]
    ]
...
async def receive_new_blacklist_word(update: Update, context: CustomContext):
    ...
    failed_words = []
    severity_names = {
        1: t("admin.ua.blacklist.severity.low", lang),
        2: t("admin.ua.blacklist.severity.medium", lang),
        3: t("admin.ua.blacklist.severity.high", lang),
    }
    ...
    for line in lines:
        ...
        try:
            ...
            success = await db.settings.add_blacklisted_word(
                word, "profanity", severity, user_id
            )
    ...
    if success_count == 0 and failed_count == 0:
        ...
        if success_count > 0:
            message += (
                t("admin.ua.blacklist.result.success", lang, count=success_count) + "\n"
            )
        ...
        if failed_count > 0:
            message += (
                t("admin.ua.blacklist.result.failed", lang, count=failed_count) + "\n"
            )
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                t("admin.ua.blacklist.buttons.back", lang),
                callback_data="ua_settings_blacklist",
            )
        ],
    ]
...
async def change_limit_daily(update: Update, context: CustomContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.daily", lang, n=3),
                callback_data="ua_limit_daily_set_3",
            ),
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.daily", lang, n=5),
                callback_data="ua_limit_daily_set_5",
            ),
        ],
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.daily", lang, n=10),
                callback_data="ua_limit_daily_set_10",
            ),
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.daily", lang, n=15),
                callback_data="ua_limit_daily_set_15",
            ),
        ],
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.daily", lang, n=t("common.unlimited", lang)),
                callback_data="ua_limit_daily_set_999",
            )
        ],
        [
            InlineKeyboardButton(
                t("menu.buttons.back", lang), callback_data="ua_settings_limits"
            )
        ],
    ]
    await query.edit_message_text(
...
async def change_limit_description(update: Update, context: CustomContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.desc", lang, n=50),
                callback_data="ua_limit_desc_set_50",
            ),
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.desc", lang, n=100),
                callback_data="ua_limit_desc_set_100",
            ),
        ],
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.desc", lang, n=150),
                callback_data="ua_limit_desc_set_150",
            ),
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.desc", lang, n=200),
                callback_data="ua_limit_desc_set_200",
            ),
        ],
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.desc", lang, n=300),
                callback_data="ua_limit_desc_set_300",
            ),
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.desc", lang, n=500),
                callback_data="ua_limit_desc_set_500",
            ),
        ],
        [
            InlineKeyboardButton(
...
async def change_limit_name(update: Update, context: CustomContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.name", lang, n=50),
                callback_data="ua_limit_name_set_50",
            ),
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.name", lang, n=75),
                callback_data="ua_limit_name_set_75",
            ),
        ],
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.name", lang, n=100),
                callback_data="ua_limit_name_set_100",
            ),
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.name", lang, n=150),
                callback_data="ua_limit_name_set_150",
            ),
        ],
        [
            InlineKeyboardButton(
...
async def change_limit_image(update: Update, context: CustomContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.image", lang, mb=2),
                callback_data="ua_limit_image_set_2097152",
            ),
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.image", lang, mb=5),
                callback_data="ua_limit_image_set_5242880",
            ),
        ],
        [
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.image", lang, mb=10),
                callback_data="ua_limit_image_set_10485760",
            ),
            InlineKeyboardButton(
                t("admin.ua.limits.buttons.image", lang, mb=15),
                callback_data="ua_limit_image_set_15728640",
            ),
        ],
        [
            InlineKeyboardButton(
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                ...
                callback_data="ua_limit_rate_set_3_300",
            ),
            InlineKeyboardButton(
                t(
                    "admin.ua.limits.buttons.rate",
                ...
                callback_data="ua_limit_rate_set_5_600",
            ),
        ],
        [
            InlineKeyboardButton(
                ...
                callback_data="ua_limit_rate_set_10_900",
            ),
            InlineKeyboardButton(
                t(
                    "admin.ua.limits.buttons.rate",
                ...
                callback_data="ua_limit_rate_set_15_1800",
            ),
        ],
        [
            InlineKeyboardButton(
...
async def set_limit_value(update: Update, context: CustomContext):
    ...
    try:
        ...
        if limit_type == "rate":
            ...
            # ذخیره هر دو مقدار
            success1 = await db.settings.set_user_attachment_setting(
                "rate_limit_requests", requests, user_id
            )
            success2 = await db.settings.set_user_attachment_setting(
                "rate_limit_window", window, user_id
            )
            ...
            if success:
                if limit_type == "image":
                    ...
                    await query.answer(
                        "✅ "
                        + t(
                            "admin.ua.limits.buttons.image", lang, mb=f"{mb_value:.1f}"
                        ),
                        show_alert=True,
                    )
                elif limit_type == "daily":
                    await query.answer(
                        "✅ " + t("admin.ua.limits.buttons.daily", lang, n=new_value),
                        show_alert=True,
                    )
                elif limit_type == "name":
                    await query.answer(
                        "✅ " + t("admin.ua.limits.buttons.name", lang, n=new_value),
                        show_alert=True,
                    )
                elif limit_type == "desc":
                    await query.answer(
                        "✅ " + t("admin.ua.limits.buttons.desc", lang, n=new_value),
                        show_alert=True,
                    )
                await show_limits_settings(update, context)
            else:
        ...
        await _handle_boundary_error(
            update, context, exc, "ua_settings.set_limit_value"
        )
...
async def start_remove_blacklist_word(update: Update, context: CustomContext):
    ...
    try:
        ...
        if not blacklist:
            await query.answer(
                t("admin.ua.blacklist.empty_title", lang), show_alert=True
            )
            await show_blacklist(update, context)
            return ConversationHandler.END
    except Exception as exc:
        await _handle_boundary_error(
            update, context, exc, "ua_settings.start_remove_blacklist_word"
        )
        return ConversationHandler.END
...
async def receive_word_to_remove(update: Update, context: CustomContext):
    ...
    try:
        ...
        blacklist = await db.settings.get_all_blacklisted_words()
        word_entry = next(
            (w for w in blacklist if w["word"].lower() == word.lower()), None
        )
...
add_blacklist_conv = ConversationHandler(
    entry_points=[
        CallbackQueryHandler(
            start_add_blacklist_word, pattern="^ua_settings_blacklist_add$"
        )
    ],
    states={
        ADD_BLACKLIST_WORD: [
    ...
    fallbacks=[
        CallbackQueryHandler(
            cancel_blacklist_operation, pattern="^ua_settings_blacklist$"
        ),
...
remove_blacklist_conv = ConversationHandler(
    entry_points=[
        CallbackQueryHandler(
            start_remove_blacklist_word, pattern="^ua_settings_blacklist_remove$"
        )
    ],
    states={
...
settings_handlers = [
    ...
    CallbackQueryHandler(
        set_limit_value, pattern="^ua_limit_(daily|desc|name|image|rate)_set_"
    ),
