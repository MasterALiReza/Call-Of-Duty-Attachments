...
class AttachmentsDashboardHandler(BaseAdminHandler):
    ...
    async def _safe_edit_message(
        self, query, message: str, keyboard, parse_mode=ParseMode.MARKDOWN
    ):
        """
    ...
    async def analytics_menu(self, update: Update, context: CustomContext) -> int:
        ...
        message += (
            t(
                "admin.analytics.menu.overview.views",
                lang,
                n=stats.get("total_views", 0),
            )
            + "\n"
        )
        message += (
            t(
                "admin.analytics.menu.overview.clicks",
                lang,
                n=stats.get("total_clicks", 0),
            )
            + "\n"
        )
        message += (
            t(
                "admin.analytics.menu.overview.shares",
                lang,
                n=stats.get("total_shares", 0),
            )
            + "\n"
        )
        message += (
            t(
                "admin.analytics.menu.overview.users",
                lang,
                n=stats.get("unique_users", 0),
            )
            + "\n"
        )
        message += (
            t(
                "admin.analytics.menu.overview.engagement",
                lang,
                rate=f"{stats.get('engagement_rate', 0):.1f}",
            )
            + "\n\n"
        )
        ...
        if stats.get("top_performer"):
            ...
            message += (
                t(
                    "admin.analytics.menu.top.most_viewed",
                    lang,
                    name=safe_name,
                    views=stats["top_performer"]["views"],
                )
                + "\n"
            )
        if stats.get("most_engaging"):
            ...
            message += (
                t(
                    "admin.analytics.menu.top.most_engaging",
                    lang,
                    name=safe_name,
                    rate=f"{stats['most_engaging']['rate']:.1f}",
                )
                + "\n"
            )
        if stats.get("highest_rated"):
            ...
            message += (
                t(
                    "admin.analytics.menu.top.highest_rated",
                    lang,
                    name=safe_name,
                    rating=f"{stats['highest_rated']['rating']:.1f}",
                )
                + "\n"
            )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.analytics.buttons.trending", lang),
                    callback_data="analytics_view_trending",
                ),
                InlineKeyboardButton(
                    t("admin.analytics.buttons.underperforming", lang),
                    callback_data="analytics_view_underperforming",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.analytics.buttons.weapon_stats", lang),
                    callback_data="analytics_view_weapon_stats",
                ),
                InlineKeyboardButton(
                    t("admin.analytics.buttons.search_attachment", lang),
                    callback_data="analytics_search_attachment",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.analytics.buttons.daily_report", lang),
                    callback_data="analytics_daily_report",
                ),
                InlineKeyboardButton(
                    t("admin.analytics.buttons.download_report", lang),
                    callback_data="analytics_download_report",
                ),
            ],
            [
                InlineKeyboardButton(
    ...
    async def analytics_cohort_analysis(
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="analytics_menu"
                )
            ]
        ]
    ...
    async def view_trending(self, update: Update, context: CustomContext) -> int:
        ...
        try:
            ...
            results = await get_container().analytics.get_trending_growth_stats(
                limit=10
            )
            ...
            if results:
                ...
                for i, result in enumerate(results, 1):
                    ...
                    message += (
                        t("admin.analytics.lines.weapon", lang, weapon=safe_weapon)
                        + "\n"
                    )
                    message += (
                        t(
                            "admin.analytics.lines.growth",
                            lang,
                            icon=icon,
                            value=f"{growth:+.0f}",
                        )
                    ...
                    message += (
                        t("admin.analytics.lines.views", lang, value=f"{views:,}")
                        + "\n\n"
                    )
            else:
            ...
            message = (
                t("admin.analytics.trending.error.title", lang)
                + "\n\n"
            ...
            for i, r in enumerate(results, 1):
                try:
                    ...
                    keyboard.append(
                        [
                            InlineKeyboardButton(
                                title, callback_data=f"weapon_details_{att_id}"
                            )
                        ]
                    )
                except Exception:
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.refresh", lang), callback_data="refresh_trending"
                )
            ]
        )
        keyboard.append(
    ...
    async def handle_search_text(self, update: Update, context: CustomContext) -> int:
        ...
        try:
            ...
            await update.message.reply_text(
                t("admin.analytics.search.searching", lang, query=q)
            )

            rows = await get_container().analytics.get_attachment_search_results(
                q, limit=10
            )
            ...
            for r in rows:
                ...
                lines.append(
                    t(
                        "admin.analytics.search.lines.meta",
                        lang,
                        weapon=wpn,
                        category=cat,
                    )
                ...
                lines.append(
                    t(
                        "admin.analytics.search.lines.stats",
                        lang,
                        views=views,
                        clicks=clicks,
                    )
    ...
    async def download_report(self, update: Update, context: CustomContext) -> int:
        ...
        await query.answer()
        try:
            rows = await get_container().analytics.get_csv_report_data(
                days=7, limit=200
            )
            ...
            writer.writerow(
                ["Attachment", "Weapon", "Category", "Views", "Clicks", "Users"]
            )
            for r in rows:
                writer.writerow(
                    [
    ...
    async def weapon_details(self, update: Update, context: CustomContext) -> int:
        ...
        if not att_id:
            ...
            await self._safe_edit_message(
                query, t("admin.analytics.weapon_stats.no_data.title", lang), keyboard
            )
            return ADMIN_MENU

        try:
            stats = await get_container().analytics.get_attachment_detailed_stats(
                att_id
            )
            if not stats:
            ...
            mode_title = (
                t("admin.analytics.weapon_stats.buttons.br", lang)
                if mode_val == "br"
                else t("admin.analytics.weapon_stats.buttons.mp", lang)
                if mode_val == "mp"
                else t("admin.analytics.weapon_stats.buttons.all", lang)
            )
            ...
            message += (
                t("admin.analytics.weapon_stats.mode", lang, mode=mode_title) + "\n\n"
            )

            message += t("admin.analytics.weekly.summary.header", lang) + "\n"
            message += (
                t("admin.analytics.weekly.summary.views", lang, n=f"{views:,}") + "\n"
            )
            message += (
                t("admin.analytics.weekly.summary.clicks", lang, n=f"{clicks:,}") + "\n"
            )
            message += (
                t("admin.analytics.weekly.summary.users", lang, n=f"{users:,}") + "\n"
            )
            message += (
                t("admin.analytics.weekly.summary.engagement", lang, rate=f"{rate:.1f}")
                + "\n\n"
            )
            ...
            days = [today - timedelta(days=i) for i in range(6, -1, -1)]
            data_map = {
                str(r["date"]): (int(r["views"] or 0), int(r["clicks"] or 0))
                for r in daily_rows
            }
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.analytics.buttons.daily_chart", lang),
                    callback_data=f"att_daily_chart_{att_id}",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.analytics.buttons.download_csv", lang),
                    callback_data=f"att_download_csv_{att_id}",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def att_daily_chart(self, update: Update, context: CustomContext) -> int:
        ...
        try:
            ...
            att_name = self._escape_markdown(
                stats["attachment"].get("name") or "Unknown"
            )
            rows = stats["daily_7d"]
            ...
            width = 28
            lines = [
                t("admin.analytics.daily.title", lang) + f" — *{att_name}*",
                "",
                "```",
            ]
            for d in days:
    ...
    async def att_download_csv(self, update: Update, context: CustomContext) -> int:
        ...
        try:
            ...
            output.close()
            filename = (
                f"attachment_{att_id}_daily_{datetime.now().strftime('%Y%m%d')}.csv"
            )
            await query.message.reply_document(
    ...
    async def view_user_behavior(self, update: Update, context: CustomContext) -> int:
        ...
        try:
            ...
            avg_views = (
                (float(views) / float(active_users)) if active_users > 0 else 0.0
            )
            ...
            message += (
                t(
                    "admin.analytics.user.summary.total_users",
                    lang,
                    n=f"{total_users:,}",
                )
                + "\n"
            ...
            message += (
                t("admin.analytics.user.summary.active_7d", lang, n=f"{active_users:,}")
                + "\n"
            )
            message += (
                t("admin.analytics.user.summary.avg_views", lang, n=f"{avg_views:.1f}")
                + "\n"
            )
            message += (
                t(
                    "admin.analytics.user.summary.engagement",
                    lang,
            ...
            very_active = [
                u for u in per_user if (int(u["views"]) >= 50 or int(u["clicks"]) >= 10)
            ]
            active = [
                u
                for u in per_user
                if (10 <= int(u["views"]) < 50 or 3 <= int(u["clicks"]) < 10)
            ]
            moderate_count = len(
                [u for u in per_user if (int(u["views"]) < 10 and int(u["clicks"]) < 3)]
            )

            if per_user:
                if very_active:
                    message += (
                        t("admin.analytics.user.group.very_active.header", lang) + "\n"
                    )
                    for item in very_active[:3]:
                        name = self._escape_markdown(f"#{item['user_id']}")
                        eng = (
                            (float(item["clicks"]) / float(item["views"]) * 100)
                            if int(item["views"]) > 0
                            else 0.0
                        )
                        message += f"\n🥇 *{name}*\n"
                        message += (
                            t(
                                "admin.analytics.user.line.views",
                                lang,
                                n=f"{item['views']:,}",
                            )
                            + "\n"
                        ...
                        message += (
                            t(
                                "admin.analytics.user.line.clicks",
                                lang,
                                n=f"{item['clicks']:,}",
                            )
                            + "\n"
                        ...
                        message += (
                            t(
                                "admin.analytics.user.line.engagement",
                                lang,
                                rate=f"{eng:.0f}",
                            )
                            + "\n"
                ...
                if active:
                    message += (
                        t("admin.analytics.user.group.active.header", lang) + "\n"
                    )
                    for item in active[:2]:
                        name = self._escape_markdown(f"#{item['user_id']}")
                        message += (
                            t(
                                "admin.analytics.user.line.item",
                                lang,
                                name=name,
                                views=f"{item['views']:,}",
                                atts="-",
                ...
                if moderate_count > 0:
                    message += (
                        t(
                            "admin.analytics.user.group.moderate.count",
                            lang,
                            n=moderate_count,
                        )
                        + "\n"
                ...
                message += (
                    t("admin.analytics.user.no_data.title", lang)
                    + "\n\n"
            ...
            message = (
                t("admin.analytics.user.error.title", lang)
                + "\n\n"
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.analytics.buttons.more_details", lang),
                    callback_data="user_behavior_details",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def user_behavior_details(
        self, update: Update, context: CustomContext
    ) -> int:
        ...
        try:
            ...
            if total > 0:
                ...
                message += (
                    t(
                        "admin.analytics.user_details.dist.line",
                        lang,
                        icon="🔥",
                        cat=t(
                            "admin.analytics.user_details.dist.cat.very_active", lang
                        ),
                        count=very,
                        pct=pct(very),
                    )
                ...
                message += (
                    t(
                        ...
                        lang,
                        icon="⚡",
                        cat=t("admin.analytics.user_details.dist.cat.active", lang),
                        count=act,
                        pct=pct(act),
                    )
                    + "\n"
                ...
                message += (
                    t(
                        ...
                        lang,
                        icon="📊",
                        cat=t("admin.analytics.user_details.dist.cat.moderate", lang),
                        count=mod,
                        pct=pct(mod),
                    )
                    + "\n\n"
            ...
            message += (
                t("admin.analytics.user.summary.active_7d", lang, n=f"{weekly:,}")
                + "\n"
            )
    ...
    async def ws_back_to_categories(
        self, update: Update, context: CustomContext
    ) -> int:
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("admin.analytics.buttons.back_to_mode_selection", lang),
                    callback_data="ws_back_to_mode",
                )
            ]
        )
        keyboard.append(
    ...
    async def view_weapon_stats(self, update: Update, context: CustomContext) -> int:
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.analytics.weapon_stats.buttons.br", lang),
                    callback_data="analytics_ws_mode_br",
                ),
                InlineKeyboardButton(
                    t("admin.analytics.weapon_stats.buttons.mp", lang),
                    callback_data="analytics_ws_mode_mp",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.analytics.weapon_stats.buttons.all", lang),
                    callback_data="analytics_ws_mode_all",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def ws_choose_mode(self, update: Update, context: CustomContext) -> int:
        ...
        mode_map = {
            ...
            "analytics_ws_mode_all": t(
                "admin.analytics.weapon_stats.buttons.all", lang
            ),
        }
    ...
    async def ws_choose_category(self, update: Update, context: CustomContext) -> int:
        ...
        try:
            agg = await get_container().analytics.get_weapon_category_stats(
                cat_id, mode_value
            )
        except Exception as e:
        ...
        if agg and agg.get("category_name"):
            ...
            message += (
                t(
                    "admin.analytics.weapon_stats.lines.attach_count",
                    lang,
                    n=agg["attachment_count"],
                )
                + "\n"
            )
            message += (
                t(
                    "admin.analytics.weapon_stats.lines.views",
                    lang,
                    views=f"{int(agg['total_views']):,}",
                )
                + "\n"
            )
            message += (
                t(
                    "admin.analytics.weapon_stats.lines.avg_max",
                    lang,
                    avg=f"{float(agg['average_views']):.1f}",
                    max=f"{int(agg['max_views']):,}",
                )
                + "\n"
            )
        else:
            message = (
                t("admin.analytics.weapon_stats.no_data.title", lang)
                + "\n"
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.analytics.buttons.change_category", lang),
                    callback_data="analytics_ws_back_to_categories",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.analytics.buttons.back_to_mode_selection", lang),
                    callback_data="analytics_ws_back_to_mode",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def view_underperforming(self, update: Update, context: CustomContext) -> int:
        ...
        try:
            ...
            message = (
                t("admin.analytics.under.title", lang)
                + "\n\n"
    ...
    async def daily_report(self, update: Update, context: CustomContext) -> int:
        ...
        try:
            ...
            message += (
                t("admin.analytics.daily.stats.engagement", lang, rate=f"{rate:.1f}")
                + "\n\n"
            )
            ...
            if top:
                ...
                for i, row in enumerate(top, 1):
                    ...
                    message += (
                        t(
                            "admin.analytics.fallback.lines.views",
                            lang,
                            value=f"{row['views']:,}",
                        )
                        + "\n\n"
                ...
                message += (
                    t("admin.analytics.daily.no_data.title", lang)
                    + "\n\n"
            ...
            message = (
                t("admin.analytics.daily.error.title", lang)
                + "\n\n"
                + t("admin.analytics.daily.error.body", lang)
            )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.analytics.buttons.daily_chart", lang),
                    callback_data="daily_chart",
                ),
                InlineKeyboardButton(
                    t("admin.analytics.buttons.download_csv", lang),
                    callback_data="download_daily_csv",
                ),
            ],
            [
                InlineKeyboardButton(
        ...
        try:
            ...
            if top:
                ...
                for i, row in enumerate(top, 1):
                    ...
                    message += (
                        t(
                            "admin.analytics.lines.weapon_simple",
                            lang,
                            weapon=safe_weapon,
                        )
                        + "\n"
                ...
                message += (
                    t("admin.analytics.weekly.no_data.title", lang)
                    + "\n\n"
            ...
            message = (
                t("admin.analytics.weekly.error.title", lang)
                + "\n\n"
    ...
    async def analytics_funnel_analysis(
        self, update: Update, context: CustomContext
    ) -> int:
    ...
    def get_conversation_handler(self) -> ConversationHandler:
        ...
        return ConversationHandler(
            entry_points=[
                ...
                CallbackQueryHandler(
                    self.analytics_menu, pattern="^attachment_analytics$"
                ),
            ],
            states={
                ANALYTICS_MENU: [
                    ...
                    CallbackQueryHandler(
                        self.view_underperforming, pattern="^view_underperforming$"
                    ),
                    CallbackQueryHandler(
                        self.view_weapon_stats, pattern="^view_weapon_stats$"
                    ),
                    CallbackQueryHandler(
                        self.view_user_behavior, pattern="^view_user_behavior$"
                    ),
                    ...
                    CallbackQueryHandler(
                        self.view_trending, pattern="^analytics_view_trending$"
                    ),
                    CallbackQueryHandler(
                        self.view_underperforming,
                        pattern="^analytics_view_underperforming$",
                    ),
                    CallbackQueryHandler(
                        self.view_weapon_stats, pattern="^analytics_view_weapon_stats$"
                    ),
                    CallbackQueryHandler(
                        self.view_user_behavior,
                        pattern="^analytics_view_user_behavior$",
                    ),
                    CallbackQueryHandler(
                        self.daily_report, pattern="^analytics_daily_report$"
                    ),
                    CallbackQueryHandler(
                        self.weekly_report, pattern="^analytics_weekly_report$"
                    ),
                    # Common
                    CallbackQueryHandler(self.daily_chart, pattern="^daily_chart$"),
                    CallbackQueryHandler(
                        self.download_daily_csv, pattern="^download_daily_csv$"
                    ),
                    CallbackQueryHandler(
                        self.att_daily_chart, pattern="^att_daily_chart_\\d+$"
                    ),
                    CallbackQueryHandler(
                        self.att_download_csv, pattern="^att_download_csv_\\d+$"
                    ),
                    CallbackQueryHandler(
                        self.search_attachment_stats,
                        pattern="^analytics_search_attachment$",
                    ),
                    CallbackQueryHandler(
                        self.download_report, pattern="^analytics_download_report$"
                    ),
                    CallbackQueryHandler(
                        self.weapon_details, pattern="^weapon_details_\\d+$"
                    ),
                    CallbackQueryHandler(
                        self.ws_choose_mode, pattern="^analytics_ws_mode_(br|mp|all)$"
                    ),
                    CallbackQueryHandler(
                        self.view_weapon_stats, pattern="^analytics_ws_back_to_mode$"
                    ),
                    CallbackQueryHandler(
                        self.ws_choose_category, pattern="^analytics_ws_cat_\\d+$"
                    ),
                    CallbackQueryHandler(
                        self.ws_choose_mode, pattern="^analytics_ws_back_to_categories$"
                    ),
                    CallbackQueryHandler(
                        self.admin_cancel, pattern="^admin_menu_return$"
                    ),
                ],
                VIEW_TRENDING: [
                    CallbackQueryHandler(
                        self.refresh_trending, pattern="^refresh_trending$"
                    ),
                    CallbackQueryHandler(
                        self.analytics_menu, pattern="^analytics_menu$"
                    ),
                    CallbackQueryHandler(
                        self.weapon_details, pattern="^weapon_details_\\d+$"
                    ),
                ...
                SEARCH_ATTACH: [
                    MessageHandler(
                        filters.TEXT & (~filters.COMMAND), self.handle_search_text
                    ),
                    CallbackQueryHandler(
                        self.analytics_menu, pattern="^analytics_menu$"
                    ),
