...
class AddAttachmentHandler(BaseAdminHandler):
    ...
    async def add_attachment_start(self, update: Update, context: CustomContext):
        ...
        try:
            await safe_edit_message_text(
                query, t("admin.attach.mode.prompt", lang), reply_markup=reply_markup
            )
    ...
    async def add_attachment_mode_selected(
    ...
    async def add_attachment_category_selected(
        self, update: Update, context: CustomContext
    ):
        ...
        logger.info(
            f"🔍 Category selected: callback_data='{query.data}', extracted_category='{category}'"
        )
        ...
        if not weapons:
            await safe_edit_message_text(
                ...
                t(
                    ...
                    lang,
                    mode=mode_name,
                    category=t(f"category.{category}", "en"),
                )
                + "\n\n"
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.attach.buttons.add_weapon", lang), callback_data="awpn_new"
                )
            ]
        ]
    ...
    async def add_attachment_weapon_selected(
        self, update: Update, context: CustomContext
    ):
        ...
        if query.data == "nav_back":
            ...
            keyboard = await build_category_keyboard(
                callback_prefix="acat_", active_ids=list(WEAPON_CATEGORIES.keys())
            )
            self._add_back_cancel_buttons(keyboard, show_back=True)
        ...
        try:
            await query.message.reply_text(
                t("admin.attach.code.ask", lang), reply_markup=ReplyKeyboardRemove()
            )
        except Exception:
            pass
    ...
    async def add_attachment_new_weapon_name_received(
        self, update: Update, context: CustomContext
    ):
        """
        ...
        result = TextValidator.validate_length(
            weapon_name, max_length=50, min_length=2, field_name="weapon_name"
        )
    ...
    async def add_attachment_code_received(
        self, update: Update, context: CustomContext
    ):
        """
        دریافت کد اتچمنت
    ...
    async def add_attachment_name_received(
        self, update: Update, context: CustomContext
    ):
        """دریافت نام اتچمنت"""
        ...
        skip_markup = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        t("admin.attach.buttons.skip_image", lang),
        ...
        await update.message.reply_text(
            t("admin.attach.image.prompt", lang), reply_markup=skip_markup
        )

        return ADD_ATTACHMENT_IMAGE

    @log_admin_action("add_attachment_image_received")
    async def add_attachment_image_received(
        self, update: Update, context: CustomContext
    ):
        """دریافت عکس اتچمنت"""
        lang = await get_user_lang(update, context, self.db) or "fa"
        ...
        if update.callback_query:
            ...
            if query.data == "skip_image":
                ...
                try:
                    await safe_edit_message_text(
                        query,
                        t("admin.attach.top.question", lang),
                        reply_markup=InlineKeyboardMarkup(keyboard),
                    )
                except BadRequest as e:
                    if "Message is not modified" in str(e):
        ...
        await update.message.reply_text(
            t("admin.attach.top.question", lang), reply_markup=reply_markup
        )
        return ADD_ATTACHMENT_TOP
    ...
    async def add_attachment_top_selected(self, update: Update, context: CustomContext):
        ...
        await safe_edit_message_text(
            query,
            t("admin.attach.season.question", lang),
            reply_markup=InlineKeyboardMarkup(keyboard),
        )
        return ADD_ATTACHMENT_SEASON

    @log_admin_action("add_attachment_season_selected")
    async def add_attachment_season_selected(
        self, update: Update, context: CustomContext
    ):
        ...
        if await self.db.attachments.add_attachment(
            category, weapon, code, name, image, is_top, is_season_top, mode=mode
        ):
            # ✅ DB Audit Logging
            ...
            status_lines = []
            status_lines.append(
                t("admin.attach.status.top", lang)
                if is_top
                else t("admin.attach.status.normal", lang)
            )
            if is_season_top:
    ...
    async def add_attachment_top_ignore_text(
        self, update: Update, context: CustomContext
    ):
    ...
    async def add_attachment_season_ignore_text(
        self, update: Update, context: CustomContext
    ):
        ...
        if state == ADD_ATTACHMENT_MODE:
            ...
            if category:
                ...
                keyboard = [
                    [
                        InlineKeyboardButton(
                            t("admin.attach.buttons.add_weapon", lang),
                            callback_data="awpn_new",
                        )
                    ]
                ]
                keyboard.extend(self._make_weapon_keyboard(weapons, "awpn_", category))
