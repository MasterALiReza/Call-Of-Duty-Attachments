...
logger = get_logger("delete_attachment", "admin.log")
...
class DeleteAttachmentHandler(BaseAdminHandler):
    ...
    async def delete_attachment_start(self, update: Update, context: CustomContext):
        ...
        try:
            await safe_edit_message_text(
                query, t("admin.delete.mode.prompt", lang), reply_markup=reply_markup
            )
    ...
    async def delete_attachment_mode_selected(
    ...
    async def delete_attachment_category_selected(
        self, update: Update, context: CustomContext
    ):
    ...
    async def delete_attachment_weapon_selected(
        self, update: Update, context: CustomContext
    ):
        ...
        if query.data == "nav_back":
            ...
            for k, v in WEAPON_CATEGORIES.items():
                if await is_category_enabled(k, mode, self.db):
                    active_categories[k] = v
            keyboard = await build_category_keyboard(
                callback_prefix="dcat_", active_ids=list(active_categories.keys())
            )
            self._add_back_cancel_buttons(keyboard, show_back=True)
        ...
        if not attachments:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        t("admin.delete.buttons.back_to_weapons", lang),
                        callback_data=f"dcat_{category}",
                    )
                ],
                [
                    InlineKeyboardButton(
        ...
        for att in attachments:
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        f"🗑️ {att['name']}", callback_data=f"delatt_{att['id']}"
                    )
                ]
            )
        self._add_back_cancel_buttons(keyboard, show_back=True)
    ...
    async def delete_attachment_code_selected(
        self, update: Update, context: CustomContext
    ):
        ...
        if await self.db.attachments.delete_attachment(
            category=category, weapon_name=weapon, code=code, mode=mode
        ):
            # Show success message as popup alert
            await query.answer(
                t("admin.delete.success_alert", lang, name=name), show_alert=True
            )
    ...
    async def _rebuild_state_screen(
        ...
        if state == DELETE_ATTACHMENT_MODE:
            ...
            try:
                await safe_edit_message_text(
                    query,
                    t("admin.delete.mode.prompt", lang),
                    reply_markup=InlineKeyboardMarkup(keyboard),
                )
            except BadRequest as e:
                if "Message is not modified" in str(e):
