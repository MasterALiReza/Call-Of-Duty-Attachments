...
class TopAttachmentsHandler(BaseAdminHandler):
    ...
    async def set_top_start(self, update: Update, context: CustomContext):
        ...
        try:
            await safe_edit_message_text(
                query, t("admin.top.choose_mode", lang), reply_markup=reply_markup
            )
    ...
    async def set_top_weapon_selected(self, update: Update, context: CustomContext):
        ...
        text += (
            t("admin.top.selected_count", lang, n=len(selected_tops), max=5) + "\n\n"
        )
        ...
        for att in attachments:
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        f"{prefix}{att['name']}", callback_data=f"tatt_{att['id']}"
                    )
                ]
            )
        ...
        if selected_tops:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t("admin.top.confirm_save", lang), callback_data="top_save_"
                    )
                ]
            )
    ...
    async def set_top_attachment_selected(self, update: Update, context: CustomContext):
        ...
        keyboard.insert(
            ...
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="top_ans_back"
                )
            ],
        )
        ...
        if query.data == "top_ans_yes":
            ...
            if att_id not in selected_tops:
                if len(selected_tops) >= 5:
                    await query.answer(
                        t("admin.top.limit_reached", lang, max=5), show_alert=True
                    )
                else:
                    selected_tops.append(att_id)
                    context.user_data["selected_tops"] = selected_tops
                    await query.answer(
                        t("admin.top.added_to_top", lang), show_alert=False
                    )
            else:
                await query.answer(
                    t("admin.top.already_selected", lang), show_alert=False
                )
            ...
            if att_id in selected_tops:
                ...
                context.user_data["selected_tops"] = selected_tops
                await query.answer(
                    t("admin.top.removed_from_top", lang), show_alert=False
                )
            else:
