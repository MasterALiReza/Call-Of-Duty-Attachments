...
class SuggestedAttachmentsHandler(BaseAdminHandler):
    ...
    async def manage_suggested_menu(self, update: Update, context: CustomContext):
        ...
        if Permission.MANAGE_SUGGESTED_ATTACHMENTS not in user_permissions:
            ...
            log_performance(
                performance_logger,
                "Suggested.manage_suggested_menu(no_permission)",
                duration,
                threshold=0.2,
            )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.suggested.buttons.add", lang), callback_data="sugg_add"
                ),
                InlineKeyboardButton(
                    t("admin.suggested.buttons.remove", lang),
                    callback_data="sugg_remove",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.suggested.buttons.view_list", lang),
                    callback_data="sugg_list",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.suggested.buttons.analytics", lang),
                    callback_data="sugg_analytics",
                )
            ],
            [
                InlineKeyboardButton(
        ...
        log_performance(
            performance_logger,
            "TG.edit_message_text(Suggested.manage_suggested_menu)",
        ...
        log_performance(
            performance_logger,
            "Suggested.manage_suggested_menu(total)",
            duration,
            threshold=0.3,
        )
    ...
    async def suggested_analytics_menu(self, update: Update, context: CustomContext):
        ...
        text = (
            ...
            + "\n\n"
            + t(
                "admin.suggested.analytics.summary",
                lang,
                total=total,
                br=br_count,
                mp=mp_count,
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.suggested.analytics.buttons.trending", lang),
                    callback_data="analytics_sugg_trending",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.suggested.analytics.buttons.underperforming", lang),
                    callback_data="analytics_sugg_underperforming",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.suggested.analytics.buttons.weapon_stats", lang),
                    callback_data="analytics_sugg_weapon_stats",
                )
            ],
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="admin_manage_suggested"
                )
            ],
        ]
    ...
    async def analytics_sugg_trending(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="sugg_analytics"
                )
            ]
        ]
        await safe_edit_message_text(
    ...
    async def analytics_sugg_underperforming(
        ...
        for item in items:
            ...
            if (likes - dislikes) <= 0 or views < 20:
                under.append(
                    ((item or {}).get("weapon"), (item or {}).get("mode"), att)
                )
    ...
    async def analytics_sugg_weapon_stats(self, update: Update, context: CustomContext):
        ...
        for item in items:
            ...
            weapon = (item or {}).get("weapon")
            s = stats.setdefault(
                weapon, {"count": 0, "likes": 0, "dislikes": 0, "views": 0}
            )
            s["count"] += 1
        ...
        if not stats:
            ...
            ranked = sorted(stats.items(), key=lambda kv: kv[1]["views"], reverse=True)[
                :10
            ]
    ...
    async def suggested_weapon_selected(self, update: Update, context: CustomContext):
        ...
        for att in attachments:
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        f"{status}{att['name']}", callback_data=f"saatt_{att['id']}"
                    )
                ]
            )
    ...
    async def suggested_attachment_selected(
        self, update: Update, context: CustomContext
    ):
        ...
        if await self.db.attachments.is_attachment_suggested(att_id, mode):
            ...
            await query.answer(
                t("admin.suggested.already_suggested_alert", lang), show_alert=True
            )
            return SUGGESTED_ADD_ATTACHMENT
            ...
            await safe_edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                t("admin.suggested.buttons.add_more", lang),
                                callback_data="sugg_add",
                            )
                        ],
                        [
                            InlineKeyboardButton(
    ...
    async def suggested_remove_mode_selected(
        self, update: Update, context: CustomContext
    ):
        ...
        log_performance(
            performance_logger,
            f"DB.get_suggested_attachments({mode})",
            time.perf_counter() - t0,
            threshold=0.2,
        )
        ...
        if not items:
            await safe_edit_message_text(
                query,
                t(
                    "admin.suggested.remove.empty_in_mode",
                    lang,
                    mode_name=f"{t('mode.label', lang)}: {t(f'mode.{mode}_short', lang)}",
                ),
                reply_markup=InlineKeyboardMarkup(
            ...
            log_performance(
                performance_logger,
                "Suggested.remove_mode_selected(empty)",
                duration,
                threshold=0.3,
            )
            return SUGGESTED_ADD_MODE
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="sugg_remove"
                )
            ]
        )
        ...
        await safe_edit_message_text(
            query,
            t(
                "admin.suggested.remove.list_prompt",
                lang,
                mode_name=f"{t('mode.label', lang)}: {t(f'mode.{mode}_short', lang)}",
            ),
            reply_markup=InlineKeyboardMarkup(keyboard),
        ...
        log_performance(
            performance_logger,
            f"TG.edit_message_text(Suggested.remove_mode_selected:{mode})",
            time.perf_counter() - edit_ts,
            threshold=0.25,
        )
        ...
        log_performance(
            performance_logger,
            "Suggested.remove_mode_selected(total)",
            duration,
            threshold=0.4,
        )
    ...
    async def suggested_view_list(self, update: Update, context: CustomContext):
        ...
        log_performance(
            performance_logger,
            "DB.get_suggested_attachments(br)",
            time.perf_counter() - t0,
            threshold=0.2,
        )
        t1 = time.perf_counter()
        ...
        log_performance(
            performance_logger,
            "DB.get_suggested_attachments(mp)",
            time.perf_counter() - t1,
            threshold=0.2,
        )
        ...
        if br_items:
            ...
            if len(br_items) > 10:
                text += (
                    t("common.items_other_count", lang, n=(len(br_items) - 10)) + "\n"
                )
            text += "\n"
        ...
        if mp_items:
            ...
            if len(mp_items) > 10:
                text += (
                    t("common.items_other_count", lang, n=(len(mp_items) - 10)) + "\n"
                )
        ...
        log_performance(
            performance_logger,
            "TG.edit_message_text(Suggested.view_list)",
            time.perf_counter() - v_ts,
            threshold=0.25,
        )
    ...
    async def _rebuild_state_screen(
        ...
        if state == MANAGE_SUGGESTED_MENU:
            ...
            keyboard = self._make_mode_selection_keyboard(
                "samode_", lang, allowed_modes
            )
            keyboard.append(
                [
                    InlineKeyboardButton(
            ...
            for k, v in WEAPON_CATEGORIES.items():
                if await is_category_enabled(k, mode, self.db):
                    active_categories[k] = v
            keyboard = await build_category_keyboard(
                callback_prefix="sacat_", active_ids=list(active_categories.keys())
            )
            self._add_back_cancel_buttons(keyboard, show_back=True)
