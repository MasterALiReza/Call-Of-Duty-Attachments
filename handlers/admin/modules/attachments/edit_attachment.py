...
class EditAttachmentHandler(BaseAdminHandler):
    ...
    async def edit_attachment_start(self, update: Update, context: CustomContext):
        ...
        await safe_edit_message_text(
            query, t("admin.edit.mode.prompt", lang), reply_markup=reply_markup
        )

        return EDIT_ATTACHMENT_MODE

    @log_admin_action("edit_attachment_mode_selected")
    async def edit_attachment_mode_selected(
    ...
    async def edit_attachment_category_selected(
        self, update: Update, context: CustomContext
    ):
    ...
    async def edit_attachment_weapon_selected(
        self, update: Update, context: CustomContext
    ):
        ...
        if query.data == "nav_back":
            ...
            keyboard = await build_category_keyboard(
                callback_prefix="ecat_", active_ids=list(WEAPON_CATEGORIES.keys())
            )
            self._add_back_cancel_buttons(keyboard, show_back=True)
    ...
    async def _edit_attachment_list_menu(self, update: Update, context: CustomContext):
        ...
        for att in attachments:
            ...
            keyboard.append(
                [InlineKeyboardButton(button_text, callback_data=f"edatt_{att['id']}")]
            )
        self._add_back_cancel_buttons(keyboard, show_back=True)
    ...
    async def edit_attachment_action_menu(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.edit.buttons.edit_name", lang), callback_data="edact_name"
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.edit.buttons.edit_code", lang), callback_data="edact_code"
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.edit.buttons.edit_image", lang),
                    callback_data="edact_image",
                )
            ],
        ]
    ...
    async def edit_attachment_action_selected(
        self, update: Update, context: CustomContext
    ):
        ...
        if data == "edact_name":
            ...
            await query.message.reply_text(
                t("admin.edit.name.ask", lang), reply_markup=ReplyKeyboardRemove()
            )
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        t("menu.buttons.back", lang), callback_data="edact_menu"
                    )
                ]
            ]
            await safe_edit_message_text(
                query,
                t("admin.edit.image.prompt", lang),
                reply_markup=InlineKeyboardMarkup(keyboard),
            )
            ...
            # حذف reply keyboard
            await query.message.reply_text(
                t("admin.edit.code.ask", lang), reply_markup=ReplyKeyboardRemove()
            )
    ...
    async def edit_attachment_name_received(
        self, update: Update, context: CustomContext
    ):
        """ویرایش نام اتچمنت"""
        import logging
        ...
        try:
            ...
            old_name = None
            try:
                for att in await self.db.attachments.get_all_attachments(
                    category, weapon, mode=mode
                ):
            ...
            if ok:
                ...
                await update.message.reply_text(
                    t("admin.edit.name.success", lang, new_name=new_name)
                )
    ...
    async def edit_attachment_image_received(
        self, update: Update, context: CustomContext
    ):
        """ویرایش عکس اتچمنت"""
        # بازگشت بدون تغییر
    ...
    async def edit_attachment_code_received(
        self, update: Update, context: CustomContext
    ):
        """ویرایش کد اتچمنت"""
        import logging
        ...
        try:
            ...
            ok = await self.db.attachments.update_attachment_code(
            ...
            if ok:
                ...
                await update.message.reply_text(
                    t("admin.edit.code.success", lang, new_code=new_code)
                )
                await self._auto_notify(
                    context,
                    "edit_code",
