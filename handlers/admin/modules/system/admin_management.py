...
class AdminManagementHandler(BaseAdminHandler):
    ...
    def __init__(self, db):
        ...
        self._admin_list_cache_time = 0.0
    ...
    async def _get_cached_admin_list(self):
        ...
        if (
            self._admin_list_cache is None
            or now - self._admin_list_cache_time > self._CACHE_TTL
        ):
    ...
    async def manage_admins_menu(self, update: Update, context: CustomContext):
        ...
        if not await self.role_manager.is_super_admin(user_id):
            ...
            await safe_edit_message_text(
                query, t("common.no_permission", lang), parse_mode="Markdown"
            )
            return ADMIN_MENU
        ...
        multi_role_admins = 0
        # شمارش نقش‌ها
        role_counts: dict[str, dict[str, int | str]] = {}
        for a in admins:
            ...
            for r in roles:
                ...
                current_count = role_counts[key]["count"]
        ...
        text += (
            t(
                "admin.admin_mgmt.stats.total",
                lang,
                n=_n(total_admins) if lang == "fa" else total_admins,
            )
            + "\n"
        )
        text += (
            t(
                "admin.admin_mgmt.stats.super",
                lang,
                n=_n(super_admins) if lang == "fa" else super_admins,
            )
            + "\n"
        )
        text += (
            t(
                "admin.admin_mgmt.stats.multi",
                lang,
                n=_n(multi_role_admins) if lang == "fa" else multi_role_admins,
            )
            + "\n\n"
        )
        ...
        if admins:
            ...
            if role_counts:
                ...
                sorted_roles = sorted(
                    role_counts.items(), key=lambda x: x[1]["count"], reverse=True
                )
                for name, info in sorted_roles:
                    count_value = info["count"] if isinstance(info["count"], int) else 0
                    if count_value > 0:
                        name_local_raw = t(f"roles.names.{name}", lang)
                        display_name_value = str(info["display_name"])
                        icon_value = str(info["icon"])
                        name_local = _strip_emoji(
                            name_local_raw
                            if name_local_raw
                            and not name_local_raw.startswith("roles.names.")
                            else display_name_value
                        )
                        n_local = _n(count_value) if lang == "fa" else str(count_value)
                        line_local = t(
            ...
            for idx, admin in enumerate(admins[:8], 1):  # نمایش 8 ادمین اول
                ...
                if roles_count > 0:
                    ...
                    if more > 0:
                        roles_line += t(
                            "admin.admin_mgmt.list.more_roles",
                            lang,
                            n=_n(more) if lang == "fa" else more,
                        )
                    line_local = t(
                        "admin.admin_mgmt.list.row.roles",
                        lang,
                        title=title,
                        count=_n(roles_count) if lang == "fa" else roles_count,
                        roles=roles_line,
                    )
                    text += line_local + "\n"
                else:
                    line_local = t(
                        "admin.admin_mgmt.list.row.no_roles",
                        lang,
                        title=title,
                        count=_n(0) if lang == "fa" else 0,
                    )
                    text += line_local + "\n"
            ...
            if len(admins) > 8:
                more_n = len(admins) - 8
                text += "\n" + t(
                    "admin.admin_mgmt.more_admins",
                    lang,
                    n=_n(more_n) if lang == "fa" else more_n,
                )
        else:
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.admin_mgmt.buttons.add", lang),
                    callback_data="add_new_admin",
                ),
                InlineKeyboardButton(
                    t("admin.admin_mgmt.buttons.view_all", lang),
                    callback_data="view_all_admins",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.admin_mgmt.buttons.edit_role", lang),
                    callback_data="edit_admin_role",
                ),
                InlineKeyboardButton(
                    t("admin.admin_mgmt.buttons.roles", lang),
                    callback_data="view_roles",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.admin_mgmt.buttons.remove", lang),
            ...
            [
                ...
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="admin_back"
                )
            ],
        ]
    ...
    async def add_admin_start(self, update: Update, context: CustomContext):
        ...
        if not await self.role_manager.is_super_admin(user_id):
            ...
            await self.audit_permission_denied(
                user_id,
                route="add_admin_start",
                permission="SUPER_ADMIN",
                source="add_admin_start",
            )
            await safe_edit_message_text(
                query, t("common.no_permission", lang), parse_mode="Markdown"
            )
        ...
        for role in roles:
            ...
            logger.info(
                f"Creating role button: {role.display_name} | callback: {callback_data}"
            )
            row.append(
                InlineKeyboardButton(
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="manage_admins"
                )
            ]
        )

        logger.info(
            f"Add admin role selection menu created (grid) with {len(roles)} roles in {len(keyboard) - 1} rows"
        )
    ...
    async def add_admin_role_selected(self, update: Update, context: CustomContext):
        ...
        text = (
            t("admin.admin_mgmt.add_admin.role_selected", lang, role=role.display_name)
            + "\n"
        )
        text += (
            t("admin.admin_mgmt.add_admin.role_desc", lang, desc=role.description)
            + "\n\n"
        )
        ...
        try:
            ...
            if await self.role_manager.is_admin(new_admin_id):
                await update.message.reply_text(
                    t("admin.admin_mgmt.add_admin.already_admin", lang)
                )
    ...
    async def add_admin_display_name_received(
        ...
        try:
            ...
            if not new_admin_id:
                await update.message.reply_text(
                    t("admin.admin_mgmt.errors.no_user_id", lang)
                )
                return await self.admin_menu_return(update, context)
            ...
            success = await self.db.users.assign_role_to_admin(
                user_id=new_admin_id, role_name=role_name, display_name=display_name
            )
            ...
            if success:
                ...
                if display_name:
                    msg += (
                        t(
                            "admin.admin_mgmt.add_admin.success.name_line",
                            lang,
                            name=display_name,
                        )
                        + "\n"
                ...
                msg += (
                    t(
                        "admin.admin_mgmt.add_admin.success.id_line",
                        lang,
                        id=new_admin_id,
                    )
                    + "\n"
                ...
                msg += t(
                    "admin.admin_mgmt.add_admin.success.role_line",
                    lang,
    ...
    async def view_roles_menu(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.admin_mgmt.buttons.role_stats", lang),
                    callback_data="role_stats",
                ),
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="manage_admins"
                ),
    ...
    async def edit_admin_role_start(self, update: Update, context: CustomContext):
        ...
        for admin in admins:
            ...
            logger.info(
                f"Processing admin: {user_id_str}, display_name: {display_name}"
            )
            ...
            logger.info(
                f"✅ Button: '{btn_text}' | Callback: '{callback_data}' | Length: {len(callback_data)}"
            )
    ...
    async def edit_admin_role_select(self, update: Update, context: CustomContext):
        ...
        admin_data = await self.db.users.get_admin(admin_user_id)
        if not admin_data:
            lang = await get_user_lang(update, context, self.db) or "fa"
            await safe_edit_message_text(
                query, t("admin.admin_mgmt.errors.admin_not_found", lang)
            )
            return await self.admin_menu_return(update, context)
        ...
        if not current_roles:
            lang = await get_user_lang(update, context, self.db) or "fa"
            await safe_edit_message_text(
                query, t("admin.admin_mgmt.errors.no_roles_for_admin", lang)
            )
            return await self.admin_menu_return(update, context)
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.admin_mgmt.buttons.add_role_new", lang),
                    callback_data=f"addrole_{admin_user_id}",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.admin_mgmt.buttons.delete_role", lang),
                    callback_data=f"delrole_{admin_user_id}",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def add_role_to_admin(self, update: Update, context: CustomContext):
        ...
        for role in roles:
            row.append(
                InlineKeyboardButton(
                    role.display_name, callback_data=f"newrole_{role.name}"
                )
            )
            if len(row) == 2:
                keyboard.append(row)
    ...
    async def add_role_confirm(self, update: Update, context: CustomContext):
        ...
        if not admin_user_id:
            await safe_edit_message_text(
                query, t("admin.admin_mgmt.errors.no_admin_id", lang)
            )
            return await self.admin_menu_return(update, context)

        # افزودن نقش جدید
        success = await self.db.users.assign_role_to_admin(admin_user_id, new_role_name)
        ...
        if not success:
            await safe_edit_message_text(
                query, t("admin.admin_mgmt.add_role.error", lang)
            )
            context.user_data.pop("edit_admin_user_id", None)
            return await self.admin_menu_return(update, context)
        ...
        msg += (
            t(
                "admin.admin_mgmt.add_role.success.added_role",
                lang,
                role=role.display_name,
            )
            + "\n\n"
        ...
        msg += (
            t(
                "admin.admin_mgmt.add_role.success.current_roles",
                lang,
                n=len(current_roles),
            )
            + "\n"
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t(
                        "admin.admin_mgmt.buttons.add_role_more",
                        await get_user_lang(update, context, self.db) or "fa",
                    ),
                    callback_data=f"addrole_{admin_user_id}",
                )
            ],
            [
                InlineKeyboardButton(
                    t(
                        "admin.admin_mgmt.buttons.delete_role",
                        await get_user_lang(update, context, self.db) or "fa",
                    ),
                    callback_data=f"delrole_{admin_user_id}",
                )
            ...
            [
                InlineKeyboardButton(
                    t(
                        "admin.admin_mgmt.buttons.back_to_admins",
                        await get_user_lang(update, context, self.db) or "fa",
                    ),
    ...
    async def delete_role_from_admin(self, update: Update, context: CustomContext):
        ...
        if len(current_roles) <= 1:
            ...
            msg += t(
                "admin.admin_mgmt.delete_role.cannot_last.body",
                lang,
                name=name_line,
                role=role_disp,
            )
            keyboard = [
                ...
                [
                    InlineKeyboardButton(
                        t("admin.admin_mgmt.buttons.remove", lang),
                        callback_data=f"remove_confirm_{admin_user_id}",
                    )
                ],
                [
                    InlineKeyboardButton(
                        t("admin.admin_mgmt.buttons.back_to_admins", lang),
                        callback_data="manage_admins",
                    )
                ],
            ]
        ...
        for role in current_roles:
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        role_disp, callback_data=f"delconfirm_{role_name}"
                    )
                ]
            )
    ...
    async def delete_role_confirm(self, update: Update, context: CustomContext):
        ...
        if not admin_user_id:
            await query.edit_message_text(
                t("admin.admin_mgmt.errors.no_admin_id", lang)
            )
            return await self.admin_menu_return(update, context)
        ...
        if role_name == "super_admin":
            ...
            super_admins = [
                ...
                for a in all_admins
                if any(
                    (r if isinstance(r, str) else r.get("name")) == "super_admin"
                    for r in a.get("roles", [])
                )
            ]
            if len(super_admins) <= 1 and any(
                a["user_id"] == int(admin_user_id) for a in super_admins
            ):
                await query.edit_message_text(
        ...
        # حذف نقش
        success = await self.db.users.remove_role_from_admin(admin_user_id, role_name)
        ...
        if not admin_data or not admin_data.get("roles"):
            ...
            await self.db.users.remove_admin(admin_user_id)
            ...
            await query.edit_message_text(
                ...
                + "\n\n"
                + t(
                    "admin.admin_mgmt.remove.success.body",
                    lang,
                    name=name_line,
                    id=admin_user_id,
        ...
        keyboard = [
            ...
            [
                InlineKeyboardButton(
                    t(
                        "admin.admin_mgmt.buttons.delete_role_more",
                        await get_user_lang(update, context, self.db) or "fa",
                    ),
                    callback_data=f"delrole_{admin_user_id}",
                )
    ...
    async def remove_admin_start(self, update: Update, context: CustomContext):
        ...
        if not await self.role_manager.is_super_admin(user_id):
            await query.answer(
                "❌ فقط ادمین کل می‌تواند ادمین حذف کند.", show_alert=True
            )
            return ADMIN_MENU
        ...
        if len(other_admins) == 0:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        t("admin.admin_mgmt.buttons.add_admin_new", lang),
                        callback_data="add_new_admin",
                    )
                ],
                [
        ...
        for admin in other_admins:
            ...
            keyboard.append(
                [InlineKeyboardButton(btn_text, callback_data=f"remove_{user_id_str}")]
            )
        ...
        lang = await get_user_lang(update, context, self.db) or "fa"
        await query.edit_message_text(
            t("admin.admin_mgmt.remove.select_admin", lang), reply_markup=reply_markup
        )
    ...
    async def remove_admin_confirmed(self, update: Update, context: CustomContext):
        ...
        if query.data.startswith("remove_") and not query.data.startswith(
            "remove_confirm_"
        ):
            admin_id = int(query.data.replace("remove_", ""))

            # دریافت اطلاعات ادمین برای نمایش
            admin_data = await self.db.users.get_admin(admin_id)
            display_name = (
                str(admin_data.get("display_name") or f"`{admin_id}`")
                if admin_data
                else f"`{admin_id}`"
            )
            ...
            # صفحه تایید
            text = (
                t("admin.admin_mgmt.remove.confirm.title", lang)
                + "\n\n"
                + t(
                    "admin.admin_mgmt.remove.confirm.body",
                    lang,
                    name=display_name,
                    id=admin_id,
                )
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        t("admin.admin_mgmt.confirm.remove.yes", lang),
                        callback_data=f"remove_confirm_{admin_id}",
                    ),
                ],
                [
                    InlineKeyboardButton(
        ...
        if admin_id == query.from_user.id:
            ...
            admin_data = context.user_data.pop(
            ...
            if admin_data and any(
                (r if isinstance(r, str) else r.get("name")) == "super_admin"
                for r in admin_data.get("roles", [])
            ):
                all_admins = await self._get_cached_admin_list()
                super_admins = [
                    a
            ...
            success = await self.db.users.remove_admin(admin_id)
            if success:
    ...
    async def role_stats(self, update: Update, context: CustomContext):
        ...
        sorted_roles = sorted(
            role_usage.items(), key=lambda x: x[1]["count"], reverse=True
        )
        ...
        text += "━━━━━━━━━━━━━━━━━━━━\n\n"
        text += (
            t("admin.admin_mgmt.role_stats.total_admins", lang, n=len(admins)) + "\n"
        )
        text += (
            t("admin.admin_mgmt.role_stats.total_roles", lang, n=len(roles)) + "\n\n"
        )
        ...
        for idx, (role_name, data) in enumerate(sorted_roles, 1):
            ...
            text += (
                t(
                    "admin.admin_mgmt.role_stats.rank.line_title",
                    lang,
                    i=idx,
                    icon=icon,
            ...
            text += (
                t(
                    "admin.admin_mgmt.role_stats.rank.bar",
                    lang,
                    bar=bar,
