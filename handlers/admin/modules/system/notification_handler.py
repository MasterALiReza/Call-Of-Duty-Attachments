...
class NotificationHandler(BaseAdminHandler):
    ...
    async def notify_home_menu(self, update: Update, context: CustomContext):
        ...
        if (
            Permission.SEND_NOTIFICATIONS not in user_permissions
            and not await self.role_manager.is_super_admin(query.from_user.id)
        ):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.notify.buttons.compose", lang),
                    callback_data="notify_compose",
                )
            ]
        ]
        if (
            Permission.MANAGE_SCHEDULED_NOTIFICATIONS in user_permissions
        ) or await self.role_manager.is_super_admin(query.from_user.id):
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t("admin.notify.buttons.scheduled", lang),
                        callback_data="admin_sched_notifications",
                    )
                ]
            )
        keyboard.append(
    ...
    async def schedule_edit_open(self, update: Update, context: CustomContext):
        ...
        lang = await get_user_lang(update, context, self.db) or "fa"
        if (
            Permission.MANAGE_SCHEDULED_NOTIFICATIONS not in user_permissions
            and not await self.role_manager.is_super_admin(query.from_user.id)
        ):
            await self.audit_permission_denied(
                query.from_user.id,
        ...
        text = (
            ...
            + "\n\n"
            + t(
                "admin.notify.schedule.edit.header",
                lang,
                id=row["id"],
                status=status_text(row["enabled"]),
                hours=row["interval_hours"],
            )
            + "\n"
            + t(
                "admin.notify.schedule.edit.next",
                lang,
                next=_fmt_dt_local(row.get("next_run_at")),
            )
        )
        ...
        if row.get("message_type") == "text":
            ...
            if preview:
                text += "\n" + t(
                    "admin.notify.schedule.edit.text_preview", lang, preview=preview
                )
            m_kb = [
                [
                    InlineKeyboardButton(
                        t("admin.notify.buttons.edit_text", lang),
                        callback_data=f"sched_edit_text_{sid}",
                    )
                ]
            ]
        else:
            ...
            if cap:
                text += "\n" + t(
                    "admin.notify.schedule.edit.photo_caption_preview", lang, cap=cap
                )
            else:
            ...
            m_kb = [
                [
                    InlineKeyboardButton(
                        t("admin.notify.buttons.edit_caption", lang),
                        callback_data=f"sched_edit_text_{sid}",
                    )
                ]
            ]
        ...
        await safe_edit_message_text(
            query, text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(m_kb)
        )
    ...
    async def schedule_edit_text_start(self, update: Update, context: CustomContext):
        ...
        await safe_edit_message_text(
            query, msg, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb)
        )
    ...
    async def schedule_edit_text_received(self, update: Update, context: CustomContext):
        ...
        if not update.message or not update.message.text:
            await update.message.reply_text(
                t("admin.notify.schedule.edit_text.only_text", lang)
            )
            from handlers.admin.admin_states import EDIT_SCHEDULE_TEXT
        ...
        if ok:
            kb = [
                [
                    InlineKeyboardButton(
                        t("admin.notify.buttons.back_to_schedules", lang),
                        callback_data="admin_sched_notifications",
                    )
                ]
            ]
            await update.message.reply_text(
                t("admin.notify.schedule.edit_text.success", lang),
            ...
            await update.message.reply_text(
                t("admin.notify.schedule.edit_text.error", lang),
                reply_markup=InlineKeyboardMarkup(kb),
            )
    ...
    async def notify_compose_start(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="notify_home"
                )
            ]
        ]
        await safe_edit_message_text(
    ...
    async def schedules_menu(self, update: Update, context: CustomContext):
        ...
        if (
            ...
            await query.answer(
                t("admin.notify.schedule.no_permission", lang), show_alert=True
            )
            from handlers.admin.admin_states import ADMIN_MENU
        ...
        if not items:
            ...
            for it in items[:20]:
                ...
                text += (
                    t(
                        "admin.notify.schedules.item.header",
                        lang,
                        sid=sid,
                        status=status_text,
                        hours=interval,
                    ...
                    + "\n"
                    + t(
                        "admin.notify.schedules.item.next",
                        lang,
                        next=fmt_dt(it.get("next_run_at")),
                    )
                    + "\n"
                )
                # نمایش نوع پیام
                ...
                if mtype == "text":
                    ...
                    if preview:
                        ...
                        text += (
                            t("admin.notify.schedules.item.text", lang, preview=preview)
                            + "\n"
                        )
                else:
                    ...
                    if cap:
                        text += (
                            t(
                                "admin.notify.schedules.item.photo_caption",
                                lang,
                                cap=cap,
                            )
                            + "\n"
        ...
        for it in items[:10]:
            ...
            en_btn = InlineKeyboardButton(
                (
                    t("admin.notify.buttons.disable", lang)
                    if it["enabled"]
                    else t("admin.notify.buttons.enable", lang)
                ),
                callback_data=f"sched_toggle_{sid}",
            )
            del_btn = InlineKeyboardButton(
                t("common.delete", lang), callback_data=f"sched_delete_{sid}"
            )
            edit_btn = InlineKeyboardButton(
                t("common.edit", lang), callback_data=f"sched_edit_{sid}"
            )
    ...
    async def notify_compose_received(self, update: Update, context: CustomContext):
        ...
        if context.user_data.get("tmpl_key"):
            ...
            if not update.message.text:
                await update.message.reply_text(
                    t("admin.notify.templates.only_text", lang)
                )
                return NOTIFY_COMPOSE
        ...
        if update.message.photo:
            ...
            result = AttachmentValidator.validate_image(
            ...
            preview_caption = (
                (context.user_data["notif_text"] or "")
                + "\n\n"
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        t("admin.notify.buttons.confirm_send", lang),
                        callback_data="nconf_send",
                    )
                ],
                [
                    InlineKeyboardButton(
                        t("admin.notify.buttons.schedule_next", lang),
                        callback_data="notify_schedule",
                    )
                ],
                [
                    InlineKeyboardButton(
        ...
        await update.message.reply_text(
            t("admin.notify.compose.only_text_or_photo", lang)
        )
        return NOTIFY_COMPOSE
            ...
            try:
                if notif_type == "photo" and photo:
                    try:
                        ...
                        await context.bot.send_photo(
                            chat_id=uid, photo=photo, caption=notif_text or None
                        )
                else:
                    try:
                        await context.bot.send_message(
                            chat_id=uid, text=notif_text, parse_mode="Markdown"
                        )
                    except Exception:
    ...
    async def notify_schedule_menu(self, update: Update, context: CustomContext):
        ...
        if not notif_type or (notif_type == "text" and not notif_text):
            await query.answer(
                t("admin.notify.compose.only_text_or_photo", lang), show_alert=True
            )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.notify.schedule.menu.6h", lang),
                    callback_data="notif_sched_6h",
                ),
                InlineKeyboardButton(
                    t("admin.notify.schedule.menu.12h", lang),
                    callback_data="notif_sched_12h",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.notify.schedule.menu.24h", lang),
                    callback_data="notif_sched_24h",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.notify.schedule.menu.48h", lang),
                    callback_data="notif_sched_48h",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def notify_schedule_preset_selected(
        ...
        try:
            new_id = await self.db.cms.create_scheduled_notification(
                message_type="photo"
                if (notif_type == "photo" and notif_photo)
                else "text",
        ...
        if new_id:
            try:
                await query.answer(
                    t("admin.notify.schedule.saved", lang), show_alert=False
                )
            except Exception:
                pass
            return await self.notify_home_menu(update, context)
    ...
    async def notify_settings_menu(self, update: Update, context: CustomContext):
        ...
        text = (
            ...
            + "\n\n"
            + t(
                "admin.notify.settings.status",
                lang,
                status=t("common.status.enabled", lang)
                if enabled
                else t("common.status.disabled", lang),
            )
            + "\n"
            + t(
                "admin.notify.settings.auto",
                lang,
                status=t("common.status.enabled", lang)
                if auto_notify
                else t("common.status.disabled", lang),
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.notify.settings.toggle.disable", lang)
                    if enabled
                    else t("admin.notify.settings.toggle.enable", lang),
                    callback_data="notif_toggle",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.notify.settings.auto.toggle.off", lang)
                    if auto_notify
                    else t("admin.notify.settings.auto.toggle.on", lang),
                    callback_data="notif_auto_toggle",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.notify.settings.edit_templates", lang),
                    callback_data="notif_templates",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def notify_toggle(self, update: Update, context: CustomContext):
        ...
        lang = await get_user_lang(update, context, self.db) or "fa"
        status_msg = (
            t("admin.notify.settings.toggled.enabled", lang)
            if settings["enabled"]
            else t("admin.notify.settings.toggled.disabled", lang)
        )
    ...
    async def notify_auto_toggle(self, update: Update, context: CustomContext):
        ...
        lang = await get_user_lang(update, context, self.db) or "fa"
        status_msg = (
            t("admin.notify.settings.auto.toggled.enabled", lang)
            if settings["auto_notify"]
            else t("admin.notify.settings.auto.toggled.disabled", lang)
        )
        await query.answer(status_msg, show_alert=True)
    ...
    async def template_list_menu(self, update: Update, context: CustomContext):
        ...
        lang = await get_user_lang(update, context, self.db) or "fa"
        text = (
            t("notification.events.title", lang)
            + "\n\n"
        ...
        for event_key, event_name in event_names.items():
            ...
            keyboard.append(
                [
                    ...
                    InlineKeyboardButton("✏️", callback_data=f"tmpl_edit_{event_key}"),
                ]
            )
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="admin_notify_settings"
                )
            ]
        )
    ...
    async def notif_event_toggle(self, update: Update, context: CustomContext):
        ...
        await query.answer(
            t("admin.notify.events.toggled", lang, status=status_text), show_alert=False
        )
