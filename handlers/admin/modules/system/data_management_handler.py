...
class DataManagementHandler(BaseAdminHandler):
    ...
    async def data_management_menu(self, update: Update, context: CustomContext) -> int:
        ...
        if not await self.role_manager.has_permission(
            user_id, Permission.BACKUP_DATA
        ) and not await self.role_manager.is_super_admin(user_id):
            await self.send_permission_denied(
                update,
                context,
                route="admin_data_management",
                permission=Permission.BACKUP_DATA,
                source="data_management_menu",
            )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.data_mgmt.btn.create_backup", lang),
                    callback_data="admin_create_backup",
                ),
                InlineKeyboardButton(
                    t("admin.data_mgmt.btn.auto_backup", lang),
                    callback_data="admin_auto_backup_menu",
                ),
            ],
            [
                InlineKeyboardButton(
    ...
    async def auto_backup_menu(self, update: Update, context: CustomContext) -> int:
        ...
        if not await self.role_manager.has_permission(
            ...
            await self.send_permission_denied(
                update,
                context,
                route="admin_auto_backup_menu",
                permission=Permission.BACKUP_DATA,
                source="auto_backup_menu",
            )
            return cast(int, ADMIN_MENU)
        ...
        try:
            ...
            enabled = (
                await self.db.settings.get_setting("auto_backup_enabled", "0") == "1"
            )
            current_interval = await self.db.settings.get_setting(
            ...
            text += t(
                "admin.auto_backup.menu.interval", lang, interval=current_interval
            )
            ...
            for label, val in intervals:
                ...
                interval_row.append(
                    InlineKeyboardButton(
                        btn_text, callback_data=f"set_ab_interval_{val}"
                    )
                )
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        t(
                            "admin.auto_backup.btn.toggle_off"
                            if enabled
                            else "admin.auto_backup.btn.toggle_on",
                            lang,
                        ),
    ...
    async def toggle_auto_backup(self, update: Update, context: CustomContext) -> None:
        ...
        try:
            ...
            await scheduler.update_schedule(
                context.application, new_state, current_interval
            )
    ...
    async def set_auto_backup_interval(
    ...
    async def create_backup(self, update: Update, context: CustomContext) -> int:
        ...
        if backup_file:
            ...
            await context.bot.send_message(
                ...
                text=t(
                    "admin.backup.created", lang, filename=os.path.basename(backup_file)
                ),
