...
class FeedbackAdminHandler(BaseAdminHandler):
    ...
    async def show_feedback_dashboard(self, update: Update, context: CustomContext):
        ...
        stats = await self.db.analytics.get_attachment_feedback_stats(
            suggested_only=suggested_only, days=30
        )
        ...
        keyboard = [
            ...
            [
                InlineKeyboardButton(
                    t("admin.feedback.buttons.period", lang),
                    callback_data="fb_change_period",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.feedback.buttons.top", lang), callback_data="fb_top"
                ),
                InlineKeyboardButton(
                    t("admin.feedback.buttons.bottom", lang), callback_data="fb_bottom"
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.feedback.buttons.comments", lang),
                    callback_data="fb_comments",
                ),
                InlineKeyboardButton(
                    t("admin.feedback.buttons.trend", lang, default="📈 Weekly Trend"),
                    callback_data="fb_trend",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.feedback.buttons.search", lang), callback_data="fb_search"
                ),
                InlineKeyboardButton(
    ...
    async def show_top_attachments(self, update: Update, context: CustomContext):
        ...
        suggested_only = context.user_data.get("fb_suggested_only", False)
        popular = await self.db.analytics.get_popular_attachments(
            limit=10,
            days=period,
            mode=mode,
            category=category,
        ...
        if not popular:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        t("menu.buttons.back", lang), callback_data="fb_dashboard"
                    )
                ]
            ]
        else:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        t("admin.feedback.buttons.change_period", lang),
                        callback_data="fb_change_period",
                    )
                ],
                [
                    InlineKeyboardButton(
                        t("admin.feedback.buttons.filter_mode", lang),
                        callback_data="fb_filter_mode",
                    )
                ],
                [
                    InlineKeyboardButton(
                        t("admin.feedback.buttons.filter_category", lang),
                        callback_data="fb_filter_category",
                    )
                ],
                [
                    InlineKeyboardButton(
    ...
    async def show_search_menu(self, update: Update, context: CustomContext):
        ...
        try:
            ...
            popular = []
        text = (
            t("admin.feedback.search.menu.title", lang)
            + "\n\n"
        ...
        if popular:
            for q in popular:
                ...
                keyboard.append(
                    [
                        InlineKeyboardButton(
                            t("admin.feedback.search.buttons.popular", lang, q=label),
                            callback_data=f"fb_search_q_{enc}",
                        )
                    ]
                )
        else:
    ...
    async def execute_search_query(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.feedback.search.buttons.popular_back", lang),
    ...
    async def filter_mode_menu(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("mode.br_short", lang), callback_data="fb_mode_br"
                ),
                InlineKeyboardButton(
                    t("mode.mp_short", lang), callback_data="fb_mode_mp"
                ),
            ],
            [InlineKeyboardButton(t("mode.all", lang), callback_data="fb_mode_all")],
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="fb_top"
                )
            ],
        ]
        await safe_edit_message_text(
    ...
    async def filter_category_menu(self, update: Update, context: CustomContext):
        ...
        for key, _ in CATEGORIES.items():
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t(f"category.{key}", "en"), callback_data=f"fb_cat_{key}"
                    )
                ]
            )
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("admin.feedback.filter.category.clear", lang),
                    callback_data="fb_cat_clear",
                )
            ]
        )
        keyboard.append(
    ...
    async def show_user_comments(self, update: Update, context: CustomContext):
        ...
        # استفاده از متد جدید ریپازیتوری
        feedbacks = await self.db.analytics.get_attachment_feedback_list(
            page=page, per_page=per_page, suggested_only=suggested_only
        )
        ...
        if not feedbacks["items"]:
            ...
            for item in feedbacks["items"]:
                ...
                feedback_preview = (
                    fb_text[:150] + "..." if len(fb_text) > 150 else fb_text
                )
                ...
                # فرمت تاریخ
                date_str = (
                    item["last_view_date"].strftime("%Y-%m-%d")
                    if isinstance(item["last_view_date"], datetime)
                    else str(item["last_view_date"])[:10]
                )
            ...
            if page > 1:
                nav_row.append(
                    InlineKeyboardButton(
                        t("nav.prev", lang),
                        callback_data=f"fb_comments_page_{page - 1}",
                    )
                )
            nav_row.append(
                InlineKeyboardButton(
    ...
    async def show_weekly_trend(self, update: Update, context: CustomContext):
        ...
        stats_by_category = await self.db.analytics.get_attachment_stats_by_category(
            suggested_only=suggested_only
        )
        stats_by_mode = await self.db.analytics.get_attachment_stats_by_mode(
            suggested_only=suggested_only
        )
        weekly = await self.db.analytics.get_attachment_weekly_trend(
            suggested_only=suggested_only
        )
        ...
        for data in stats_by_mode:
            ...
            text += (
                t(
                    "admin.feedback.detailed.modes.line",
                    lang,
                    mode=mode_name,
                    votes=data["votes"],
                    likes=data["likes"],
        ...
        for data in stats_by_category:
            ...
            text += (
                t(
                    "admin.feedback.detailed.categories.line",
                    lang,
                    category=cat_name,
        ...
        chart_data = [
            (w["week_label"], float(w["votes"])) for w in weekly[-8:]
        ]  # Last 8 weeks
        chart_buf = chart_gen.generate_bar_chart(
            chart_data,
            title=t(
                "admin.feedback.detailed.trend_chart",
                lang,
                default="Weekly Voting Trend",
            ),
        )
    ...
    async def change_period(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.feedback.period.7", lang), callback_data="fb_period_7"
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.feedback.period.30", lang), callback_data="fb_period_30"
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.feedback.period.90", lang), callback_data="fb_period_90"
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.feedback.period.all", lang), callback_data="fb_period_all"
                )
            ],
            [
                InlineKeyboardButton(
