...
class TextHandler(BaseAdminHandler):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.texts.buttons.edit_welcome", lang),
                    callback_data="text_edit_welcome",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.texts.buttons.edit_help", lang),
                    callback_data="text_edit_help",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def text_edit_start(self, update: Update, context: CustomContext):
        ...
        if data in ("text_edit_welcome", "text_edit_help"):
            ...
            title_label = (
                t("admin.texts.item.welcome", lang_ui)
                if key == "welcome"
                else t("admin.texts.item.help", lang_ui)
            )
            text = t("admin.texts.choose_language", lang_ui)
            keyboard = [
                [
                    InlineKeyboardButton(
                        t("lang.fa", lang_ui), callback_data=f"{data}_lang_fa"
                    )
                ],
                [
                    InlineKeyboardButton(
                        t("lang.en", lang_ui), callback_data=f"{data}_lang_en"
                    )
                ],
                [
                    InlineKeyboardButton(
                        t("menu.buttons.back", lang_ui),
        ...
        key = (
            "welcome"
            if data.startswith("text_edit_welcome_lang_")
            else ("help.text" if data.startswith("text_edit_help_lang_") else None)
        )
        if key is None:
        ...
        lang_name = (
            t("lang.fa", lang_ui) if edit_lang == "fa" else t("lang.en", lang_ui)
        )
        msg = (
        ...
        kb = [
            [
                InlineKeyboardButton(
                    t("menu.buttons.cancel", lang_ui), callback_data="admin_cancel"
                )
            ]
        ]
        await query.edit_message_text(
    ...
    def _persist_message_text(self, key: str, new_value: str) -> bool:
        ...
        try:
            config_path = os.path.join(
                os.path.dirname(config_module.__file__), "config.py"
            )
            ...
            def try_sub(pat: str, repl: str) -> int:
                ...
                new_content, count = re.subn(
                    pat, repl, content, count=1, flags=re.DOTALL
                )
                if count:
                    content = new_content
                    replaced = True
