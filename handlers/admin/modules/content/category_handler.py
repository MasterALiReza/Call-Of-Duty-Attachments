...
class CategoryHandler(BaseAdminHandler):
    ...
    async def category_mgmt_menu(self, update: Update, context: CustomContext):
        ...
        text = (
            t("admin.categories.menu.title", lang)
            + "\n\n"
        ...
        if "br" in allowed_modes:
            mode_buttons.append(
                InlineKeyboardButton(
                    f"{t('mode.br', lang)} ({t('mode.br_short', lang)})",
                    callback_data="cmm_br",
                )
            )
        if "mp" in allowed_modes:
            mode_buttons.append(
                InlineKeyboardButton(
                    f"{t('mode.mp', lang)} ({t('mode.mp_short', lang)})",
                    callback_data="cmm_mp",
                )
            )
        if mode_buttons:
    ...
    async def show_category_management_menu(
        self, update: Update, context: CustomContext, mode: str = None
    ):
        """
        ...
        for key in WEAPON_CATEGORIES_IDS:
            enabled = (await get_category_setting(key, mode, self.db)).get(
                "enabled", True
            )
            toggle_label = (
                "✅" if enabled else "❌"
            ) + f" {t(f'category.{key}', 'en')}"
            toggle_buttons.append(
                InlineKeyboardButton(
                    toggle_label, callback_data=f"adm_cat_toggle_{key}"
                )
            )
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("admin.categories.separator.clear", lang), callback_data="noop"
                )
            ]
        )
        ...
        for key, name in WEAPON_CATEGORIES.items():
            clear_buttons.append(
                InlineKeyboardButton(
                    f"🗑 {t(f'category.{key}', 'en')}",
                    callback_data=f"adm_cat_clear_{key}",
                )
            )
    ...
    async def category_clear_prompt(self, update: Update, context: CustomContext):
        ...
        txt = (
            ...
            + "\n\n"
            + t(
                "admin.categories.clear.confirm.prompt",
                lang,
        ...
        kb = [
            [
                InlineKeyboardButton(
                    t("admin.categories.clear.buttons.confirm", lang),
                    callback_data="cat_clear_confirm",
                )
            ],
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="cat_clear_cancel"
                )
            ],
        ]
    ...
    async def category_clear_confirm(self, update: Update, context: CustomContext):
        ...
        if success:
            ...
            msg += t(
                "admin.categories.clear.success.summary",
                lang,
                mode=mode_name,
                category=category_name,
            ...
            logger.info(
                f"Category {category} cleared successfully for mode {mode}, {total_attachments} attachments, backup: {backup_file}"
            )
        else:
