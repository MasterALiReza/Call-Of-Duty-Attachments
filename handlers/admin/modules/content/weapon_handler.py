...
class WeaponHandler(BaseAdminHandler):
    ...
    async def weapon_mgmt_menu(self, update: Update, context: CustomContext):
        ...
        if "br" in allowed_modes:
            mode_buttons.append(
                InlineKeyboardButton(t("mode.br_btn", lang), callback_data="wmm_br")
            )
        if "mp" in allowed_modes:
            mode_buttons.append(
                InlineKeyboardButton(t("mode.mp_btn", lang), callback_data="wmm_mp")
            )
        if mode_buttons:
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="admin_cancel"
                )
            ]
        )
    ...
    async def weapon_mode_selected(self, update: Update, context: CustomContext):
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="nav_back"
                )
            ]
        )
    ...
    async def weapon_select_category_menu(self, update: Update, context: CustomContext):
        ...
        # دریافت سلاح‌ها
        weapons = await self.db.attachments.get_weapons_in_category(
            category, include_inactive=True
        )
        ...
        if not weapons:
            await safe_edit_message_text(
                ...
                t(
                    "admin.weapons.path",
                    lang,
                    mode=t(f"mode.{mode}_short", lang),
        ...
        logger.info(
            f"Category {category} selected, showing {len(weapons)} weapons for mode {mode}"
        )
    ...
    async def weapon_select_weapon_menu(self, update: Update, context: CustomContext):
        ...
        if query.data == "nav_back":
            ...
            keyboard = await build_category_keyboard(
                callback_prefix="wmcat_", lang=lang
            )
            keyboard.append(
                [
                    InlineKeyboardButton(
        ...
        text += (
            t(
                "admin.weapons.stats.line",
                lang,
                mode=t("mode.br_short", lang),
                count=info["br"]["attachment_count"],
                top=info["br"]["top_count"],
            )
            + "\n"
        )
        text += (
            t(
                "admin.weapons.stats.line",
                lang,
                mode=t("mode.mp_short", lang),
                count=info["mp"]["attachment_count"],
                top=info["mp"]["top_count"],
            )
            + "\n\n"
        )
        ...
        if mode_count > 0:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t(
                            "admin.weapons.buttons.clear_mode",
                            lang,
                            mode=mode_name,
                            count=mode_count,
                        ),
        ...
        keyboard.append(
            [InlineKeyboardButton(toggle_text, callback_data="wmact_toggle")]
        )
    ...
    async def weapon_action_selected(self, update: Update, context: CustomContext):
        ...
        if action == "toggle":
            ...
            text = (
                ...
                + "\n\n"
                + t(
                    "admin.weapons.confirm.clear.prompt",
                    lang,
                    mode=clear_mode_short,
                    weapon=weapon,
                )
                + "\n\n"
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        t(
                            "admin.weapons.buttons.confirm_clear",
                            lang,
                            mode=clear_mode_btn,
                        ),
                        callback_data=f"wmconf_clear_{clear_mode}",
                    )
                ],
                [
                    InlineKeyboardButton(
                        t("menu.buttons.cancel", lang), callback_data="nav_back"
                    )
                ],
            ]
            await safe_edit_message_text(
            ...
            logger.info(
                f"Clear {clear_mode} confirmation requested for weapon: {weapon}"
            )
    ...
    async def weapon_delete_confirmed(self, update: Update, context: CustomContext):
        ...
        if query.data == "wmconf_delete":
            ...
            logger.warning(
                f"Legacy cleanup: Attempt to delete weapon blocked: {weapon}"
            )
            ...
            success = await self.db.attachments.delete_weapon(
                category, weapon, mode=clear_mode
            )
            if success:
                msg = (
                    t(
                        "admin.weapons.clear.success",
                        lang,
                        mode=clear_mode_name,
                        weapon=weapon,
                    )
                    + "\n"
                )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.weapons.buttons.back_to_mgmt", lang),
    ...
    def _make_weapon_keyboard(self, weapons: list, prefix: str, category: str) -> list:
        ...
        keyboard = []
        buttons = [
            InlineKeyboardButton(w, callback_data=f"{prefix}{w}") for w in weapons
        ]
