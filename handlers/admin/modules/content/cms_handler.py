...
class CMSHandler(BaseAdminHandler):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.cms.buttons.add", lang), callback_data="cms_add"
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.cms.buttons.list", lang), callback_data="cms_list"
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.cms.buttons.search", lang), callback_data="cms_search"
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def cms_add_start(self, update: Update, context: CustomContext) -> int:
        ...
        kb = [
            [
                InlineKeyboardButton(
                    t("admin.cms.type.news", lang), callback_data="cms_type_news"
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.cms.type.tutorial", lang),
                    callback_data="cms_type_tutorial",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.cms.type.event", lang), callback_data="cms_type_event"
                )
            ],
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="admin_cms"
                )
            ],
        ]
        await safe_edit_message_text(
            query, text, reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown"
        )
        return CMS_ADD_TYPE
    ...
    async def cms_type_selected(self, update: Update, context: CustomContext) -> int:
        ...
        await safe_edit_message_text(
            query,
            t("admin.cms.add.title_prompt", lang),
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            t("menu.buttons.cancel", lang), callback_data="admin_cms"
                        )
                    ]
                ]
            ),
    ...
    async def cms_body_received(self, update: Update, context: CustomContext) -> int:
        ...
        user_id = update.effective_user.id
        cid = await self.cms.create_content(
            ctype, title, body, author_id=user_id, tags=[]
        )
    ...
    async def cms_list_menu(self, update: Update, context: CustomContext) -> int:
        ...
        for r in rows:
            ...
            kb.append(
                [
                    InlineKeyboardButton(
                        t("admin.cms.buttons.publish", lang),
                        callback_data=f"cms_pub_{rid}",
                    ),
                    InlineKeyboardButton(
                        t("admin.cms.buttons.delete", lang),
                        callback_data=f"cms_del_{rid}",
                    ),
                ]
            )
        kb.append(
        ...
        await safe_edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            t("menu.buttons.back", lang), callback_data="cms_list"
                        )
                    ]
                ]
            ),
    ...
    async def cms_search_start(self, update: Update, context: CustomContext) -> int:
        ...
        await safe_edit_message_text(
            query,
            t("admin.cms.search.prompt", lang),
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
