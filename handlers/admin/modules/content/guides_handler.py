...
class GuidesHandler(BaseAdminHandler):
    ...
    async def guides_menu(self, update: Update, context: CustomContext):
        ...
        if self.role_manager:
            if hasattr(self.role_manager, "get_guide_mode_permissions"):
                allowed_modes = await self.role_manager.get_guide_mode_permissions(
                    user_id
                )
            else:
        ...
        if "br" in allowed_modes:
            mode_buttons.append(
                InlineKeyboardButton(t("mode.br", lang), callback_data="gmode_br")
            )
        if "mp" in allowed_modes:
            mode_buttons.append(
                InlineKeyboardButton(t("mode.mp", lang), callback_data="gmode_mp")
            )
    ...
    async def guides_mode_selected(self, update: Update, context: CustomContext):
        ...
        kb = [
            ...
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="admin_guides"
                )
            ],
        ]
    ...
    async def guide_section_menu(
        self, update: Update, context: CustomContext, send_new: bool = False
    ):
        """
        ...
        if send_new:
            ...
            if not key:
                lang = await get_user_lang(update, context, self.db) or "fa"
                await update.message.reply_text(
                    t("admin.guides.error.key_not_found", lang)
                )
        ...
        kb = [
            [
                InlineKeyboardButton(
                    t("admin.guides.buttons.rename", lang),
                    callback_data=f"gop_rename_{key}_{mode}",
                )
            ],
            [
                InlineKeyboardButton(
                    t("admin.guides.buttons.add_photo", lang),
                    callback_data=f"gop_addphoto_{key}_{mode}",
                ),
                InlineKeyboardButton(
                    t("admin.guides.buttons.add_video", lang),
                    callback_data=f"gop_addvideo_{key}_{mode}",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.guides.buttons.clear_media", lang),
                    callback_data=f"gop_clearmedia_{key}_{mode}",
                )
            ],
        ]
        ...
        if key in ["sens", "hud"]:
            ...
            code_label = (
                t("guides.sens_short", lang)
                if key == "sens"
                else t("guides.hud_short", lang)
            )
            ...
            kb.append(
                [
                    InlineKeyboardButton(
                        t("admin.guides.buttons.set_code", lang, section=code_label),
                        callback_data=f"gop_setcode_{key}_{mode}",
                    )
                ]
            )
            if code:
                kb.append(
                    [
                        InlineKeyboardButton(
                            t(
                                "admin.guides.buttons.clear_code",
                                lang,
                                section=code_label,
                            ),
                            callback_data=f"gop_clearcode_{key}_{mode}",
                        )
                    ]
                )
        ...
        kb.append(
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data=f"gmode_{mode}"
                )
            ]
        )
    ...
    async def guide_op_router(self, update: Update, context: CustomContext):
        ...
        if data.startswith("gop_rename_"):
            ...
            labels = {
                "basic": t("guides.basic_short", lang),
                "sens": t("guides.sens_short", lang),
                "hud": t("guides.hud_short", lang),
            }
            ...
            kb = [
                [
                    InlineKeyboardButton(
                        t("admin.guides.buttons.confirm_and_continue", lang),
                        callback_data=f"gop_confirm_media_{key}_{mode}",
                    )
                ],
                [
                    InlineKeyboardButton(
                        t("menu.buttons.back", lang), callback_data=f"gsel_{key}_{mode}"
                    )
                ],
            ]
            await query.edit_message_text(
            ...
            if await self.db.cms.clear_guide_media(key, mode=mode):
                await query.message.reply_text(
                    t("admin.guides.clearmedia.success", lang), parse_mode="HTML"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to delete guides clearmedia confirmation message: {e}"
                )
            ...
            kb = [
                [
                    InlineKeyboardButton(
                        t("admin.guides.buttons.confirm_clear", lang),
                        callback_data=f"gop_clearmedia_confirm_{key}_{mode}",
                    )
                ],
                [
                    InlineKeyboardButton(
                        t("menu.buttons.cancel", lang),
                        callback_data=f"gsel_{key}_{mode}",
                    )
                ],
            ]
            await query.edit_message_text(
            ...
            if await self.db.cms.clear_guide_code(key, mode=mode):
                await query.message.reply_text(
                    t("admin.guides.clearcode.success", lang), parse_mode="HTML"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to delete guides clearcode confirmation message: {e}"
                )
    ...
    async def guide_photo_received(self, update: Update, context: CustomContext):
        ...
        if update.message.photo:
            ...
            await update.message.reply_text(
                t("admin.guides.photo.saved", lang, count=count)
            )
    ...
    async def guide_video_received(self, update: Update, context: CustomContext):
        ...
        if update.message.video:
            ...
            await update.message.reply_text(
                t("admin.guides.video.saved", lang, count=count)
            )
    ...
    async def guide_media_confirmed(
        self, update: Update, context: CustomContext, key: str
    ):
        ...
        text = t(
            "admin.guides.media.saved", lang, photos=len(photos), videos=len(videos)
        )

        await query.message.reply_text(text, parse_mode="HTML")
        logger.info(
            f"Media saved for {key} ({mode}): {len(photos)} photos, {len(videos)} videos"
        )
    ...
    async def guide_code_received(self, update: Update, context: CustomContext):
        ...
        if await self.db.cms.set_guide_code(key, code, mode=mode):
            ...
            await update.message.reply_text(
                t("admin.guides.code.saved", lang, code=code_esc), parse_mode="HTML"
            )
