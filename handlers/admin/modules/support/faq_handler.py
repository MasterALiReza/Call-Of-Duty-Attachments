...
class FAQHandler(BaseAdminHandler):
    ...
    def _get_faq_lang_label(self, ui_lang: str, faq_lang: str) -> str:
        return (
            t("admin.faq.lang.fa", ui_lang)
            if faq_lang == "fa"
            else t("admin.faq.lang.en", ui_lang)
        )
    ...
    async def admin_faqs_menu(self, update: Update, context: CustomContext):
        ...
        text += (
            t(
                "admin.faq.menu.avg_rating",
                ui_lang,
                rating=f"{feedback_stats.get('average_rating', 0):.1f}",
            )
            + "\n"
        )
        text += (
            t(
                "admin.faq.menu.feedback_count",
                ui_lang,
                count=feedback_stats.get("total", 0),
            )
            + "\n"
        )
        lang_label = self._get_faq_lang_label(ui_lang, faq_lang)
        text += (
            t("admin.faq.menu.lang_current", ui_lang, current_lang=lang_label) + "\n"
        )
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.faq.buttons.add", ui_lang), callback_data="adm_faq_add"
                ),
                InlineKeyboardButton(
                    t("admin.faq.buttons.list", ui_lang), callback_data="adm_faq_list"
                ),
            ],
            [
                InlineKeyboardButton(
                ...
                InlineKeyboardButton(
                    t("admin.faq.buttons.feedback", ui_lang),
                    callback_data="adm_feedback",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.faq.lang.fa", ui_lang), callback_data="adm_faq_lang_fa"
                ),
                InlineKeyboardButton(
                    t("admin.faq.lang.en", ui_lang), callback_data="adm_faq_lang_en"
                ),
            ],
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", ui_lang), callback_data="admin_back"
                )
            ],
        ]
    ...
    async def admin_faq_list(self, update: Update, context: CustomContext):
        ...
        if not faqs:
            ...
            keyboard = [
                ...
                [
                    InlineKeyboardButton(
                        t("menu.buttons.back", ui_lang), callback_data="admin_faqs"
                    )
                ],
            ]
        else:
    ...
    async def admin_faq_view(self, update: Update, context: CustomContext):
        ...
        text += (
            t("admin.faq.view.question_label", ui_lang)
            + "\n"
            + faq["question"]
            + "\n\n"
        )
        text += (
            t("admin.faq.view.answer_label", ui_lang) + "\n" + faq["answer"] + "\n\n"
        )
        text += (
            t(
                "admin.faq.view.category",
                ui_lang,
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("common.edit", ui_lang), callback_data=f"adm_faq_edit_{faq_id}"
                ),
                InlineKeyboardButton(
                    t("common.delete", ui_lang), callback_data=f"adm_faq_del_{faq_id}"
                ),
            ],
            [
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", ui_lang), callback_data="adm_faq_list"
                )
            ],
        ]
        await query.edit_message_text(
    ...
    async def admin_feedback_stats(self, update: Update, context: CustomContext):
        ...
        text += (
            t(
                "admin.faq.feedback.avg",
                ui_lang,
                rating=f"{stats.get('average_rating', 0):.2f}",
            )
            + "\n"
        )
        text += (
            t("admin.faq.feedback.total", ui_lang, total=stats.get("total", 0)) + "\n\n"
        )
    ...
    async def admin_faq_add_start(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("menu.buttons.cancel", ui_lang), callback_data="admin_faqs"
                )
            ]
        ]
        await query.edit_message_text(
    ...
    async def admin_faq_question_received(self, update: Update, context: CustomContext):
        ...
        if len(question) < 5:
            await update.message.reply_text(
                t("admin.faq.validation.question_short", ui_lang)
            )
            return ADD_FAQ_QUESTION
    ...
    async def admin_faq_answer_received(self, update: Update, context: CustomContext):
        ...
        if len(answer) < 10:
            await update.message.reply_text(
                t("admin.faq.validation.answer_short", ui_lang)
            )
            return ADD_FAQ_ANSWER
        ...
        if success:
            ...
            keyboard = [
                [
                    ...
                    InlineKeyboardButton(
                        t("admin.faq.buttons.list", ui_lang),
                        callback_data="adm_faq_list",
                    ),
                ],
                [
                    InlineKeyboardButton(
                        t("admin.faq.buttons.stats", ui_lang),
                        callback_data="adm_faq_stats",
                    ),
                    InlineKeyboardButton(
                        t("admin.faq.buttons.feedback", ui_lang),
                        callback_data="adm_feedback",
    ...
    async def admin_faq_edit(self, update: Update, context: CustomContext):
        ...
        text += (
            t("admin.faq.edit.current_question", ui_lang)
            + "\n"
            + faq["question"]
            + "\n\n"
        )
        text += (
            t("admin.faq.edit.current_answer_preview", ui_lang)
            + "\n"
            + faq["answer"][:100]
            + "...\n\n"
        )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.faq.buttons.edit_q", ui_lang), callback_data="edit_faq_q"
                ),
                InlineKeyboardButton(
                    t("admin.faq.buttons.edit_a", ui_lang), callback_data="edit_faq_a"
                ),
            ],
            [
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", ui_lang),
                    callback_data=f"adm_faq_view_{faq_id}",
                )
            ],
        ]
        await query.edit_message_text(
    ...
    async def admin_faq_edit_field_select(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.faq.lang.fa", lang), callback_data="adm_faq_lang_fa"
                ),
                InlineKeyboardButton(
                    t("admin.faq.lang.en", lang), callback_data="adm_faq_lang_en"
                ),
            ],
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data=f"adm_faq_edit_{faq_id}"
                ),
                InlineKeyboardButton(
    ...
    async def admin_faq_edit_question_received(
        ...
        if success:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        t("admin.faq.buttons.view", ui_lang),
                        callback_data=f"adm_faq_view_{faq_id}",
                    )
                ],
                [
                    InlineKeyboardButton(
                        t("admin.faq.buttons.back_to_list", ui_lang),
                        callback_data="adm_faq_list",
                    )
                ],
            ]
    ...
    async def admin_faq_edit_answer_received(
        self, update: Update, context: CustomContext
    ):
