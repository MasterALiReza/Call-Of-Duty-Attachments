...
class TicketHandler(BaseAdminHandler):
    ...
    async def _notify_user_status_change(self, context, ticket, new_status):
        ...
        try:
            ...
            try:
                lang = await self.db.users.get_user_language(ticket["user_id"]) or "fa"
            except Exception:
                lang = "fa"
            # متن وضعیت با i18n
            ...
            message = (
                ...
                + "\n"
                + t(
                    "user.tickets.subject", lang, subject=html_escape(ticket["subject"])
                )
            )

            await context.bot.send_message(
                chat_id=ticket["user_id"], text=message, parse_mode="HTML"
            )
    ...
    async def _show_ticket_detail(
        self, update: Update, context: CustomContext, query, ticket_id: int
    ):
        """نمایش جزئیات تیکت - helper function"""
        ticket = await self.db.support.get_ticket(ticket_id)
        ...
        if ticket.get("assigned_to"):
            text += (
                t("admin.tickets.detail.assigned", lang, admin_id=ticket["assigned_to"])
                + "\n"
            )
        ...
        # نمایش تعداد پاسخ‌ها
        replies = await self.db.support.get_ticket_replies(ticket_id)
        if replies:
            text += "\n" + t(
                "admin.tickets.detail.replies_header", lang, count=len(replies)
            )
            # نمایش آخرین پاسخ
            last_reply = replies[-1]
            reply_type = (
                t("admin.tickets.detail.reply.by_admin", lang)
                if last_reply.get("is_admin")
                else t("admin.tickets.detail.reply.by_user", lang)
            )
            reply_time = (
                last_reply["created_at"].strftime("%Y-%m-%d %H:%M")
                if isinstance(last_reply.get("created_at"), (datetime, date))
                else last_reply.get("created_at", "")[:16]
            )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.tickets.buttons.reply", lang),
                    callback_data=f"adm_reply_{ticket_id}",
                ),
                InlineKeyboardButton(
                    t("admin.tickets.buttons.attach", lang),
                    callback_data=f"adm_attach_{ticket_id}",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.tickets.buttons.change_status", lang),
                    callback_data=f"adm_status_{ticket_id}",
                ),
                InlineKeyboardButton(
                    t("admin.tickets.buttons.change_priority", lang),
                    callback_data=f"adm_priority_{ticket_id}",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.tickets.buttons.assign", lang),
                    callback_data=f"adm_assign_{ticket_id}",
                ),
                InlineKeyboardButton(
                    t("admin.tickets.buttons.close", lang),
                    callback_data=f"adm_close_{ticket_id}",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="admin_tickets"
                )
            ],
        ]
    ...
    async def admin_tickets_menu(self, update: Update, context: CustomContext):
        ...
        if not await self.role_manager.has_permission(
            user_id, Permission.MANAGE_TICKETS
        ):
        ...
        # دریافت آمار
        stats = await self.db.support.get_ticket_stats()
        ...
        text += (
            t(
                "admin.tickets.menu.stats.in_progress",
                lang,
                n=stats.get("in_progress", 0),
            )
            + "\n"
        )
        text += (
            t(
                "admin.tickets.menu.stats.waiting_user",
                lang,
                n=stats.get("waiting_user", 0),
            )
            + "\n"
        )
        text += (
            t("admin.tickets.menu.stats.resolved", lang, n=stats.get("resolved", 0))
            + "\n"
        )
        text += (
            t("admin.tickets.menu.stats.closed", lang, n=stats.get("closed", 0))
            + "\n\n"
        )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.tickets.buttons.new", lang),
                    callback_data="adm_tickets_new",
                ),
                InlineKeyboardButton(
                    t("admin.tickets.buttons.in_progress", lang),
                    callback_data="adm_tickets_progress",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.tickets.buttons.waiting_user", lang),
                    callback_data="adm_tickets_waiting",
                ),
                InlineKeyboardButton(
                    t("admin.tickets.buttons.resolved", lang),
                    callback_data="adm_tickets_resolved",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.tickets.buttons.all", lang),
                    callback_data="adm_tickets_all",
                ),
                InlineKeyboardButton(
                    t("admin.tickets.buttons.search", lang),
                    callback_data="adm_tickets_search",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("admin.tickets.buttons.filter_category", lang),
                    callback_data="adm_tickets_filter_category",
                ),
                InlineKeyboardButton(
                    t("admin.tickets.buttons.mine", lang),
                    callback_data="adm_tickets_mine",
                ),
            ],
        ]
        ...
        if await self.role_manager.has_permission(user_id, Permission.MANAGE_SETTINGS):
            keyboard.append(
                [
                    InlineKeyboardButton(
                        "💬 تماس مستقیم", callback_data="adm_direct_contact"
                    )
                ]
            )
    ...
    async def admin_tickets_list(self, update: Update, context: CustomContext):
        ...
        tickets = await self.db.support.get_all_tickets(status=status)
        ...
        if not tickets:
            ...
            text += (
                t(
                    ...
                    lang,
                    start=pagination["start_idx"] + 1,
                    end=pagination["end_idx"],
            ...
            for ticket in pagination["tickets"]:
                ...
                keyboard.append(
                    [
                        InlineKeyboardButton(
                            button_text, callback_data=f"adm_ticket_{ticket_id}"
                        )
                    ]
                )
            ...
            if pagination["has_prev"]:
                nav_buttons.append(
                    InlineKeyboardButton(
                        t("nav.prev", lang), callback_data=f"ticket_page_{page - 1}"
                    )
                )
            if pagination["has_next"]:
                nav_buttons.append(
                    InlineKeyboardButton(
                        t("nav.next", lang), callback_data=f"ticket_page_{page + 1}"
                    )
                )
    ...
    async def admin_tickets_page_navigation(
    ...
    async def admin_ticket_reply_start(self, update: Update, context: CustomContext):
        ...
        text = (
            t("admin.tickets.reply.title", lang, id=ticket_id)
            + "\n\n"
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("menu.buttons.cancel", lang), callback_data="admin_tickets"
                )
            ]
        ]
    ...
    async def admin_ticket_reply_received(self, update: Update, context: CustomContext):
        ...
        # ذخیره پاسخ
        success = await self.db.support.add_ticket_reply(
            ticket_id, admin_id, reply_text, is_admin=True
        )

        # تغییر وضعیت به "منتظر کاربر"
        if success:
            await self.db.support.update_ticket_status(ticket_id, "waiting_user")
        ...
        if success:
            ...
            try:
                ...
                try:
                    user_lang = (
                        await self.db.users.get_user_language(ticket["user_id"]) or "fa"
                    )
                except Exception:
                    user_lang = "fa"
                await context.bot.send_message(
                    ...
                    text=t(
                        "user.tickets.reply.received",
                        user_lang,
                        id=ticket_id,
                        preview=reply_text[:100],
                    ),
                )
            except Exception as e:
    ...
    async def admin_ticket_change_status(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("ticket.status.in_progress", lang),
                    callback_data=f"adm_setstatus_{ticket_id}_in_progress",
                )
            ],
            [
                InlineKeyboardButton(
                    t("ticket.status.waiting_user", lang),
                    callback_data=f"adm_setstatus_{ticket_id}_waiting_user",
                )
            ],
            [
                InlineKeyboardButton(
                    t("ticket.status.resolved", lang),
                    callback_data=f"adm_setstatus_{ticket_id}_resolved",
                )
            ],
            [
                InlineKeyboardButton(
                    t("ticket.status.closed", lang),
                    callback_data=f"adm_setstatus_{ticket_id}_closed",
                )
            ],
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang),
                    callback_data=f"adm_ticket_{ticket_id}",
                )
            ],
        ]
    ...
    async def admin_ticket_set_status(self, update: Update, context: CustomContext):
        ...
        success = await self.db.support.update_ticket_status(ticket_id, new_status)
    ...
    async def admin_ticket_close(self, update: Update, context: CustomContext):
        ...
        success = await self.db.support.update_ticket_status(ticket_id, "closed")
    ...
    async def admin_ticket_search_start(self, update: Update, context: CustomContext):
        ...
        lang = await get_user_lang(update, context, self.db) or "fa"
        text = (
            t("admin.tickets.search.title", lang)
            + "\n\n"
    ...
    async def admin_ticket_search_received(
        self, update: Update, context: CustomContext
    ):
        ...
        # جستجو در تیکت‌ها
        tickets = await self.db.support.search_tickets(search_query)

        text = (
            t("admin.tickets.search.results", lang, query=html_escape(search_query))
            + "\n\n"
        )
    ...
    async def admin_ticket_view_attachments(
        self, update: Update, context: CustomContext
    ):
        ...
        if ticket.get("attachments"):
            if isinstance(ticket["attachments"], list):
                ...
                logger.warning(
                    f"Ticket {ticket_id} attachments is not a list: {type(ticket['attachments'])}"
                )
        ...
        for reply in replies:
            if reply.get("attachments"):
                if isinstance(reply["attachments"], list):
                    ...
                    logger.warning(
                        f"Reply attachments is not a list: {type(reply['attachments'])}"
                    )
        ...
        await context.bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            t("admin.tickets.buttons.back_to_ticket", lang),
                            callback_data=f"adm_ticket_{ticket_id}",
                        )
                    ]
                ]
    ...
    async def admin_ticket_change_priority(
        self, update: Update, context: CustomContext
    ):
        """تغییر اولویت تیکت"""
        query = update.callback_query
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("ticket.priority.low", lang),
                    callback_data=f"adm_setpriority_{ticket_id}_low",
                )
            ],
            [
                InlineKeyboardButton(
                    t("ticket.priority.medium", lang),
                    callback_data=f"adm_setpriority_{ticket_id}_medium",
                )
            ],
            [
                InlineKeyboardButton(
                    t("ticket.priority.high", lang),
                    callback_data=f"adm_setpriority_{ticket_id}_high",
                )
            ],
            [
                InlineKeyboardButton(
                    t("ticket.priority.critical", lang),
                    callback_data=f"adm_setpriority_{ticket_id}_critical",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def admin_ticket_set_priority(self, update: Update, context: CustomContext):
        ...
        if success:
            lang = await get_user_lang(update, context, self.db) or "fa"
            await query.answer(
                t("admin.tickets.priority.changed", lang), show_alert=True
            )
    ...
    async def admin_ticket_assign_start(self, update: Update, context: CustomContext):
        ...
        for admin in admins[:10]:  # نمایش 10 ادمین اول
            ...
            display_name = (
                admin.get("display_name") or admin.get("username") or f"User {admin_id}"
            )
            keyboard.append(
                [
                    InlineKeyboardButton(
    ...
    async def admin_ticket_assign_confirm(self, update: Update, context: CustomContext):
        ...
        success = await self.db.support.assign_ticket(ticket_id, admin_id)
    ...
    async def admin_tickets_filter_category(
        self, update: Update, context: CustomContext
    ):
        ...
        for cat_key in categories:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t(f"admin.tickets.category.{cat_key}", lang),
                        callback_data=f"adm_tickets_cat_{cat_key}",
                    )
                ]
            )
    ...
    async def admin_tickets_by_category(self, update: Update, context: CustomContext):
        ...
        all_tickets = await self.db.support.get_all_tickets()
    ...
    async def admin_tickets_mine(self, update: Update, context: CustomContext):
        ...
        tickets = await self.db.support.get_all_tickets(assigned_to=admin_id)
