...
class DirectContactHandler(BaseAdminHandler):
    ...
    async def admin_direct_contact_menu(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("admin.direct.buttons.change_name", lang),
                    callback_data="dc_change_name",
                ),
                InlineKeyboardButton(
                    t("admin.direct.buttons.change_link", lang),
                    callback_data="dc_change_link",
                ),
            ],
        ]
        ...
        if enabled.lower() == "true":
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t("admin.direct.buttons.disable", lang),
                        callback_data="dc_disable",
                    )
                ]
            )
        else:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t("admin.direct.buttons.enable", lang),
                        callback_data="dc_enable",
                    )
                ]
            )
    ...
    async def direct_contact_toggle_status(
        ...
        if success:
            status_msg = (
                t("admin.direct.toggled.enabled", lang)
                if action == "enable"
                else t("admin.direct.toggled.disabled", lang)
            )
    ...
    async def direct_contact_edit_name_start(
        self, update: Update, context: CustomContext
    ):
        """
        شروع تغییر نام دکمه
        ...
        lang = await get_user_lang(update, context, self.db) or "fa"
        current_name = await self.db.settings.get_setting(
            "direct_contact_name", "💬 تماس مستقیم"
        )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("menu.buttons.cancel", lang), callback_data="adm_direct_contact"
                )
            ]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    async def direct_contact_name_received(
        self, update: Update, context: CustomContext
    ):
        """
        ...
        if success:
            await update.message.reply_text(
                t("admin.direct.name.updated", lang, new=new_name),
                parse_mode="Markdown",
            )
    ...
    async def direct_contact_edit_link_start(
        self, update: Update, context: CustomContext
    ):
        """
        ...
        lang = await get_user_lang(update, context, self.db) or "fa"
        current_link = await self.db.settings.get_setting(
            "direct_contact_link", "https://t.me/YourSupportChannel"
        )
    ...
    async def direct_contact_link_received(
        self, update: Update, context: CustomContext
    ):
        """
        ...
        if not new_link.startswith("https://t.me/"):
            await update.message.reply_text(
                t("admin.direct.link.must_start_tme", lang), parse_mode="Markdown"
            )
            return DIRECT_CONTACT_LINK
        ...
        if success:
            await update.message.reply_text(
                t("admin.direct.link.updated", lang, new=new_link),
                parse_mode="Markdown",
            )
