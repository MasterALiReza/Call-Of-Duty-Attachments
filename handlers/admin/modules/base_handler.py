...
class BaseAdminHandler:
    ...
    async def send_permission_denied(
        ...
        *,
        ...
        message = (
            t("admin.permission.denied.title", lang)
            + "\n\n"
    ...
    def _make_weapon_keyboard(
        self, weapons: List[str], prefix: str, category: str = None
    ...
    def _make_mode_selection_keyboard(
        self, prefix: str, lang: str = "fa", allowed_modes: List[str] = None
    ) -> List[List[InlineKeyboardButton]]:
        """
        ...
        if allowed_modes is None or "br" in allowed_modes:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t("mode.br_btn", lang), callback_data=f"{prefix}br"
                    )
                ]
            )
        if allowed_modes is None or "mp" in allowed_modes:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t("mode.mp_btn", lang), callback_data=f"{prefix}mp"
                    )
                ]
            )
        return keyboard
    ...
    async def _get_admin_main_keyboard(
        self, user_id: int, lang: str = "fa"
    ) -> List[List[InlineKeyboardButton]]:
        """
        ...
        if is_super:
            keyboard = [
                ...
                [
                    InlineKeyboardButton(
                        t("admin.buttons.manage_users", lang),
                        callback_data="admin_users",
                    ),
                    InlineKeyboardButton(
                        t("admin.buttons.manage_admins", lang),
                ...
                [
                    InlineKeyboardButton(
                        t("admin.buttons.notify_send", lang),
                        callback_data="admin_notify",
                    ),
                    InlineKeyboardButton(
                        t("admin.buttons.notify_settings", lang),
                        callback_data="admin_notify_settings",
                    ),
                ],
                # ─── بخش پشتیبانی و بازخورد ───
                [
                    InlineKeyboardButton(
                        t("admin.buttons.tickets", lang), callback_data="admin_tickets"
                    ),
                    InlineKeyboardButton(
                        t("admin.buttons.feedback_dashboard", lang),
                        callback_data="fb_dashboard",
                    ),
                ],
                # ─── بخش محتوا و راهنما ───
                [
                    InlineKeyboardButton(
                        t("admin.buttons.cms", lang), callback_data="admin_cms"
                    ),
                    InlineKeyboardButton(
                        t("admin.buttons.faq", lang), callback_data="admin_faqs"
                    ),
                ],
                # ─── بخش فنی و سیستم ───
                [
                    InlineKeyboardButton(
                        t("admin.buttons.analytics", lang),
                        callback_data="attachment_analytics",
                    ),
                    InlineKeyboardButton(
                        t("admin.buttons.data_health", lang),
                        callback_data="data_health",
                    ),
                ],
                # ─── بخش زیرساخت و داده ───
                [
                    InlineKeyboardButton(
                        t("admin.buttons.manage_channels", lang),
                        callback_data="channel_management",
                    ),
                    InlineKeyboardButton(
                        t("admin.buttons.data_mgmt", lang),
                        callback_data="admin_data_management",
                    ),
                ],
                # ─── تنظیمات ربات ───
                [
                    InlineKeyboardButton(
                        t("admin.buttons.edit_texts", lang), callback_data="admin_texts"
                    ),
                    InlineKeyboardButton(
                        t("admin.buttons.game_settings", lang),
                        callback_data="admin_guides",
                    ),
                ],
                [
                    InlineKeyboardButton(
                ...
                [
                    InlineKeyboardButton(
                        t("admin.buttons.exit", lang), callback_data="admin_exit"
                    )
                ],
            ]
            return keyboard
        ...
        if Permission.MANAGE_USERS in user_permissions:
            user_row.append(
                InlineKeyboardButton(
                    t("admin.buttons.manage_users", lang), callback_data="admin_users"
                )
            )
        ...
        if Permission.SEND_NOTIFICATIONS in user_permissions:
            notify_row.append(
                InlineKeyboardButton(
                    t("admin.buttons.notify_send", lang), callback_data="admin_notify"
                )
            )
        ...
        if Permission.VIEW_ANALYTICS in user_permissions:
            ...
            analytics_row.append(
                InlineKeyboardButton(
                    t("admin.buttons.data_health", lang), callback_data="data_health"
                )
            )
        if analytics_row:
        ...
        if (
            Permission.IMPORT_EXPORT in user_permissions
            or Permission.BACKUP_DATA in user_permissions
        ):
            infra_row.append(
                InlineKeyboardButton(
                    t("admin.buttons.data_mgmt", lang),
        ...
        if Permission.MANAGE_SETTINGS in user_permissions:  # تنظیمات بازی/راهنما
            settings_row.append(
                InlineKeyboardButton(
                    t("admin.buttons.game_settings", lang), callback_data="admin_guides"
                )
            )
        if settings_row:
        ...
        if (
            ...
            or Permission.MANAGE_USER_ATTACHMENTS in user_permissions
            or Permission.MANAGE_SUGGESTED_ATTACHMENTS in user_permissions
            or Permission.MANAGE_CATEGORIES in user_permissions
        ):
            keyboard.append(
                [
                    InlineKeyboardButton(
    ...
    async def data_management_menu(self, update: Update, context: CustomContext):
        ...
        if Permission.IMPORT_EXPORT in user_permissions:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t("admin.data_mgmt.import", lang), callback_data="admin_import"
                    ),
                    InlineKeyboardButton(
                        t("admin.data_mgmt.export", lang), callback_data="admin_export"
                    ),
                ]
            )
        ...
        if Permission.BACKUP_DATA in user_permissions:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t("admin.data_mgmt.backup", lang), callback_data="admin_backup"
                    )
                ]
            )
