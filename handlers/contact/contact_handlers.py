...
class ContactHandlers:
    ...
    async def handle_invalid_input(self, update: Update, context: CustomContext):
        ...
        logger.info(
            f"DEBUG: handle_invalid_input called by {user_id}. Data: {update.message.text if update.message else 'None'}"
        )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("contact.menu.new_ticket", lang),
                    callback_data="contact_new_ticket",
                ),
                InlineKeyboardButton(
                    t("contact.menu.my_tickets", lang),
                    callback_data="contact_my_tickets",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("contact.menu.faq", lang), callback_data="contact_faq"
                ),
                InlineKeyboardButton(
                    t("contact.menu.feedback", lang), callback_data="contact_feedback"
                ),
            ],
        ]

        # اضافه کردن دکمه تماس مستقیم اگر فعال باشد
        direct_contact_enabled = await self.db.settings.get_setting(
            "direct_contact_enabled", "true"
        )
        if direct_contact_enabled.lower() == "true":
            contact_link = await self.db.settings.get_setting(
                "direct_contact_link", "https://t.me/YourSupportChannel"
            )
            contact_name = await self.db.settings.get_setting(
                "direct_contact_name", "💬 تماس مستقیم"
            )
    ...
    async def new_ticket_start(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("contact.ticket.category.bug", lang), callback_data="tc_bug"
                ),
                InlineKeyboardButton(
                    t("contact.ticket.category.feature_request", lang),
                    callback_data="tc_feature_request",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("contact.ticket.category.question", lang),
                    callback_data="tc_question",
                ),
                InlineKeyboardButton(
                    t("contact.ticket.category.content_issue", lang),
                    callback_data="tc_content_issue",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("contact.ticket.category.channel_issue", lang),
                    callback_data="tc_channel_issue",
                ),
                InlineKeyboardButton(
                    t("contact.ticket.category.other", lang), callback_data="tc_other"
                ),
            ],
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="contact_menu"
                )
            ],
        ]

        text = (
            t("contact.ticket.new.title", lang)
            + "\n\n"
    ...
    async def ticket_category_selected(self, update: Update, context: CustomContext):
        """دریافت دسته تیکت"""
        logger.info(
            f"DEBUG: ticket_category_selected called by {update.effective_user.id} with data {update.callback_query.data}"
        )
        query = update.callback_query
        await query.answer()
        lang = await get_user_lang(update, context, self.db) or "fa"
        ...
        text = (
            ...
            + "\n\n"
            + t(
                "contact.ticket.subject.selected_category", lang, category=category_name
            )
            + "\n\n"
    ...
    async def ticket_subject_received(self, update: Update, context: CustomContext):
        ...
        logger.info(
            f"DEBUG: ticket_subject_received called by {update.effective_user.id} with subject: {subject}"
        )
        lang = await get_user_lang(update, context, self.db) or "fa"

        if len(subject) < 5:
            await update.message.reply_text(
                t("contact.ticket.subject.too_short", lang, n=5)
            )
            return TICKET_SUBJECT

        if len(subject) > 200:
            await update.message.reply_text(
                t("contact.ticket.subject.too_long", lang, n=200)
            )
            return TICKET_SUBJECT
        ...
        suggested_faqs = await self.contact_system.get_suggested_faqs(
            subject, limit=3, lang=lang
        )
        ...
        if suggested_faqs:
            ...
            for i, faq in enumerate(suggested_faqs, 1):
                ...
                keyboard.append(
                    [
                        InlineKeyboardButton(
                            t("contact.faq.view_answer", lang, i=i),
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        t("nav.next", lang), callback_data="ticket_continue"
                    )
                ]
            )
            keyboard.append(
            ...
            await update.message.reply_text(
                t("contact.ticket.description.prompt", lang), parse_mode="Markdown"
            )
            return TICKET_DESCRIPTION
    ...
    async def ticket_description_received(self, update: Update, context: CustomContext):
        ...
        logger.info(
            f"DEBUG: ticket_description_received called by {update.effective_user.id} with description length: {len(description)}"
        )
        lang = await get_user_lang(update, context, self.db) or "fa"

        if len(description) < 10:
            await update.message.reply_text(
                t("contact.ticket.description.too_short", lang, n=10)
            )
            return TICKET_DESCRIPTION
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("contact.attachment.add_image", lang),
                    callback_data="ticket_add_image",
                )
            ],
            [
                InlineKeyboardButton(
                    t("contact.submit_without_image", lang),
                    callback_data="ticket_submit",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def ticket_add_image_request(self, update: Update, context: CustomContext):
        ...
        lang = await get_user_lang(update, context, self.db) or "fa"
        await query.edit_message_text(
            t("contact.image.send", lang), parse_mode="Markdown"
        )
        return TICKET_ATTACHMENT
    ...
    async def ticket_image_received(self, update: Update, context: CustomContext):
        ...
        if not update.message.photo:
            await update.message.reply_text(
                t("contact.validation.image_required", lang)
            )
            return TICKET_ATTACHMENT
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("contact.image.add_more", lang), callback_data="ticket_add_image"
                )
            ],
            [
                InlineKeyboardButton(
                    t("contact.submit.confirm", lang), callback_data="ticket_submit"
                )
            ],
            [
                InlineKeyboardButton(
    ...
    async def ticket_submit(self, update: Update, context: CustomContext):
        ...
        if ticket_id:
            ...
            await self._notify_admins_new_ticket(
                context, ticket_id, category, subject, user_id
            )
    ...
    async def my_tickets(self, update: Update, context: CustomContext):
        ...
        if not tickets:
            text = (
                t("contact.my_tickets.title", lang)
                + "\n\n"
    ...
    async def view_ticket(self, update: Update, context: CustomContext):
        ...
        category_name = escape_markdown(
            ContactSystem.format_category_name(ticket["category"])
        )
        status_name = escape_markdown(
            ContactSystem.format_status_name(ticket["status"])
        )
        priority_name = escape_markdown(
            ContactSystem.format_priority_name(ticket["priority"])
        )
...
"""
        ...
        for reply in replies[-5:]:  # آخرین 5 پاسخ
            ...
            reply_time = (
                ...
                if hasattr(reply["created_at"], "strftime")
                else str(reply["created_at"])[:16]
            )
            text += f"\n{sender} | {reply_time}\n{message_safe}\n"
        ...
        if ticket["status"] not in ["closed", "resolved"]:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        "💬 پاسخ دادن", callback_data=f"ticket_reply_{ticket_id}"
                    )
                ]
            )

        keyboard.append(
            [InlineKeyboardButton("⬅️ بازگشت", callback_data="contact_my_tickets")]
        )
    ...
    async def faq_menu(self, update: Update, context: CustomContext):
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("contact.faq.buttons.search", lang), callback_data="faq_search"
                )
            ]
        )
        keyboard.append(
    ...
    async def faq_view(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("contact.faq.buttons.helpful", lang),
                    callback_data=f"faq_helpful_{faq_id}",
                ),
                InlineKeyboardButton(
                    t("contact.faq.buttons.not_helpful", lang),
                    callback_data=f"faq_not_helpful_{faq_id}",
                ),
            ],
            [
                InlineKeyboardButton(
                    t("menu.buttons.back", lang), callback_data="contact_faq"
                )
            ],
        ]
    ...
    async def feedback_rating_selected(self, update: Update, context: CustomContext):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    t("contact.feedback.submit_no_comment", lang),
                    callback_data="feedback_submit_no_comment",
                )
            ]
        ]
    ...
    async def _submit_feedback(self, update: Update, context: CustomContext):
        ...
        success = await self.contact_system.submit_feedback(
            user_id=user_id, rating=rating, category="general", message=message
        )
    ...
    async def _notify_admins_new_ticket(
        self, context, ticket_id: int, category: str, subject: str, user_id: int
    ):
...
"""
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "🔍 مشاهده تیکت", callback_data=f"adm_ticket_{ticket_id}"
                    )
                ]
            ]
            ...
            for admin in admins:
                ...
                # بررسی دسترسی
                if await role_manager.has_permission(
                    admin_id, Permission.MANAGE_TICKETS
                ):
                    try:
                        ...
                        logger.error(
                            f"Error sending notification to admin {admin_id}: {e}"
                        )
