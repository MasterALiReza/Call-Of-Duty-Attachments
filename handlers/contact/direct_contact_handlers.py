...
این قسمت به کاربران امکان دسترسی مستقیم به کانال/اکانت پشتیبانی شما را می‌دهد."""
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "📝 تغییر نام دکمه", callback_data="dc_change_name"
                ),
        ...
        if enabled.lower() == "true":
            keyboard.append(
                [InlineKeyboardButton("🔴 غیرفعال کردن", callback_data="dc_disable")]
            )
        else:
            keyboard.append(
                [InlineKeyboardButton("🟢 فعال کردن", callback_data="dc_enable")]
            )
    ...
    async def direct_contact_change_name_start(
...
**نکته:** می‌تونید از emoji استفاده کنید (مثل 💬 یا 📞)"""
        ...
        keyboard = [
            [InlineKeyboardButton("❌ لغو", callback_data="adm_direct_contact")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    async def direct_contact_name_received(
        ...
        if success:
            await update.message.reply_text(
                f"✅ نام دکمه به `{new_name}` تغییر یافت.", parse_mode="Markdown"
            )
        else:
    ...
    async def direct_contact_change_link_start(
        self, update: Update, context: CustomContext
    ):
        """شروع تغییر لینک"""
        query = update.callback_query
...
**نکته:** لینک باید با https://t.me/ شروع شود."""
    ...
    async def direct_contact_link_received(
        ...
        if success:
            await update.message.reply_text(
                f"✅ لینک تماس مستقیم به `{new_link}` تغییر یافت.",
                parse_mode="Markdown",
            )
        else:
