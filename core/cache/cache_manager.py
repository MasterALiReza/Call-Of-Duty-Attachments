...
class RedisCacheManager:
    ...
    - L1 in-memory synchronization with L2 Redis cache
    """
            ...
            try:
                ...
                if data:
                    ...
                    try:
                        ...
                        value = data
                    # Update L1 memory cache on Redis hit
                    with self._lock:
                        self._cache[key] = CacheEntry(value, DEFAULT_CACHE_TTL)
                    return value
            except Exception as e:
    ...
    async def set(self, key: str, value: Any, ttl: int = DEFAULT_CACHE_TTL):
        ...
        if self.use_redis and self.redis_client:
            try:
                data = json.dumps(cache_value, default=str)
...
async def warm_cache(
    data_loader: Callable, keys: list, ttl: int = DEFAULT_CACHE_TTL
) -> int:
    """
...
def get_cache_stats() -> Dict[str, Any]:
    ...
    with cache._lock:
        return {
            ...
            "utilization": len(cache._cache) / cache.max_size
            if cache.max_size > 0
            else 0,
        }
...
def cached(
    ttl_or_key=300, key_func: Optional[Callable] = None, ttl: Optional[int] = None
):
    """
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            args_suffix = (
                f":{'_'.join((str(a) for a in args))}:{'_'.join((f'{k}={v}' for k, v in sorted(kwargs.items())))}"
                if (args or kwargs)
                else ""
            )
            if cache_key_prefix:
                cache_key = f"{cache_key_prefix}{args_suffix}"
            elif key_func:
                cache_key = key_func(*args, **kwargs)
            else:
                cache_key = f"{func.__qualname__}{args_suffix}"
