...
class UACache:
        ...
        try:
            ...
            if not force_refresh:
                try:
                    async with self.db.get_connection() as conn:
                        async with conn.cursor() as cursor:
                            try:
                                ...
                                logger.debug(
                                    f"Cache table might not have updated_at column or table missing: {e}"
                                )
                                await cursor.execute(
                                    "\n                                    SELECT * FROM ua_stats_cache \n                                    WHERE id = 1\n                                    "
                                )
                    ...
                    logger.debug(
                        f"Skipping DB cache for stats (will compute fresh): {cache_err}"
                    )
                    cache_row = None
                if cache_row:
                        ...
                        async with tconn.cursor() as tcur:
                            await tcur.execute(
                                ...
                                (
                                    ...
                                    stats["last_week_submissions"],
                                    stats["last_week_approvals"],
                                ),
                            )
                except Exception as e:
    ...
    async def get_top_weapons(
        ...
        if not force_refresh and cache_key in self.memory_cache:
            ...
            if self._is_cache_valid(cached.get("timestamp")):
                logger.debug("Top weapons retrieved from memory cache")
                return cached["data"]
        try:
            if not hasattr(self.db, "get_connection"):
                return []
    ...
    async def get_top_users(
        self, limit: int = 5, force_refresh: bool = False
    ) -> List[Dict]:
    ...
    async def invalidate(self, cache_type: Optional[str] = None):
        ...
        with self.lock:
            if cache_type:
                keys_to_delete = [
                    k for k in self.memory_cache if k.startswith(cache_type)
                ]
                for key in keys_to_delete:
                    del self.memory_cache[key]
                logger.info(
                    f"Invalidated {len(keys_to_delete)} {cache_type} cache entries"
                )
            else:
    ...
    async def batch_get_users(self, user_ids: List[int]) -> Dict[int, Dict]:
        ...
        if cache_key in self.memory_cache:
            cached = self.memory_cache[cache_key]
            if self._is_cache_valid(cached.get("timestamp")):
                logger.debug("Batch users retrieved from cache")
                return cached["data"]
        try:
            if not hasattr(self.db, "get_connection"):
                return {}
