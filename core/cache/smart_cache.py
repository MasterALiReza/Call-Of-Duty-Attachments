...
class SmartCacheManager:
    ...
    def _make_key(
        self, func_name: str, args: tuple, kwargs: dict, data_type: str = None
    ) -> str:
        """
        ...
        with self._lock:
            if key in self._cache:
                ...
                if time.time() < entry["expires_at"]:
                    ...
                    logger.debug(
                        f"Cache HIT: {key[:8]}... (TTL remaining: {entry['expires_at'] - time.time():.1f}s)"
                    )
                    return entry["value"]
                else:
                    # Expired - remove it
    ...
    def invalidate_pattern(self, pattern: str):
        ...
        with self._lock:
            ...
            if keys_to_delete:
                logger.info(
                    f"Cache INVALIDATE: {len(keys_to_delete)} keys with pattern '{pattern}'"
                )
