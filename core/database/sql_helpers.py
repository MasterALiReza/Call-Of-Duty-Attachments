...
def sanitize_identifier(identifier: str) -> str:
    """
    Sanitize and validate SQL identifier (table name, column name) against SQL injection.
    Only alphanumeric characters, underscores, and dots (schema/table qualification) are permitted.
    Length is limited to 63 characters (PostgreSQL standard limit).
    """
    ...
    if len(identifier) > 63:
        raise ValueError(
            f"Invalid SQL identifier: {identifier} (exceeds 63 characters)"
        )
    return identifier
...
def build_upsert_query(
    ...
    columns = [sanitize_identifier(col) for col in columns]
    conflict_columns = [sanitize_identifier(col) for col in conflict_columns]
    placeholder = "%s"
...
def build_insert_ignore_query(table: str, columns: list) -> str:
    """
...
def build_fts_where_clause(query: str, search_columns: list) -> str:
    """
    ...
    Returns:
        WHERE clause string
    """
    search_columns = [sanitize_identifier(col) for col in search_columns]
    conditions = " OR ".join([f"{col} %% %s" for col in search_columns])
    return conditions


def build_fts_order_clause(query: str, primary_column: str) -> str:
    """
    ...
    primary_column = sanitize_identifier(primary_column)
    return f"similarity({primary_column}, %s) DESC"


def get_fts_params(query: str, search_columns: list) -> tuple:
    """
...
def build_fts_query(
    query: str, search_columns: list, primary_column: str = None
) -> tuple:
    """
