...
class AnalyticsRepository(BaseRepository):
    ...
    async def vote_attachment(
        self, user_id: int, attachment_id: int, vote: int
    ) -> Dict:
        """
        ...
        try:
            async with self.transaction() as conn:
                ...
                logger.info(
                    f"✅ Vote (atomic): user={user_id}, att={attachment_id}, {previous_vote}→{new_rating}, action={action}"
                )
                return {
                    "success": True,
                    "action": action,
    ...
    async def get_user_vote(self, attachment_id: int, user_id: int) -> Optional[int]:
        ...
        try:
            ...
            result = await self.execute_query(
                query, (user_id, attachment_id), fetch_one=True
            )
            ...
            async with self.transaction() as conn:
                async with conn.cursor() as cursor:
                    ...
                    await self.track_event(
                        user_id, "view", {"attachment_id": attachment_id}
                    )
                return True
        except Exception as e:
            log_exception(
                logger, e, f"track_attachment_view({user_id}, {attachment_id})"
            )
            return False
        ...
        try:
            async with self.transaction() as conn:
                async with conn.cursor() as cursor:
                    ...
                    # Track in unified events as well
                    await self.track_event(
                        user_id, "copy", {"attachment_id": attachment_id}
                    )
                return True
        except Exception as e:
            log_exception(
                logger, e, f"track_attachment_copy({user_id}, {attachment_id})"
            )
            return False

    async def get_popular_attachments(
        self,
        category: str = None,
        weapon: str = None,
        mode: str = None,
        ...
        try:
            ...
            where_clauses.append(
                "uae.last_view_date >= NOW() - make_interval(days => %s)"
            )
            params.append(days)
    ...
    async def get_attachment_stats(
        self, attachment_id: int, period: str = "all"
    ) -> Dict:
        ...
        try:
            ...
            vote_stats = (
                await self.execute_query(query_votes, (attachment_id,), fetch_one=True)
                or {}
            )
            ...
            return {
                ...
                "total_views": vote_stats.get("total_views", 0) or 0,
                "total_clicks": vote_stats.get("total_clicks", 0) or 0,
                "unique_users": vote_stats.get("unique_users", 0) or 0,
                "period": period,
            }
        except Exception as e:
    ...
    async def search_attachments_like(self, query: str, limit: int = 30) -> List[Dict]:
        ...
        try:
            normalized_query = (
                "%" + "".join((c.lower() for c in query if c.isalnum())) + "%"
            )
    ...
    async def track_search(
        self, user_id: int, query: str, results_count: int, execution_time_ms: float
    ) -> bool:
    ...
    async def get_search_analytics(self, days: int = 30) -> Dict:
        ...
        try:
            ...
            failed_queries = await self.execute_query(
                query_failed, (days,), fetch_all=True
            )
            ...
            return {
                "total_searches": stats.get("total_searches", 0) or 0,
                "unique_users": stats.get("unique_users", 0) or 0,
                "avg_results": round(stats.get("avg_results", 0) or 0, 1),
                "avg_time_ms": round(stats.get("avg_time_ms", 0) or 0, 1),
                "zero_results": zero_results,
                "zero_rate": round(zero_rate, 1),
                "top_queries": [
                    {"query": q["query"], "count": q["count"]} for q in top_queries
                ],
                "failed_queries": [
    ...
    async def get_statistics(self) -> Dict:
        ...
        try:
            ...
            stats["total_weapons"] = (
                (
                    await self.execute_query(
                        "SELECT COUNT(*) as count FROM weapons", fetch_one=True
                    )
                )
                or {}
            ).get("count", 0)
            stats["total_attachments"] = (
                (
                    await self.execute_query(
                        "SELECT COUNT(*) as count FROM attachments", fetch_one=True
                    )
                )
                or {}
            ).get("count", 0)
            stats["total_attachments_br"] = (
                (
                    await self.execute_query(
                        "SELECT COUNT(*) as count FROM attachments WHERE mode = 'br'",
                        fetch_one=True,
                    )
                )
                or {}
            ).get("count", 0)
            stats["total_attachments_mp"] = (
                (
                    await self.execute_query(
                        "SELECT COUNT(*) as count FROM attachments WHERE mode = 'mp'",
                        fetch_one=True,
                    )
                )
                or {}
            ).get("count", 0)
            stats["total_top_attachments"] = (
                (
                    await self.execute_query(
                        "SELECT COUNT(*) as count FROM attachments WHERE is_top = TRUE",
                        fetch_one=True,
                    )
                )
                or {}
            ).get("count", 0)
            stats["total_season_attachments"] = (
                (
                    await self.execute_query(
                        "SELECT COUNT(*) as count FROM attachments WHERE is_season_top = TRUE",
                        fetch_one=True,
                    )
                )
                or {}
    ...
    async def submit_attachment_feedback(
        self, user_id: int, attachment_id: int, feedback_text: str
    ) -> bool:
        ...
        try:
            ...
            if not feedback_text:
                logger.warning(
                    f"Empty feedback rejected: user={user_id}, att={attachment_id}"
                )
                return False
            async with self.transaction() as conn:
                async with conn.cursor() as cursor:
            ...
            log_exception(
                logger, e, f"submit_attachment_feedback({user_id}, {attachment_id})"
            )
            return False
    ...
    async def track_event(
        self, user_id: int, event_type: str = "general", metadata: Dict = None, **kwargs
    ) -> bool:
        ...
        try:
            async with self.transaction() as conn:
                async with conn.cursor() as cursor:
                    ...
                    await cursor.execute(
                        "UPDATE analytics_channels SET total_joins = total_joins + 1 WHERE channel_id = %s",
                        (channel_id,),
                    )
                    ...
                    await self.track_event(
                        user_id, "join_success", {"channel_id": channel_id}
                    )
                    return True
        except Exception as e:
        ...
        try:
            async with self.transaction() as conn:
                async with conn.cursor() as cursor:
                    ...
                    await self.track_event(
                        user_id, "join_attempt", {"channel_id": channel_id}
                    )
                    return True
        except Exception as e:
            ...
            async with self.transaction() as conn:
                async with conn.cursor() as cursor:
                    ...
                    return {
                        ...
                        "engagement_rate": (
                            core.get("clicks", 0) / core.get("views", 1) * 100
                        )
                        if core.get("views", 0) > 0
                        else 0,
                        "top_performer": {"name": top["name"], "views": top["views"]}
                        if top
                        else None,
                        "most_engaging": {
                            "name": engaging["name"],
    ...
    async def get_cohort_retention(self, weeks: int = 4) -> List[Dict]:
        ...
        try:
            ...
            return await self.execute_query(
                query, (weeks + 8, weeks + 8), fetch_all=True
            )
        except Exception as e:
                ...
                funnel.append(
                    {
                        "step": step_name,
                        "count": count,
                        "conversion_rate": round(count / max_val * 100, 1)
                        if max_val > 0
                        else 0,
                    }
                )
    ...
    async def get_attachment_search_results(
        ...
        try:
            ...
            """
            return await self.execute_query(
                query, (pattern, pattern, pattern, limit), fetch_all=True
            )
        except Exception as e:
    ...
    async def get_attachment_detailed_stats(self, attachment_id: int) -> Dict:
        ...
        try:
            ...
            meta_query = """
                ...
                WHERE a.id = %s
            """
            meta = await self.execute_query(
                meta_query, (attachment_id,), fetch_one=True
            )
            if not meta:
                return {}
            ...
            """
            summary = await self.execute_query(
                summary_query, (attachment_id,), fetch_one=True
            )
            ...
            breakdown_query = """
                ...
                ORDER BY date ASC
            """
            breakdown = await self.execute_query(
                breakdown_query, (attachment_id,), fetch_all=True
            )

            return {"attachment": meta, "summary_30d": summary, "daily_7d": breakdown}
        except Exception as e:
    ...
    async def get_attachment_feedback_stats(
        self, suggested_only: bool = False, days: int = 30
    ) -> Dict:
    ...
    async def get_attachment_feedback_list(
        self, page: int = 1, per_page: int = 5, suggested_only: bool = False
    ) -> Dict:
        ...
        try:
            ...
            feedbacks = await self.execute_query(
                list_sql, (per_page, offset), fetch_all=True
            )
    ...
    async def get_attachment_stats_by_category(
        self, suggested_only: bool = False, days: int = 30
    ) -> List[Dict]:
    ...
    async def get_attachment_stats_by_mode(
        self, suggested_only: bool = False, days: int = 30
    ) -> List[Dict]:
    ...
    async def get_attachment_weekly_trend(
        self, suggested_only: bool = False, weeks: int = 8
    ) -> List[Dict]:
    ...
    async def get_user_behavior_analytics(self, days: int = 7) -> Dict:
        ...
        try:
            ...
            user_stats = await self.execute_query(
                per_user_query, (days,), fetch_all=True
            )
            ...
            top_reach_query = """
                ...
                LIMIT 5
            """
            top_reach = await self.execute_query(
                top_reach_query, (days,), fetch_all=True
            )
    ...
    async def get_weapon_category_stats(
        self, category_id: int, mode: str = None
    ) -> Dict:
        ...
        try:
            ...
            return cast(
                Dict[str, Any],
                (await self.execute_query(query, tuple(params), fetch_one=True)) or {},
            )
        except Exception as e:
    ...
    async def get_weapon_categories(self) -> List[Dict]:
        ...
        try:
            return await self.execute_query(
                "SELECT id, name FROM weapon_categories ORDER BY name", fetch_all=True
            )
        except Exception as e:
