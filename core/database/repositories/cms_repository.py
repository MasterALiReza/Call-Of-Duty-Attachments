...
class CMSRepository(BaseRepository):
    ...
    async def update_required_channel(
        self, channel_id: str, new_title: str = None, new_url: str = None
    ) -> bool:
        """ویرایش کانال اجباری"""
        try:
            updates = []
    ...
    async def get_guides(self, mode: str = "br") -> Dict[str, Dict]:
        ...
        try:
            ...
            for guide in result:
                ...
                photos = await self.execute_query(
                    query_photos, (guide_id,), fetch_all=True
                )
                ...
                videos = await self.execute_query(
                    query_videos, (guide_id,), fetch_all=True
                )
    ...
    async def clear_guide_media(self, key: str, mode: str = "br") -> bool:
        ...
        try:
            ...
            async with self.transaction() as conn:
                async with conn.cursor() as cursor:
                    await cursor.execute(
                        "DELETE FROM guide_photos WHERE guide_id = %s", (guide_id,)
                    )
                    await cursor.execute(
                        "DELETE FROM guide_videos WHERE guide_id = %s", (guide_id,)
                    )
    ...
    async def _add_guide_media(
        self, key: str, file_id: str, media_type: str, mode: str
    ) -> bool:
        ...
        try:
            async with self.transaction() as conn:
                async with conn.cursor() as cursor:
                    ...
                    table_name = (
                        "guide_photos" if media_type == "photo" else "guide_videos"
                    )
    ...
    async def update_scheduled_notification(
        ...
        try:
            ...
            if not sets:
                logger.warning(
                    "update_scheduled_notification called with no fields to update"
                )
                return False
    ...
    async def delete_scheduled_notification(self, schedule_id: int) -> bool:
        ...
        try:
            await self.execute_query(
                "DELETE FROM scheduled_notifications WHERE id = %s", (schedule_id,)
            )
            return True
        except Exception as e:
    ...
    async def mark_schedule_sent(
        self, schedule_id: int, last_sent_at, next_run_at
    ) -> bool:
    ...
    async def get_scheduled_notification_by_id(
