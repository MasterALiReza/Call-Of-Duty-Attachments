...
class AttachmentRepository(BaseRepository):
    ...
    @cached(
        ttl=600,
        key_func=lambda self, category, include_inactive=False: (
            f"weapons_in_cat:{category}:{include_inactive}"
        ),
    )
    async def get_weapons_in_category(
    ...
    @invalidate_cache_on_write(
        ["get_weapons_in_category", "get_all_category_counts", "cat_kb"]
    )
    ...
    @cached(
        ttl=300,
        key_func=lambda self, category, weapon_name, mode, limit=None, offset=None: (
            f"weapon_attachments:{category}:{weapon_name}:{mode}:{limit}:{offset}"
        ),
    )
    async def get_weapon_attachments(
        self,
        category: str,
        weapon_name: str,
        mode: str,
        ...
        try:
            ...
            log_exception(
                logger, e, f"get_weapon_attachments({category}, {weapon_name})"
            )
            return []

    async def get_top_attachments(
        self, category: str, weapon_name: str, mode: str = "br"
    ) -> List[Dict]:
        """دریافت 5 اتچمنت برتر"""
        try:
            query = "\n                SELECT \n                    a.id, a.code, a.name, \n                    a.image_file_id as image,\n                    a.is_top, a.is_season_top as season_top\n                FROM attachments a\n                JOIN weapons w ON a.weapon_id = w.id\n                JOIN weapon_categories c ON w.category_id = c.id\n                WHERE c.name = %s \n                  AND w.name = %s \n                  AND a.mode = %s \n                  AND a.is_top = TRUE\n                ORDER BY a.order_index NULLS LAST, a.id\n                LIMIT 5\n            "
            results = await self.execute_query(
                query, (category, weapon_name, mode), fetch_all=True
            )
            return results
        except Exception as e:
    ...
    async def get_all_attachments(
        ...
        return cast(
            List[Dict],
            await self.get_weapon_attachments(
                category, weapon_name, mode, limit=limit, offset=offset
            ),
        )
    ...
    async def search(self, query_text: str) -> List[Dict]:
        ...
        try:
            ...
            results = await self.execute_query(
                sql_query,
                (search_param, search_param, search_param, query_text, query_text),
                fetch_all=True,
            )
            ...
            if isinstance(db, DatabasePostgres) and db.fuzzy_engine:
                ...
                similar_weapons = [
                    name
                    for name, score in fuzzy_matches.get("weapons", [])
                    if score >= 80
                ]
                similar_codes = [
                    code
                    for code, score in fuzzy_matches.get("codes", [])
                    if score >= 85
                ]
                ...
                if similar_weapons or similar_codes:
                    additional_query = """
                        ...
                        LIMIT 10
                    """
                    fuzzy_results = await self.execute_query(
                        additional_query,
                        (similar_weapons, similar_codes),
                        fetch_all=True,
                    )
    ...
    @invalidate_cache_on_write(
        ["get_weapon_attachments", "get_top_attachments", "get_weapon_info"]
    )
    async def set_top_attachments(
        self,
        category: str,
        weapon_name: str,
    ...
    async def add_attachment(
        ...
        try:
            ...
            log_exception(
                logger, e, f"add_attachment({category}, {weapon_name}, {code})"
            )
            return False
    ...
    async def update_attachment_code(
        self,
        category: str,
        weapon_name: str,
        old_code: str,
        ...
        try:
            ...
            result = await self.execute_query(
                query, (new_code, category, weapon_name, old_code, mode)
            )
            ...
            log_exception(
                logger,
                e,
                f"update_attachment_code({category}, {weapon_name}, {old_code}, {new_code})",
            )
            return False
    ...
    async def edit_attachment(
        ...
        try:
            ...
            if new_name is not None or new_image is not None:
                ok = await self.update_attachment(
                    category=category,
            ...
            if new_code:
                ok = await self.update_attachment_code(
                    category=category,
                    weapon_name=weapon_name,
                    old_code=code,
                    new_code=new_code,
            ...
            log_exception(
                logger, e, f"edit_attachment({category}, {weapon_name}, {code})"
            )
            return False
    ...
    async def get_season_top_attachments_for_weapon(
        self, category: str, weapon_name: str, mode: str = "br"
    ) -> List[Dict]:
    ...
    async def get_weapon_by_name(
        self, category: str, weapon_name: str
    ) -> Optional[dict]:
        """دریافت اطلاعات سلاح بر اساس نام و دسته"""
        try:
            query = "\n                SELECT w.* \n                FROM weapons w\n                JOIN weapon_categories c ON w.category_id = c.id\n                WHERE c.name = %s AND w.name = %s\n            "
            result = await self.execute_query(
                query, (category, weapon_name), fetch_one=True
            )
    ...
    async def get_weapon_info(self, category: str, weapon_name: str) -> dict:
        ...
        try:
            ...
            result = await self.execute_query(
                query_weapon, (category, weapon_name), fetch_one=True
            )
            if not result:
                return {
                    "br": {"attachment_count": 0, "top_count": 0},
            ...
            for mode in ["br", "mp"]:
                ...
                counts = (
                    await self.execute_query(
                        query_counts, (weapon_id, mode), fetch_one=True
                    )
                    or {}
                )
                info[mode] = {
                    "attachment_count": counts.get("total", 0) or 0,
                    "top_count": counts.get("top_count", 0) or 0,
                }
            return info
        except Exception as e:
    ...
    async def delete_weapon(
        self, category: str, weapon_name: str, mode: str = None
    ) -> bool:
        ...
        try:
            async with self.transaction() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if mode is None:
                        logger.warning(
                            f"⚠️ Weapon deletion attempted but blocked: {category}/{weapon_name}"
                        )
                        return False
                    await cursor.execute(
                        """
                ...
                logger.info(
                    f"✅ Weapon attachments deleted: {category}/{weapon_name} ({mode})"
                )
                return True
        except Exception as e:
        ...
        try:
            async with self.transaction() as conn:
                ...
                if result:
                    ...
                    logger.info(
                        f"✅ Weapon status toggled: {category}/{weapon_name} -> {('Active' if new_status else 'Inactive')}"
                    )
                    return True
                return False
        except Exception as e:
    ...
    async def add_suggested_attachment(
        self,
        attachment_id: int,
        mode: str,
        priority: int = 999,
        ...
        try:
            ...
            await self.execute_query(
                query, (attachment_id, mode, priority, reason, added_by)
            )
            logger.info(
                f"✅ Attachment {attachment_id} added to suggested list ({mode})"
            )
            return True
        except Exception as e:
            log_exception(
                logger, e, f"add_suggested_attachment({attachment_id}, {mode})"
            )
            return False
    ...
    async def remove_suggested_attachment(self, attachment_id: int, mode: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ Attachment {attachment_id} removed from suggested list ({mode})"
            )
            return True
        except Exception as e:
            log_exception(
                logger, e, f"remove_suggested_attachment({attachment_id}, {mode})"
            )
            return False
    ...
    async def is_attachment_suggested(self, attachment_id: int, mode: str) -> bool:
        ...
        try:
            ...
            result = await self.execute_query(
                query, (attachment_id, mode), fetch_one=True
            )
    ...
    async def get_suggested_ranked(
        self, mode: str, category: str = None, weapon: str = None
    ) -> List[Dict]:
