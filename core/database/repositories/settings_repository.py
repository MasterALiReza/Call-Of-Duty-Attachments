...
class SettingsRepository(BaseRepository):
    ...
    async def set_setting(
        ...
        try:
            ...
            await self.execute_query(
                query, (key, value, description, category, updated_by)
            )
    ...
    async def add_blacklisted_word(
        ...
        try:
            ...
            await self.execute_query(
                query, (word.lower(), category, severity, admin_id)
            )
    ...
    async def get_user_submission_stats(self, user_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            try:
                ...
                log_exception(
                    logger, e, f"get_user_submission_stats->create_new({user_id})"
                )
                return defaults
        except Exception as e:
    ...
    async def update_submission_stats(
        ...
        increment_daily: bool = False,
        add_violation: int = 0,
        add_strike: float = 0.0,
    ) -> bool:
    ...
    async def update_ua_setting(
        self, key: str, value: str, admin_id: int = None
    ) -> bool:
    ...
    async def set_user_attachment_setting(
        self, key: str, value: str, admin_id: int = None
    ) -> bool:
    ...
    async def backup_database(self, backup_dir: str = "backups") -> str | None:
        ...
        try:
            ...
            db = os.getenv("POSTGRES_DB", "ox_loadout_bot")
            cmd = [
                "pg_dump",
                "-h",
                host,
            ...
            try:
                if postgres_password:
                    ...
                    with open(pgpass_file, "w", encoding="utf-8") as f:
                        f.write(f"{host}:{port}:*:{user}:{postgres_password}\n")
                    try:
