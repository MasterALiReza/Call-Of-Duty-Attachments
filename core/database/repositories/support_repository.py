...
class SupportRepository(BaseRepository):
    ...
    def _is_faq_language_schema_error(exc: Exception) -> bool:
        ...
        return "column" in error_str and (
            "lang" in error_str or "language" in error_str
        )
    ...
    async def _ensure_faq_not_helpful_count_column(self) -> None:
        try:
            ...
            raise InfrastructureError(
                "Failed to repair FAQ not_helpful_count schema."
            ) from e
    ...
    async def get_ticket(self, ticket_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            result = await self.execute_query(
                query_ticket, (ticket_id,), fetch_one=True
            )
            if not result:
            ...
            attachments = await self.execute_query(
                query_att, (ticket_id,), fetch_all=True
            )
            ticket["attachments"] = (
                [row["file_id"] for row in attachments] if attachments else []
            )
            return ticket
        except Exception as e:
        ...
        try:
            if status:
                ...
                results = await self.execute_query(
                    query, (user_id, status), fetch_all=True
                )
            else:
                query = "\n                    SELECT * FROM tickets \n                    WHERE user_id = %s \n                    ORDER BY created_at DESC\n                "
            ...
            for ticket in tickets:
                query_att = "\n                    SELECT file_id FROM ticket_attachments \n                    WHERE ticket_id = %s AND reply_id IS NULL\n                "
                attachments = await self.execute_query(
                    query_att, (ticket["id"],), fetch_all=True
                )
                ticket["attachments"] = (
                    [row["file_id"] for row in attachments] if attachments else []
                )
    ...
    async def get_all_tickets(
        self, status: Optional[str] = None, assigned_to: Optional[int] = None
    ) -> List[Dict]:
    ...
    async def get_ticket_replies(self, ticket_id: int) -> List[Dict]:
        ...
        try:
            ...
            for reply in replies:
                ...
                attachments = await self.execute_query(
                    query_att, (reply["id"],), fetch_all=True
                )
                reply["attachments"] = (
                    [row["file_id"] for row in attachments] if attachments else []
                )
    ...
    async def search_tickets(self, query: str) -> List[Dict]:
        ...
        try:
            ...
            results = await self.execute_query(
                query_sql, (search_term, search_term, search_term), fetch_all=True
            )
    ...
    async def _ensure_faqs_language_column(self):
        ...
        try:
            async with self.transaction() as conn:
                async with conn.cursor() as cursor:
                    await cursor.execute(
                        "SELECT 1 FROM information_schema.columns WHERE table_name='faqs' AND column_name='language'"
                    )
                    if await cursor.fetchone():
                        return
                    await cursor.execute(
                        "SELECT 1 FROM information_schema.columns WHERE table_name='faqs' AND column_name='lang'"
                    )
                    if await cursor.fetchone():
                        await cursor.execute(
                            "ALTER TABLE faqs RENAME COLUMN lang TO language"
                        )
                        logger.info("Renamed 'lang' column to 'language' in faqs table")
                    else:
                        await cursor.execute(
                            "ALTER TABLE faqs ADD COLUMN IF NOT EXISTS language VARCHAR(8) NOT NULL DEFAULT 'fa'"
                        )
        ...
        for attempt in range(2):
            try:
                if category and lang:
                    ...
                    results = await self.execute_query(
                        query, (category, lang), fetch_all=True
                    )
                elif category:
                    query = "SELECT * FROM faqs WHERE category = %s ORDER BY views DESC"
                    results = await self.execute_query(
                        query, (category,), fetch_all=True
                    )
                elif lang:
                ...
                if attempt == 0 and self._is_faq_language_schema_error(e):
                    logger.warning(
                        f"Schema mismatch in get_faqs, attempting fix... Error: {e}"
                    )
                    try:
    ...
    async def add_faq(
        self, question: str, answer: str, category: str = "general", lang: str = "fa"
    ) -> bool:
    ...
    async def mark_faq_helpful(self, faq_id: int, helpful: bool = True) -> bool:
        ...
        try:
            if helpful:
                query = (
                    "UPDATE faqs SET helpful_count = helpful_count + 1 WHERE id = %s"
                )
                await self.execute_query(query, (faq_id,))
                return True
            else:
                try:
                    ...
                    try:
                        ...
                        log_exception(
                            logger, inner2, "mark_faq_not_helpful_migration_failed"
                        )
                        return False
        except Exception as e:
