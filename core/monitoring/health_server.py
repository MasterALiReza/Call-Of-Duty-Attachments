...
class HealthServer:
    ...
    async def index(self, request):
        return web.Response(
            text="OX_LOADOUT Bot Monitoring Server is running.", content_type="text/plain"
        )
    ...
    async def health_check(self, request):
        ...
        health = {
            ...
            "services": {"database": "unknown", "telegram": "unknown"},
        }
        ...
        return web.json_response(
            health, status=200 if health["status"] == "pass" else 503
        )
    ...
    async def metrics(self, request):
        ...
        lines = [
            "# HELP bot_uptime_hours Hours since bot started",
            "# TYPE bot_uptime_hours gauge",
        ...
        for port in ports_to_try:
            try:
                ...
                if port != self.port:
                    ...
                    logger.info(
                        f"Health check server started at http://{self.host}:{port}"
                    )
