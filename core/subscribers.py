...
async def on_user_registered(
    user_id: int, user: User, is_new_user: bool, context: CustomContext, **kwargs
):
    """
...
async def on_attachment_submitted(
    ...
    **kwargs,
    ...
    """
    logger.info(
        f"[Notification] Event: ATTACHMENT_SUBMITTED for attachment {attachment_id}"
    )
    try:
