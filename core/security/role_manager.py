...
class RoleManager:
    ...
    async def _init_predefined_roles(self):
        ...
        try:
            ...
            obsolete_roles = [
                row for row in all_db_roles if row.get("name") not in ALLOWED_ROLE_NAMES
            ]

            if obsolete_roles:
                logger.info(
                    f"🧹 Found {len(obsolete_roles)} obsolete roles to purge: {[r['name'] for r in obsolete_roles]}"
                )
                    ...
                    try:
                        ...
                        async with self.db.transaction() as conn:
                            async with conn.cursor() as cursor:
                                ...
                                await cursor.execute(
                                    "DELETE FROM admin_roles WHERE role_id = %s",
                                    (r_id,),
                                )
                                await cursor.execute(
                                    "DELETE FROM roles WHERE id = %s", (r_id,)
                                )
        ...
        if SUPER_ADMIN_ID:
            await self.db.users.assign_role_to_admin(
                user_id=SUPER_ADMIN_ID,
                role_name="super_admin",
                display_name="Super Admin",
                added_by=SUPER_ADMIN_ID,
            )
            await self.clear_user_cache(SUPER_ADMIN_ID)
            logger.info(
                f"✅ Ensured SUPER_ADMIN_ID ({SUPER_ADMIN_ID}) has super_admin role."
            )
        ...
        if self._roles_cache is None:
            async with self._cache_lock:
                if self._roles_cache is None:
                    ...
                    def _safe_perms(perms):
                        ...
                        for p in perms:
                            try:
                                ...
                                logger.warning(
                                    f'⚠️ Skipping unknown permission in DB: "{p}"'
                                )
                        return set(valid)
                    ...
                    logger.info(
                        f"📦 Loaded {len(self._roles_cache)} role definitions into cache"
                    )
    ...
    async def assign_role(self, user_id: int, role_name: str) -> bool:
        ...
        if not role:
            ...
            return False
        success = await self.db.users.assign_role_to_admin(user_id, role_name)
        if success:
    ...
    async def remove_role(self, user_id: int) -> bool:
        """حذف نقش کاربر"""
        success = await self.db.users.remove_admin(user_id)
        if success:
            await self.clear_user_cache(user_id)
        return success
        ...
        if not await self.role_manager.is_admin(user_id):
            if update.callback_query:
                await update.callback_query.answer(
                    "❌ شما دسترسی ادمین ندارید.", show_alert=True
                )
            else:
...
def require_permission(*required_permissions: Permission):
    """
    Decorator برای بررسی دسترسی کاربر به یک یا چند دسترسی خاص
    ...
    def decorator(func):
        ...
        @wraps(func)
        async def wrapper(
            self, update: Update, context: CustomContext, *args, **kwargs
        ):
            user_id = update.effective_user.id
            permission_name = (
                "|".join((perm.value for perm in required_permissions))
                or "ADMIN_ACCESS"
            )
            ...
            async def _audit_denial(reason: str) -> None:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to audit permission denial for %s: %s",
                        func.__name__,
                        exc,
                    )
            ...
            if not has_permission:
                ...
                logger.warning(
                    f"User {user_id} tried to access {func.__name__} without permission: {permission_names}"
                )
                await _audit_denial("permission_denied")
                if update.callback_query:
                    await update.callback_query.answer(
                        "❌ شما دسترسی به این بخش را ندارید.", show_alert=True
                    )
                else:
...
def require_super_admin(func):
    ...
    async def wrapper(self, update: Update, context: CustomContext, *args, **kwargs):
        ...
        if not await self.role_manager.is_super_admin(user_id):
            if update.callback_query:
                await update.callback_query.answer(
                    "❌ فقط ادمین کل به این بخش دسترسی دارد.", show_alert=True
                )
            else:
                await update.message.reply_text(
                    "❌ فقط ادمین کل به این بخش دسترسی دارد."
                )
            return None
        return await func(self, update, context, *args, **kwargs)
...
def format_permissions_list(permissions: Set[Permission]) -> str:
    ...
    return "\n".join(
        [
            f"  • {get_permission_display_name(p)}"
            for p in sorted(permissions, key=lambda x: x.value)
        ]
    )
