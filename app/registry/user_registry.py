...
from handlers.user.modules.feedback import FEEDBACK_TEXT
...
class UserHandlerRegistry(BaseHandlerRegistry):
    ...
    def _register_commands(self):
        ...
        self.application.add_handler(
            CommandHandler("subscribe", self.notification_handler.subscribe_cmd)
        )
        self.application.add_handler(
            CommandHandler("unsubscribe", self.notification_handler.unsubscribe_cmd)
        )
    ...
    def _register_search_conversation(self):
        ...
        search_conv = ConversationHandler(
            entry_points=[
                CallbackQueryHandler(
                    self.search_handler.search_start, pattern="^search$"
                ),
                CallbackQueryHandler(
            ...
            states={
                SEARCHING: [
                    ...
                    MessageHandler(
                        filters.Regex(build_regex_for_key("menu.buttons.search")),
                        self.search_handler.search_restart_silently,
                    ),
                    # دکمه‌های دیگه - لغو جستجو و رفتن به بخش دیگه
                    MessageHandler(
                        filters.Regex(build_regex_for_key("menu.buttons.get")),
                        self._search_cancel_and_show_mode_selection,
                    ),
                    MessageHandler(
                        filters.Regex(build_regex_for_key("menu.buttons.season_top")),
                        self._search_cancel_and_season_top,
                    ),
                    MessageHandler(
                        filters.Regex(build_regex_for_key("menu.buttons.season_list")),
                        self._search_cancel_and_season_list,
                    ),
                    MessageHandler(
                        filters.Regex(build_regex_for_key("menu.buttons.suggested")),
                        self._search_cancel_and_suggested,
                    ),
                    # CMS: خروج از جستجو و نمایش CMS
                    MessageHandler(
                        filters.Regex(build_regex_for_key("menu.buttons.cms")),
                        self._search_cancel_and_cms,
                    ),
                    MessageHandler(
                        filters.Regex(
                            build_regex_for_key("menu.buttons.game_settings")
                        ),
                        self._search_cancel_and_game_settings,
                    ),
                    MessageHandler(
                        filters.Regex(build_regex_for_key("menu.buttons.help")),
                        self._search_cancel_and_help,
                    ),
                    MessageHandler(
                        filters.Regex(build_regex_for_key("menu.buttons.contact")),
                        self.contact_handlers.search_cancel_and_contact,
                    ),
                    MessageHandler(
                        filters.Regex(build_regex_for_key("menu.buttons.notify")),
                        self._search_cancel_and_notifications,
                    ),
                    MessageHandler(
                        filters.Regex(build_regex_for_key("menu.buttons.leaderboard")),
                        self._search_cancel_and_leaderboard,
                    ),
                    MessageHandler(
                        filters.Regex(build_regex_for_key("menu.buttons.admin")),
                        self.admin_handlers.search_cancel_and_admin,
                    ),
                    ...
                    MessageHandler(
                        ...
                        self.search_handler.search_process,
                    ),
                    # فال‌بک نهایی برای ورودی‌های غیرمتنی
                    MessageHandler(
                        filters.ALL, self.search_handler.handle_invalid_input
                    ),
                ]
            },
            fallbacks=[
    ...
    async def _search_cancel_and_show_categories(
    ...
    async def _search_cancel_and_season_top(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
    ...
    async def _search_cancel_and_season_list(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        if self.season_handler:
    ...
    async def _search_cancel_and_suggested(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        if self.suggested_handler:
            await self.suggested_handler.suggested_attachments_select_mode_msg(
                update, context
            )
        return ConversationHandler.END

    async def _search_cancel_and_game_settings(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
    ...
    async def _search_cancel_and_help(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
    ...
    async def _search_cancel_and_notifications(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
    ...
    async def _search_cancel_and_show_mode_selection(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        if self.category_handler:
    ...
    async def _search_cancel_and_cms(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
    ...
    async def _search_cancel_and_leaderboard(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
    ...
    def _register_callback_handlers(self):
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.notification_handler.view_attachment_from_notification,
                pattern="^attm__",
            ),
            group=-1,
        )
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.category_handler.show_mode_selection, pattern="^categories$"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.category_handler.show_mode_selection, pattern="^select_mode_first$"
            )
        )
        # انتخاب مود (MP/BR) در ابتدای فلوی دریافت اتچمنت
        self.application.add_handler(
            CallbackQueryHandler(
                self.category_handler.mode_selected, pattern="^mode_(mp|br)$"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(self.weapon_handler.show_weapons, pattern="^cat_")
        )
        self.application.add_handler(
            CallbackQueryHandler(self.weapon_handler.show_weapon_menu, pattern="^wpn_")
        )
        # Handler برای انتخاب mode بعد از انتخاب سلاح (BR/MP در سطح weapon)
        self.application.add_handler(
            CallbackQueryHandler(
                self.weapon_handler.show_mode_menu, pattern="^mode_(?!mp$|br$)"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.top_handler.show_top_attachments, pattern="^show_top$"
            )
        )
        # نمایش همه اتچمنت‌ها + مسیر مستقیم از نتایج جستجو: all_{category}__{weapon}
        self.application.add_handler(
            CallbackQueryHandler(
                self.all_handler.show_all_attachments,
                pattern="^show_all$|^all_page_|^all_",
            )
        )
        # نتیجه سریع جستجو: qatt_{category}__{weapon}__{mode}__{code}
        self.application.add_handler(
            CallbackQueryHandler(
                self.search_handler.send_attachment_quick, pattern="^qatt_"
            )
        )
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.all_handler.attachment_detail_with_mode, pattern=r"^attm_(?!_)"
            )
        )
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.all_handler.attachment_detail,
                pattern=r"^att_(?!top_|season_|like_|dislike_|fb_|copy_)",
            )
        )

        # دیگر handlers
        self.application.add_handler(
            CallbackQueryHandler(self.help_handler.help_command, pattern="^help$")
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.main_menu_handler.main_menu, pattern="^main_menu$"
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.cms_user_handler.cms_type_selected, pattern="^cms_type_"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.cms_user_handler.cms_view, pattern="^cms_view_\\d+$"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.cms_user_handler.cms_list_page_navigation,
                pattern="^cmslist_page_\\d+$",
            )
        )
        # تنظیمات ربات (کاربر)
        self.application.add_handler(
            CallbackQueryHandler(
                self.language_handler.open_user_settings, pattern="^user_settings_menu$"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.language_handler.open_language_menu,
                pattern="^user_settings_language$",
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.language_handler.set_language, pattern="^set_lang_(fa|en)$"
            )
        )
        # تنظیمات بازی
        self.application.add_handler(
            CallbackQueryHandler(
                self.guides_handler.game_settings_menu, pattern="^game_settings_menu$"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.guides_handler.game_settings_mode_selected,
                pattern="^game_settings_(br|mp)$",
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.guides_handler.show_guide_inline, pattern="^show_guide_"
            )
        )
    ...
    def _register_season_top_handlers(self):
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.season_handler.season_top_select_mode, pattern="^season_top$"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.season_handler.season_top_media_with_mode,
                pattern="^season_top_mode_",
            )
        )
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.season_handler.season_top_list_select_mode,
                pattern="^season_top_list$",
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.season_handler.season_top_list_with_mode,
                pattern="^season_list_mode_",
            )
        )
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.season_handler.season_top_list_page_navigation,
                pattern="^slist_page_",
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.season_handler.season_top_item_detail, pattern="^satt_"
            )
        )
    ...
    def _register_suggested_handlers(self):
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.suggested_handler.suggested_attachments_select_mode,
                pattern="^suggested_attachments$",
            )
        )
        # نمایش لیست سلاح‌ها (بعد از انتخاب mode)
        self.application.add_handler(
            CallbackQueryHandler(
                self.suggested_handler.suggested_media_with_mode,
                pattern="^suggested_mode_",
            )
        )
        # نمایش لیست اتچمنت‌های یک سلاح
        self.application.add_handler(
            CallbackQueryHandler(
                self.suggested_handler.suggested_weapon_attachments,
                pattern="^sugg_wpn_",
            )
        )
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.suggested_handler.suggested_send_attachment, pattern="^sugg_send_"
            )
        )
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.suggested_handler.suggested_list_with_mode,
                pattern="^suggested_list_mode_",
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.suggested_handler.suggested_list_page_navigation,
                pattern="^sugglist_page_",
            )
        )
    ...
    def _register_feedback_handlers(self):
        ...
        feedback_conv_handler = ConversationHandler(
            entry_points=[
                CallbackQueryHandler(
                    self.feedback_handler.handle_feedback_request,
                    pattern=r"^att_fb_\d+$",
                )
            ],
            states={
                FEEDBACK_TEXT: [
                    ...
                    CallbackQueryHandler(
                        self.feedback_handler.handle_feedback_cancel,
                        pattern="^att_fb_cancel_",
                    ),
                ]
            },
            fallbacks=[
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.feedback_handler.handle_vote_like, pattern=r"^att_like_\d+$"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.feedback_handler.handle_vote_dislike, pattern=r"^att_dislike_\d+$"
            )
        )
        # Callback handler برای کپی کد
        self.application.add_handler(
            CallbackQueryHandler(
                self.feedback_handler.handle_copy_code, pattern=r"^att_copy_\d+$"
            )
        )
    ...
    def _register_notification_handlers(self):
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.notification_handler.notification_toggle,
                pattern="^user_notif_toggle$",
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.notification_handler.notification_toggle_mode,
                pattern="^user_notif_mode_",
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.notification_handler.notification_events_menu,
                pattern="^user_notif_events$",
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.notification_handler.notification_toggle_event,
                pattern="^user_notif_event_",
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                self.notification_handler.notification_settings,
                pattern="^user_notif_back$",
            )
        )
    ...
    def _register_analytics_handlers(self):
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.leaderboard_handler.show_leaderboard, pattern="^leaderboard$"
            )
        )
