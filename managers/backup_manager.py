...
class BackupManager:
    ...
    async def _backup_postgres(self) -> Optional[str]:
        ...
        try:
            ...
            if not pg_dump:
                logger.error(
                    "pg_dump command not found. Install PostgreSQL client tools or set PG_DUMP_PATH to the full path of pg_dump."
                )
                return None
            ...
            logger.error(
                "pg_dump command not found. Install PostgreSQL client tools or set PG_DUMP_PATH."
            )
            return None
    ...
    async def restore_postgres_backup(self, backup_file: str) -> bool:
        ...
        try:
            ...
            if not pg_restore:
                logger.error(
                    "pg_restore command not found. Install PostgreSQL client tools or set PG_RESTORE_PATH to the full path of pg_restore."
                )
                return False
            ...
            if backup_file.endswith(".zip"):
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    # پیدا کردن فایل .dump
                    dump_files = [
                        f for f in os.listdir(temp_dir) if f.endswith(".dump")
                    ]
                    if not dump_files:
            ...
            logger.error(
                "pg_restore command not found. Install PostgreSQL client tools or set PG_RESTORE_PATH."
            )
            return False
        except Exception as e:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if json_files:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to import JSON backup into PostgreSQL: {e}"
                        )
        ...
        try:
            if output_file is None:
                output_file = os.path.join(
                    self.backup_dir, f"export_{self._get_timestamp()}.json"
                )
            ...
            export_data = {
                ...
                "data": {"weapons": {}, "guides": {}, "channels": [], "users": []},
            }
            ...
            for category in categories:
                ...
                for weapon in weapons:
                    export_data["data"]["weapons"][category][weapon] = {
                        "br": {"top_attachments": [], "all_attachments": []},
                        "mp": {"top_attachments": [], "all_attachments": []},
                    }

                    for mode in ["br", "mp"]:
                        # Get top attachments
                        top_atts = await self.db.attachments.get_top_attachments(
                            category, weapon, mode
                        )
                        export_data["data"]["weapons"][category][weapon][mode][
                            "top_attachments"
                        ] = top_atts

                        # Get all attachments
                        all_atts = await self.db.attachments.get_all_attachments(
                            category, weapon, mode
                        )
                        export_data["data"]["weapons"][category][weapon][mode][
                            "all_attachments"
    ...
    async def export_to_csv(self, output_dir: str = None) -> Optional[str]:
        ...
        try:
            ...
            with open(channels_file, "w", newline="", encoding="utf-8") as f:
                ...
                for ch in channels:
                    writer.writerow(
                        [ch.get("channel_id"), ch.get("title"), ch.get("url")]
                    )
                        ...
                        if "br" in weapon_data or "mp" in weapon_data:
                            ...
                            for mode in ["br", "mp"]:
                                ...
                                if "top_attachments" in mode_data:
                                    ...
                                    if top_codes:
                                        await self.db.attachments.set_top_attachments(
                                            category, weapon_name, top_codes, mode
                                        )
                        else:
                            ...
                            if "top_attachments" in weapon_data:
                                ...
                                if top_codes:
                                    await self.db.attachments.set_top_attachments(
                                        category, weapon_name, top_codes, "br"
                                    )
