...
class ChannelManager:
    ...
    async def _check_single_channel(
        self, user_id: int, channel: Dict[str, str], bot
        ...
        try:
            ...
            logger.info(
                f"کاربر {user_id} در کانال {channel['title']}: status={member.status}"
            )
            ...
            if member.status in [
                ...
                logger.warning(
                    f"کاربر {user_id} عضو کانال {channel['title']} نیست (status={member.status})"
                )
            ...
            logger.error(
                f"خطا در بررسی عضویت کانال {channel.get('channel_id', 'unknown')} ({channel.get('title', 'unknown')}): {error_type}: {e}"
            )
    ...
    async def check_user_membership(
        self, user_id: int, bot, use_cache: bool = True
        ...
        with _cache_lock:
            ...
            if use_cache and user_id in _membership_cache:
                ...
                if now - timestamp < ttl:
                    logger.debug(
                        f"استفاده از cache برای کاربر {user_id} (TTL={'30m' if is_member else '2m'})"
                    )
        ...
        tasks = [
            self._check_single_channel(user_id, channel, bot) for channel in channels
        ]
    ...
    def create_join_keyboard(
        self, channels: List[Dict[str, str]], lang: str = "fa"
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    t("membership.cta.joined", lang), callback_data="check_membership"
                )
            ]
        )
    ...
    def create_membership_message(
        ...
        if is_first_time:
            message = (
                t("membership.title.first", lang, app_name=t("app.name", lang)) + "\n\n"
            )
        ...
        for i, channel in enumerate(channels, 1):
            message += (
                t("membership.list.item", lang, i=i, title=channel["title"]) + "\n"
            )
    ...
    async def wrapper(
        ...
        if not db:
            ...
            try:
                ...
                logger.error(
                    f"Failed to send membership error message when database is missing: {e}"
                )
            return None
...
async def _send_main_menu(query, context: ContextTypes.DEFAULT_TYPE, db, user_id: int):
    ...
    # تعیین زبان کاربر
    try:
        lang = await db.users.get_user_language(user_id) or DEFAULT_LANG
    except Exception:
        lang = DEFAULT_LANG
...
async def check_membership_callback(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_allowed, wait_time = await rate_limiter.check_rate_limit(
        f"membership_check_{user_id}", user_check_limit
    )

    if not is_allowed:
        await query.answer(
            t("rate_limit.wait_seconds", lang, seconds=int(wait_time)), show_alert=True
        )
    ...
    if network_error:
        ...
        await query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            t("common.retry", lang), callback_data="check_membership"
                        )
                    ]
                ]
            ),
        ...
        try:
            ...
            lang = DEFAULT_LANG
        message = channel_manager.create_membership_message(
            not_joined, is_first_time=False, lang=lang
        )
        keyboard = channel_manager.create_join_keyboard(not_joined, lang=lang)
