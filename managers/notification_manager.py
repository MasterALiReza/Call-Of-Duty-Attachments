...
class NotificationManager:
    ...
    async def queue_notification(
        ...
        logger.info(
            f"[NotifManager] Queue notification: {event_type} - Payload: {payload}"
        )

        # بررسی وضعیت کلی سیستم
        settings = await get_notification_settings(self.db)
        if not settings.get("enabled", True):
            logger.info("[NotifManager] System disabled, skipping")
            return
        ...
        task = asyncio.create_task(self._send_batch_notification(context, group_key))
        self._batch_tasks[group_key] = task

    async def _send_batch_notification(
        self, context: ContextTypes.DEFAULT_TYPE, group_key: str
    ):
        ...
        try:
            ...
            active_users = await self._get_active_users_for_events(
                [e["type"] for e in events], base_info["mode"]
            )
            ...
            if active_users:
                ...
                for user_id in active_users:
                    ...
                    try:
                        ...
                        logger.error(
                            f"[NotifManager] Error getting language for user {user_id}: {e}"
                        )
                    if not user_lang:
                ...
                for lang, users in lang_to_users.items():
                    ...
                    message = await self._build_combined_message(
                        base_info, events, lang=lang
                    )
                    logger.info(
                        f"[NotifManager] Built message ({lang}): {message[:100] if message else 'None'}..."
                    )
                    ...
                    await self._broadcast_to_users(
                        context, message, users, reply_markup
                    )
    ...
    async def _build_combined_message(
        self, base_info: dict, events: List[dict], lang: str = "fa"
    ) -> Optional[str]:
        ...
        weapon_line = t(
            "notification.combined.weapon", lang, weapon=base_info.get("weapon", "")
        )
        attachment_line = t(
    ...
    async def _get_active_users_for_events(
        self, event_types: List[str], mode: str
    ) -> Set[int]:
        ...
        try:
            ...
            if hasattr(self.db, "get_users_for_notification"):
                active_users = await self.db.users.get_users_for_notification(
                    event_types, mode
                )

                duration = (datetime.now() - start_time).total_seconds()
                logger.info(
                    f"[NotifManager] SQL Filter: Found {len(active_users)} users in {duration:.3f}s"
                )
                return set(active_users)

            # Fallback به روش قدیمی (کند) اگر متد جدید در دیتابیس نبود
            logger.warning(
                "[NotifManager] Optimized query not found, falling back to Python filtering"
            )
    ...
    async def _get_active_users_for_events_legacy(
        self, event_types: List[str], mode: str
    ) -> Set[int]:
        ...
        for user_id in all_subscribers:
            try:
                ...
                user_prefs = await self.db.settings.get_user_notification_preferences(
                    user_id
                )
    ...
    async def _broadcast_to_users(
        ...
        for user_id in stats["blocked_users"]:
            try:
                ...
                logger.warning(
                    f"Failed to remove blocked user {user_id} from subscribers: {e}"
                )
    ...
    async def update_user_preferences(self, user_id: int, preferences: dict) -> bool:
        ...
        return await self.db.settings.update_user_notification_preferences(
            user_id, preferences
        )
